...
contains
  ...
  function distribution_moments(distribution, mu, sigma, skew, shape, lambda) result(summary)
    ...
    if(present(lambda))lam=lambda
    if(s<=0.0_dp)then
    summary%status=tsd_invalid_argument
    summary%message='sigma must be positive'
    return
    end if
    summary%mean=m
    summary%standard_deviation=s
    ...
    if(.not.ieee_is_finite(summary%skewness).or..not.ieee_is_finite(summary%excess_kurtosis))then
      summary%status=tsd_invalid_argument
      summary%message='requested moment does not exist'
    else
      summary%status=tsd_success
      summary%message=''
    end if
  ...
  pure real(dp) function sstd_skewness(xi,eta) result(value)
    ...
    a=4.0_dp*lambda_s*cx*(eta-2.0_dp)/(eta-1.0_dp)
    b=sqrt(1.0_dp+3.0_dp*lambda_s**2-a*a)
  ...
  pure real(dp) function jsu_skewness(skew,shape) result(value)
    ...
    omega=-skew/shape
    w=exp(shape**(-2))
    s3=-0.25_dp*sqrt(w)*(w-1.0_dp)**2*(w*(w+2.0_dp)*sinh(3.0_dp*omega)+3.0_dp*sinh(omega))
    var=0.5_dp*(w-1.0_dp)*(w*cosh(2.0_dp*omega)+1.0_dp)
    value=s3/var**1.5_dp
  ...
  pure real(dp) function jsu_excess_kurtosis(skew,shape) result(value)
    ...
    omega=-skew/shape
    w=exp(shape**(-2))
    s4=0.125_dp*(w-1.0_dp)**2*(w*w*(w**4+2.0_dp*w**3+3.0_dp*w*w-3.0_dp)*cosh(4.0_dp*omega)+ &
      4.0_dp*w*w*(w+2.0_dp)*cosh(2.0_dp*omega)+3.0_dp*(2.0_dp*w+1.0_dp))
    var=0.5_dp*(w-1.0_dp)*(w*cosh(2.0_dp*omega)+1.0_dp)
    value=s4/(var*var)-3.0_dp
  ...
  subroutine gh_raw_central(skew,shape,lambda,var,c3,c4,raw2,raw3,raw4)
    ...
    chi=d*d
    psi=max(a*a-b*b,0.0_dp)
    w1=gig_raw_moment(1.0_dp,lambda,chi,psi)
    w2=gig_raw_moment(2.0_dp,lambda,chi,psi)
    w3=gig_raw_moment(3.0_dp,lambda,chi,psi)
    w4=gig_raw_moment(4.0_dp,lambda,chi,psi)
    r1=b*w1
    raw2=w1+b*b*w2
    raw3=3.0_dp*b*w2+b**3*w3
    raw4=3.0_dp*w2+6.0_dp*b*b*w3+b**4*w4
    var=raw2-r1*r1
    c3=raw3-3.0_dp*r1*raw2+2.0_dp*r1**3
    c4=raw4-4.0_dp*r1*raw3+6.0_dp*r1*r1*raw2-3.0_dp*r1**4
  ...
  real(dp) function ghst_excess_kurtosis(skew,shape) result(value)
    ...
    b2=b*b
    d2=d*d
    k1=6.0_dp/(2.0_dp*b2*d2+(shape-2.0_dp)*(shape-4.0_dp))**2
  ...
  function authorized_domain(distribution,max_kurt,n,lambda) result(result)
    ...
    kmax=30.0_dp
    if(present(max_kurt))kmax=max_kurt
    lam=1.0_dp
    if(present(lambda))lam=lambda
    nn=25
    if(present(n))nn=max(2,n)
    select case(distribution_id(distribution))
    case(dist_sstd)
    slow=1.0_dp
    sup=12.0_dp
    ...
    hup=60.0_dp
    case default
    result%status=tsd_invalid_argument
    result%message='authorized domain is defined for sstd, nig, gh, jsu, or ghst'
    return
    end select
    ...
    do i=1,nn
      ...
      result%skewness(i)=0.0_dp
      result%kurtosis(i)=target
      do j=0,ns
        sval=slow+(sup-slow)*real(j,dp)/real(ns,dp)
        do k=0,nh
          hval=hlow+(hup-hlow)*real(k,dp)/real(nh,dp)
          skv=dskewness(distribution,sval,hval,lam)
          kv=3.0_dp+dkurtosis(distribution,sval,hval,lam)
          ...
          if(abs(kv-target)<best_error .or. (abs(kv-target)<=best_error*1.02_dp .and. skv>result%skewness(i)))then
            best_error=abs(kv-target)
            result%skewness(i)=skv
            result%kurtosis(i)=kv
            result%skew_parameter(i)=sval
            result%shape_parameter(i)=hval
          end if
        end do
      end do
    end do
    result%status=tsd_success
    result%message='grid-constrained numerical boundary'
