...
! Supplied r_mod.f90 helper module; only formatting-only line wrapping was applied.
...
real(kind=dp), intent(in) :: x(:,:) ! input matrix
real(kind=dp), allocatable :: s(:,:)
character(len=*), parameter :: labels(6) = [character(len=8) :: "Min.   :", "1st Qu.:", "Median :", &
   & "Mean   :", "3rd Qu.:", "Max.   :"]
integer :: j
