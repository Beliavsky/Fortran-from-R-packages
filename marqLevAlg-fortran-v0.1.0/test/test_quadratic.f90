program test_quadratic
    ...
    call assert_solution(r)
    call marqlev_optimize(x0, f, g, r, ctl)
    call assert_solution(r)
    call marqlev_optimize(x0, f, g, info, r, ctl)
    call assert_solution(r)
...
contains
    ...
    subroutine info(x, h)
        ...
        if (size(x) /= 2) error stop 10
        h = 0.0_dp
        h(1, 1) = 8.0_dp
        h(2, 2) = 2.0_dp
    end subroutine info

    subroutine assert_solution(z)
        type(mla_result), intent(in) :: z
        if (z%istop /= 1) error stop 1
        if (maxval(abs(z%par - [5.0_dp, 6.0_dp])) > 2.0e-3_dp) error stop 2
        if (abs(z%fn_value) > 1.0e-6_dp) error stop 3
