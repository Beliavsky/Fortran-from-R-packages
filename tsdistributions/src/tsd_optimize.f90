...
module tsd_optimize
  ...
  use tsd_types, only : tsd_success, tsd_invalid_argument, tsd_no_convergence
  implicit none
  private
...
contains
  ...
  subroutine nelder_mead(fun, context, x, fval, status, iterations, max_iterations, tolerance, scale)
    ...
    n = size(x)
    if (n == 0) then
      fval = fun(x,context); status = tsd_invalid_argument; iterations = 0; return
    end if
    maxit = 2000
  ...
  subroutine parameters_to_unconstrained(par,lower,upper,z,status)
    ...
    if(size(lower)/=n .or. size(upper)/=n .or. any(lower>=upper)) then
      z=0.0_dp; status=tsd_invalid_argument; return
    end if
    do i=1,n
      p=par(i)
