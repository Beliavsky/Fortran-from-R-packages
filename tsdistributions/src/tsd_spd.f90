...
contains
  ...
  function estimate_spd(spec) result(fit)
    ...
    if(n<10 .or. spec%lower_probability<=0.0_dp .or. spec%upper_probability>=1.0_dp .or. &
      ...
      fit%message='invalid data or tail probabilities';return
    end if
    ys=spec%y;call sort_real(ys)
    allocate(fit%y(size(ys)))
    fit%y=ys;fit%lower_probability=spec%lower_probability;fit%upper_probability=spec%upper_probability
    ...
    nl=count(ys<fit%lower_threshold);nu=count(ys>fit%upper_threshold)
    if(nl<3 .or. nu<3)then;fit%message='too few tail exceedances';return;end if
    allocate(lo_exc(nl),up_exc(nu));nl=0;nu=0
    do i=1,n
      if(ys(i)<fit%lower_threshold)then;nl=nl+1;lo_exc(nl)=fit%lower_threshold-ys(i);end if
      if(ys(i)>fit%upper_threshold)then;nu=nu+1;up_exc(nu)=ys(i)-fit%upper_threshold;end if
    end do
    ...
    s=sample_sd(ys);fit%bandwidth=1.06_dp*s*real(n,dp)**(-0.2_dp)
    if(.not.ieee_is_finite(fit%bandwidth).or.fit%bandwidth<=0.0_dp)fit%bandwidth=max(s,1.0_dp)*0.1_dp
    fit%log_likelihood=0.0_dp
    do i=1,n;fit%log_likelihood=fit%log_likelihood+log(max(dspd(ys(i),fit),tiny(1.0_dp)));end do
    fit%aic=-2.0_dp*fit%log_likelihood+8.0_dp
    fit%bic=-2.0_dp*fit%log_likelihood+4.0_dp*log(real(n,dp))
    fit%status=tsd_success;fit%message='PWM tails and kernel interior fitted'
  ...
  real(dp) function dspd(x,fit,log_density) result(value)
    ...
    if(fit%status/=tsd_success)then;value=ieee_value(0.0_dp,ieee_quiet_nan);return;end if
  ...
  real(dp) function pspd(q,fit,lower_tail) result(value)
    ...
    value=min(max(value,0.0_dp),1.0_dp);if(.not.lower)value=1.0_dp-value
  end function pspd
  ...
  real(dp) function qspd(p,fit,lower_tail) result(value)
    ...
    integer::iter
    lower=.true.;if(present(lower_tail))lower=lower_tail
    pr=p;if(.not.lower)pr=1.0_dp-pr
    if(pr<0.0_dp.or.pr>1.0_dp.or.fit%status/=tsd_success)then;value=ieee_value(0.0_dp,ieee_quiet_nan);return;end if
    if(pr<fit%lower_probability)then
      value=fit%lower_threshold-gpd_quantile(1.0_dp-pr/fit%lower_probability,fit%lower_scale,fit%lower_shape);return
    else if(pr>fit%upper_probability)then
      value=fit%upper_threshold+gpd_quantile((pr-fit%upper_probability)/(1.0_dp-fit%upper_probability),fit%upper_scale,fit%upper_shape);return
    end if
    lo=fit%lower_threshold;hi=fit%upper_threshold
    do iter=1,100
      mid=0.5_dp*(lo+hi)
      if(pspd(mid,fit)<pr)then;lo=mid;else;hi=mid;end if
  ...
  function rspd(n,fit,rng) result(x)
    ...
    integer::i
    allocate(x(max(n,0)))
    do i=1,n;x(i)=qspd(uniform_rng(rng),fit);end do
  end function rspd

  pure real(dp) function gpd_density(x,scale,shape) result(value)
    real(dp),intent(in)::x,scale,shape
    real(dp)::z
    if(x<0.0_dp.or.scale<=0.0_dp)then;value=0.0_dp;return;end if
    if(abs(shape)<1.0e-10_dp)then;value=exp(-x/scale)/scale;return;end if
    z=1.0_dp+shape*x/scale
    if(z<=0.0_dp)then;value=0.0_dp;else;value=z**(-1.0_dp/shape-1.0_dp)/scale;end if
  end function gpd_density

  pure real(dp) function gpd_cdf(x,scale,shape) result(value)
    real(dp),intent(in)::x,scale,shape
    real(dp)::z
    if(x<=0.0_dp)then;value=0.0_dp;return;end if
    if(abs(shape)<1.0e-10_dp)then;value=1.0_dp-exp(-x/scale);return;end if
    z=1.0_dp+shape*x/scale
    if(z<=0.0_dp)then;value=1.0_dp;else;value=1.0_dp-z**(-1.0_dp/shape);end if
  ...
  pure real(dp) function gpd_quantile(p,scale,shape) result(value)
    ...
    if(abs(shape)<1.0e-10_dp)then;value=-scale*log(1.0_dp-pr);else;value=scale*((1.0_dp-pr)**(-shape)-1.0_dp)/shape;end if
  ...
  subroutine pwm_gpd(excess,scale,shape,covariance)
    ...
    integer::i,n
    n=size(excess);x=excess;call sort_real(x);mu=sum(x)/real(n,dp);a1=0.0_dp
    do i=1,n;pv=(real(i,dp)-0.35_dp)/real(n,dp);a1=a1+x(i)*(1.0_dp-pv);end do
    a1=a1/real(n,dp)
    if(abs(mu-2.0_dp*a1)<=tiny(1.0_dp))then;shape=0.0_dp;scale=max(mu,tiny(1.0_dp));else
      shape=2.0_dp-mu/(mu-2.0_dp*a1);scale=(2.0_dp*mu*a1)/(mu-2.0_dp*a1)
    end if
    shape=min(max(shape,-0.49_dp),0.49_dp);scale=max(scale,1.0e-12_dp);covariance=0.0_dp
  ...
  real(dp) function kde_density(x,fit) result(value)
    ...
    integer::i,n
    real(dp)::u
    n=size(fit%y);value=0.0_dp
    do i=1,n;u=(x-fit%y(i))/fit%bandwidth;value=value+kernel_pdf(u,fit%kernel_type);end do
  ...
  real(dp) function kde_cdf(x,fit) result(value)
    ...
    real(dp)::u
    n=size(fit%y);value=0.0_dp
    do i=1,n;u=(x-fit%y(i))/fit%bandwidth;value=value+kernel_cdf(u,fit%kernel_type);end do
  ...
  pure real(dp) function kernel_pdf(u,name) result(value)
    ...
    case('box');if(abs(u)<=1.0_dp)then;value=0.5_dp;else;value=0.0_dp;end if
    case('epanech','epanechnikov');if(abs(u)<=1.0_dp)then;value=0.75_dp*(1.0_dp-u*u);else;value=0.0_dp;end if
    case('biweight');if(abs(u)<=1.0_dp)then;value=15.0_dp/16.0_dp*(1.0_dp-u*u)**2;else;value=0.0_dp;end if
    case('triweight');if(abs(u)<=1.0_dp)then;value=35.0_dp/32.0_dp*(1.0_dp-u*u)**3;else;value=0.0_dp;end if
    case default;value=normal_pdf(u)
    end select
  ...
  pure real(dp) function kernel_cdf(u,name) result(value)
    real(dp),intent(in)::u
    character(len=*),intent(in)::name
    if(trim(adjustl(name))=='normal')then;value=normal_cdf(u);return;end if
    if(u<=-1.0_dp)then;value=0.0_dp;return;else if(u>=1.0_dp)then;value=1.0_dp;return;end if
    select case(trim(adjustl(name)))
    case('box');value=0.5_dp*(u+1.0_dp)
    case('epanech','epanechnikov');value=0.5_dp+0.75_dp*(u-u**3/3.0_dp)
    case('biweight');value=0.5_dp+15.0_dp/16.0_dp*(u-2.0_dp*u**3/3.0_dp+u**5/5.0_dp)
    case('triweight');value=0.5_dp+35.0_dp/32.0_dp*(u-u**3+3.0_dp*u**5/5.0_dp-u**7/7.0_dp)
    case default;value=normal_cdf(u)
    end select
