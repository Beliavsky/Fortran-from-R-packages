...
#' @aliases plindleygeometric dlindleygeometric hlindleygeometric qlindleygeometric rlindleygeometric
...
#' @return \code{plindleygeometric} gives the culmulative distribution function
#' @return \code{dlindleygeometric} gives the probability density function
#' @return \code{hlindleygeometric} gives the hazard rate function
#' @return \code{qlindleygeometric} gives the quantile function
#' @return \code{rlindleygeometric} gives the random number generatedy by distribution
...
#' @rdname LindleyGeometric
#' @export
dlindleygeometric <- function(x, lambda, theta)
{
...
#' @rdname LindleyGeometric
#' @export
hlindleygeometric <- function(x, lambda, theta)
{
  stopifnot(theta < 1,theta > 0,lambda > 0,x > 0)
  phi = theta * (1 - (lambda + 1 + lambda * x) / (lambda + 1) * exp(-lambda * x))
  #change form
...
#' @rdname LindleyGeometric
#' @export
qlindleygeometric <- function(p, lambda, theta)
{
  stopifnot(theta < 1,theta > 0,lambda > 0)
  #change form
  atheta = theta / (1 - theta)
  t0 = p * atheta / (1 + p * atheta)
  t1 = t0 / theta - 1
  #change form
...
#' #' @rdname LindleyGeometric
#' #' @export
#' rlindleygeometric <- function(n, lambda, theta)
#' {
...
rlindleygeometric <- function(n, lambda, theta)
{
  ...
  #change form
  randdata=qlindleygeometric(y, lambda, theta)
  #change form
  return(randdata)
