...
for src in src/mixedind_kinds.f90 src/mixedind_types.f90 src/mixedind_rng.f90 \
  src/mixedind_special.f90 src/mixedind_core.f90 src/mixedindtests.f90; do
...
for src in test/*.f90; do
  ...
  gfortran $FLAGS -J build_gfortran/mod -I build_gfortran/mod build_gfortran/obj/*.o "$src" -o "$exe"
  "$exe"
done
