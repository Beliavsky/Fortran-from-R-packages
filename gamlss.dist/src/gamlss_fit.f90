...
contains
   ...
   subroutine fit_gamlss(y,x_mu,family,result,x_sigma,x_nu,x_tau,weights,start,max_iter,tol)
      ...
      call bfgs_minimize(objective,theta,fval,result%iterations_status,max_iter=max_iter,tol=tolerance)
   ...
   contains
      ...
      real(dp) function objective(par) result(nll)
         ...
         o4=p1+p2+p3
         do ii=1,n
            eta1=dot_product(x_mu(ii,:),par(1:p1))
            eta2=0
            eta3=0
            eta4=0
            if(p2>0)eta2=dot_product(x_sigma(ii,:),par(o2+1:o3))
            if(p3>0)eta3=dot_product(x_nu(ii,:),par(o3+1:o4))
            if(p4>0)eta4=dot_product(x_tau(ii,:),par(o4+1:np))
            call map_parameters(family,eta1,eta2,eta3,eta4,a,b,c,d)
            lp=family_logpdf(family,y(ii),a,b,c,d)
