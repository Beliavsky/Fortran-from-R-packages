...
contains
   ...
   real(dp) function pGIG(q,mu,sigma,nu,lower_tail,log_p) result(v)
      ...
      if (q<=0.0_dp) then
         ...
         raw=adaptive_integral(pdf,0.0_dp,q,2.0e-9_dp,22)
      end if
   ...
   real(dp) function qGIG(p,mu,sigma,nu,lower_tail,log_p) result(v)
      ...
      hi=max(mu,1.0e-6_dp)
      do while (cdf(hi)<pp .and. hi<1.0e12_dp)
         hi=2.0_dp*hi
      end do
      v=bisection_root(cdf,pp,0.0_dp,hi,2.0e-8_dp,100)
   contains
      function cdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=pGIG(x,mu,sigma,nu)
      end function cdf
   end function qGIG
   ...
   real(dp) function qSHASH(p,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      v=bisection_root(cdf,pp,lo,hi,1.0e-9_dp,100)
   contains
      function cdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=pSHASH(x,mu,sigma,nu,tau)
      end function cdf
   end function qSHASH
   ...
   real(dp) function qSIMPLEX(p,mu,sigma,lower_tail,log_p) result(v)
      ...
      if (pp<0.0_dp .or. pp>1.0_dp) then
         ...
         v=bisection_root(cdf,pp,1.0e-10_dp,1.0_dp-1.0e-10_dp,1.0e-9_dp,100)
      end if
   contains
      function cdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=pSIMPLEX(x,mu,sigma)
      end function cdf
   ...
   real(dp) function pSEP(q,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      if (z<=lo) then
         ...
         raw=adaptive_integral(pdf,lo,z,2.0e-8_dp,22)
      end if
      v=finish_prob(raw,lower_tail,log_p)
   contains
      function pdf(x) result(y)
   ...
   real(dp) function qSEP(p,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      v=bisection_root(cdf,pp,lo,hi,2.0e-8_dp,100)
   contains
      function cdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=pSEP(x,mu,sigma,nu,tau)
      end function cdf
   end function qSEP
   ...
   contains
      function pdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=dSEP1(x,0.0_dp,1.0_dp,nu,tau)
      end function pdf
   end function pSEP1
   ...
   real(dp) function pST1(q,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      if (z<=lo) then
         ...
         raw=adaptive_integral(pdf,lo,z,4.0e-8_dp,22)
      end if
      v=finish_prob(raw,lower_tail,log_p)
   contains
      function pdf(x) result(y)
   ...
   real(dp) function qST1(p,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      v=bisection_root(cdf,pp,lo,hi,3.0e-8_dp,100)
   contains
      function cdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=pST1(x,mu,sigma,nu,tau)
      end function cdf
   end function qST1
   ...
   real(dp) function qNET(p,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      v=bisection_root(cdf,pp,lo,hi,1.0e-10_dp,100)
   contains
      function cdf(x) result(y)
         real(dp), intent(in) :: x
         real(dp) :: y
         y=pNET(x,mu,sigma,nu,tau)
      end function cdf
   end function qNET
