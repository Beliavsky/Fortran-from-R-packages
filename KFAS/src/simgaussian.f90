...
! Derived from KFAS 1.6.0 src/simgaussian.f90 by Jouni Helske.
...
tol,sim,c,simwhat,simdim,antithetics)
    ...
    real(dp), dimension(m) :: ahat
    real(dp), dimension(m) :: aplushat
    ...
    real(dp), dimension(p,n) :: epshat
    real(dp), dimension(r,n) :: etaplushat
    real(dp), dimension(p,n) :: epsplushat
    ...
    do t = 1, (n - 1) * tv + 1
        ...
        call dgemm('n','t',m,m,r,1.0_dp,mr,m,rtv(:,:,(t - 1) * timevar(4) + 1),m,0.0_dp,rqr(:,:,t),m)
    end do
    ...
    if(simwhat > 3) then
        ahat = a1
        call dsymv('l',m,1.0_dp,p1,m,rt0,1,1.0_dp,ahat,1)
        if(d > 0) then
            call dsymv('l',m,1.0_dp,p1inf,m,rt1,1,1.0_dp,ahat,1)
        end if
    end if
    ...
    do i = 1, nsim
        ...
        select case(simwhat)
            ...
            case(4)
                ...
                call dsymv('l',m,1.0_dp,p1,m,rt0,1,1.0_dp,aplushat,1)
                if(d > 0) then
                    call dsymv('l',m,1.0_dp,p1inf,m,rt1,1,1.0_dp,aplushat,1)
                end if
