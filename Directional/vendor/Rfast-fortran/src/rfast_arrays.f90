...
contains
   ...
   function gini_r(x) result(g)
      ...
      if(sx==0.0_dp)then; g=0.0_dp; return; end if
      g=0.0_dp
   ...
   function unique_real(x) result(u)
      ...
      do i=2,size(y);if(y(i)/=y(i-1))n=n+1;end do
      allocate(u(n));u(1)=y(1);n=1
      do i=2,size(y);if(y(i)/=y(i-1))then;n=n+1;u(n)=y(i);end if;end do
   ...
   integer function binary_search_real(x,value) result(pos)
      ...
      do while(lo<=hi)
         mid=(lo+hi)/2
         if(x(mid)==value)then;pos=mid;return
   ...
   pure logical function is_element_real(x,value) result(ok)
      real(dp),intent(in)::x(:),value;ok=any(x==value)
