...
set SOURCES=src\jdmbs_kinds.f90 src\jdmbs_status.f90 src\jdmbs_rng.f90 src\jdmbs_model.f90 src\jdmbs.f90
for %%S in (%SOURCES%) do (
  gfortran %FLAGS% -Jbuild-gfortran\mod -Ibuild-gfortran\mod -c %%S -o build-gfortran\obj\%%~nS.o || exit /b 1
)
ar rcs build-gfortran\libjdmbs.a build-gfortran\obj\*.o || exit /b 1
for %%S in (test\*.f90) do (
  gfortran %FLAGS% -Ibuild-gfortran\mod %%S build-gfortran\libjdmbs.a -o build-gfortran\bin\%%~nS.exe || exit /b 1
