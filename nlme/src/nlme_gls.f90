...
contains
  ...
  subroutine initialize_variance(spec,var_group)
    ...
    integer :: k,nlevels
    nlevels=max(1,maxval(var_group)); k=variance_parameter_count(spec,nlevels)
    if (allocated(spec%par)) then
      if (size(spec%par)==k) return
      deallocate(spec%par)
    end if
  ...
  subroutine fit_gls(y,x,result,correlation,variance,time,group,var_covariate,var_group,coordinates,control)
    ...
    if (present(group)) then
      if (size(group)/=n) then; result%status=NLME_DIMENSION_ERROR; return; end if
      allocate(ctx%group(n)); ctx%group=group
    else
    ...
    ctx%corr_template=corr; ctx%var_template=var; ctx%reml=ctl%reml
    nlevels=max(1,maxval(ctx%var_group)); ctx%nlevels=nlevels
    ...
    allocate(par(ctx%kc+ctx%kv)); par=0.0_dp
    if (ctx%kc>0) then
    ...
    if (size(par)>0 .and. ctl%optimize_covariance) then
      ...
      objective=gls_objective(par,ctx); status=NLME_SUCCESS; it=0
    end if
  ...
  subroutine evaluate_gls(ctx,corr,var,nll,status,beta,beta_cov,sigma,covariance,residuals,loglik)
    ...
    if (status/=NLME_SUCCESS) then; nll=huge(1.0_dp); return; end if
    call solve_spd(v,ctx%x,vinvx,status,logdetv)
    if (status/=NLME_SUCCESS) then; nll=huge(1.0_dp); return; end if
    call solve_spd(v,ctx%y,vinvy,status)
    if (status/=NLME_SUCCESS) then; nll=huge(1.0_dp); return; end if
    xtvinvx=matmul(transpose(ctx%x),vinvx); xtvinvy=matmul(transpose(ctx%x),vinvy)
    call solve_spd(xtvinvx,xtvinvy,b,status,logdetx)
    if (status/=NLME_SUCCESS) then; nll=huge(1.0_dp); return; end if
    r=ctx%y-matmul(ctx%x,b)
    call solve_spd(v,r,vinvr,status)
    if (status/=NLME_SUCCESS) then; nll=huge(1.0_dp); return; end if
    rss=dot_product(r,vinvr); df=merge(n-p,n,ctx%reml)
    if (rss<=0.0_dp .or. df<=0) then; status=NLME_INVALID_ARGUMENT; nll=huge(1.0_dp); return; end if
    s2=rss/real(df,dp)
    if (ctx%reml) then
    ...
    if (present(beta_cov)) then
      ...
      if (status/=NLME_SUCCESS) return
      allocate(beta_cov(p,p)); beta_cov=s2*binv
    end if
    if (present(sigma)) sigma=sqrt(s2)
    if (present(covariance)) then; allocate(covariance(n,n)); covariance=s2*v; end if
    if (present(residuals)) then; allocate(residuals(n)); residuals=r; end if
  ...
  subroutine fill_gls_result(par,ctx,result,opt_status,iterations)
    ...
    if (status/=NLME_SUCCESS) then; result%status=status; return; end if
    allocate(result%fitted(size(ctx%y))); result%fitted=ctx%y-result%residuals
    ...
    if (allocated(var%par)) then
      allocate(result%variance_parameters(size(var%par))); result%variance_parameters=var%par
    else
  ...
  subroutine gls_log_likelihood(y,x,covariance,reml,loglik,beta,sigma,status)
    ...
    if (size(x,1)/=n .or. any(shape(covariance)/=[n,n])) then
      allocate(beta(0)); status=NLME_DIMENSION_ERROR; return
    end if
    call solve_spd(covariance,x,vinvx,status,ldv); if(status/=NLME_SUCCESS)return
    call solve_spd(covariance,y,vinvy,status); if(status/=NLME_SUCCESS)return
    xtx=matmul(transpose(x),vinvx); xty=matmul(transpose(x),vinvy)
    call solve_spd(xtx,xty,beta,status,ldx); if(status/=NLME_SUCCESS)return
    r=y-matmul(x,beta); call solve_spd(covariance,r,vr,status); if(status/=NLME_SUCCESS)return
    rss=dot_product(r,vr); df=merge(n-p,n,reml); s2=rss/real(df,dp); sigma=sqrt(s2)
    if(reml)then
