...
contains
  ...
  subroutine group_level_indices(group,levels,index_of,status)
    ...
    if (status/=NLME_SUCCESS) then; allocate(index_of(0)); return; end if
    allocate(index_of(size(group))); index_of=0
    do i=1,size(group)
      do j=1,size(levels)
        if (group(i)==levels(j)) then; index_of(i)=j; exit; end if
      end do
    end do
  ...
  subroutine build_residual_covariance(corr,var,time,group,var_covariate,var_group,covariance,status,coordinates)
    ...
    if (size(group)/=n .or. size(var_covariate)/=n .or. size(var_group)/=n) then
      allocate(covariance(0,0)); status=NLME_DIMENSION_ERROR; return
    end if
    ...
    if (status/=NLME_SUCCESS) then; allocate(covariance(0,0)); return; end if
    allocate(covariance(n,n)); covariance=0.0_dp
    ...
    do g=1,size(levels)
      ...
      allocate(tlocal(size(idx))); tlocal=time(idx)
      if (present(coordinates)) then
        allocate(clocal(size(idx),size(coordinates,2))); clocal=coordinates(idx,:)
