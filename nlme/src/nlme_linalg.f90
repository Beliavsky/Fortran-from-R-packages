...
contains
  ...
  subroutine solve_lower(l, b, x, status)
    ...
    if (size(l,2) /= n .or. size(b) /= n) then
      allocate(x(0)); status = NLME_DIMENSION_ERROR; return
    end if
    allocate(x(n)); x = 0.0_dp
    do i = 1, n
      if (abs(l(i,i)) <= tiny(1.0_dp)) then
        status = NLME_SINGULAR; return
      end if
      s = b(i)
      do j = 1, i - 1
        s = s - l(i,j) * x(j)
  ...
  subroutine solve_spd_matrix(a, b, x, status, logdet)
    ...
    n = size(a,1)
    if (size(b,1) /= n) then
      allocate(x(0,0)); status = NLME_DIMENSION_ERROR; return
    end if
    call cholesky_lower(a, l, status)
    if (status /= NLME_SUCCESS) then
  ...
  subroutine inverse_spd(a, ainv, status, logdet)
    ...
    n = size(a,1)
    if (size(a,2) /= n) then
      allocate(ainv(0,0)); status = NLME_DIMENSION_ERROR; return
    end if
  ...
  subroutine general_inverse(a, ainv, status)
    ...
    do k = 1, n
      ...
      if (abs(aug(imax,k)) <= epsilon(1.0_dp) * max(1.0_dp, maxval(abs(a)))) then
        allocate(ainv(0,0)); status = NLME_SINGULAR; return
      end if
      if (imax /= k) then
    ...
    allocate(ainv(n,n)); ainv = aug(:,n+1:2*n); status = NLME_SUCCESS
  ...
  subroutine mean_columns(x, mu, status)
    ...
    integer, intent(out) :: status
    if (size(x,1) < 1) then
      allocate(mu(0)); status = NLME_DIMENSION_ERROR; return
    end if
  ...
  subroutine covariance_matrix(x, cov, status, center)
    ...
    n = size(x,1)
    if (n < 2) then
      allocate(cov(0,0)); status = NLME_DIMENSION_ERROR; return
    end if
    do_center = .true.; if (present(center)) do_center = center
    allocate(xc(size(x,1),size(x,2)))
    if (do_center) then
      call mean_columns(x, mu, status); if (status /= NLME_SUCCESS) return
    ...
    cov = symmetrize(cov); status = NLME_SUCCESS
  ...
  subroutine unique_integers(values, unique, status)
    ...
    if (size(values) == 0) then
      allocate(unique(0)); status = NLME_SUCCESS; return
    end if
    ...
    allocate(unique(n)); unique = work(:n); status = NLME_SUCCESS
  ...
  subroutine solve_least_squares(x, y, beta, covariance, rss, status, weights)
    ...
    if (size(y) /= n .or. n < p) then
      allocate(beta(0),covariance(0,0)); rss = huge(1.0_dp); status = NLME_DIMENSION_ERROR; return
    end if
