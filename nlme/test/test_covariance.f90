...
program test_covariance
  ...
  c%kind=COR_AR1;allocate(c%par(1));c%par=0.5_dp
  ...
  c%kind=COR_COMPOUND_SYMM;c%par=0.2_dp
  call correlation_matrix(c,t,m,status)
  call check_close(m(1,4),0.2_dp,1.0e-12_dp,'compound symmetry')

  c%kind=COR_EXPONENTIAL;c%par=2.0_dp;c%nugget=.false.
  call correlation_matrix(c,t,m,status)
  ...
  v%kind=VAR_POWER;allocate(v%par(1));v%par=0.5_dp
  ...
  p%kind=PD_LOG_CHOL;p%dim=2;allocate(p%par(3));p%par=[log(2.0_dp),log(1.5_dp),0.4_dp]
