...
program test_lme
  ...
  do g=1,ng
    ...
    do i=1,m
      k=k+1;tt=real(i-1,dp)/real(m-1,dp)
      x(k,:)=[1.0_dp,tt];z(k,1)=1.0_dp;group(k)=g;time(k)=real(i,dp)
      eps=0.08_dp*sin(2.1_dp*real(k,dp))+0.03_dp*cos(0.7_dp*real(k,dp))
      y(k)=2.0_dp+1.25_dp*tt+btrue(g)+eps
    end do
  end do
  random%kind=PD_DIAG;random%dim=1;allocate(random%par(1));random%par=log(0.25_dp)
  ctl%reml=.true.;ctl%max_iter=700;ctl%tolerance=2.0e-7_dp;ctl%step=0.15_dp
