option.call=function(S,K,r,t,sd,price=NA,position,plot=FALSE){
  ...
  if (position=="long"){
    for(i in 1:length(stock)){
      ...
      profit[i]=payoff[i]-callP*exp(r*t)
    }
  }
  if (position=="short"){
    for(i in 1:length(stock)){
  ...
  if(plot==T){
    if(position=="long") {lpos="topleft";m="Long Call\nPayoff and Profit"} else {lpos="bottomleft";m="Short Call\nPayoff and Profit"}
