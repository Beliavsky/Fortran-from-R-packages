bear.call = function(S,K1,K2,r,t,price1,price2,plot=FALSE){
  ...
  NA.Neg=c(r,S,K1,K2,t,price1,price2)
  #Positive
  ...
  for(i in 1:length(stock)){
    ...
    if(stock[i]>=K2) payoff[i]=K1-K2
    if(stock[i]<K2 & stock[i]>K1) payoff[i]=K1-stock[i]
    profit[i]=payoff[i]+(callP1-callP2)*exp(r*t)
  }

  if(plot==T){
    plot(stock,profit,type="l",xlab="Stock Price",main="Bear Call Spread\nPayoff and Profit",ylab="$",
  ...
  out2=matrix(c(callP1,callP2,callP2-callP1),nrow=3)
