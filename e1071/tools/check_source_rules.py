...
if any(path.name != "e1071_kinds.f90" for path, _, _ in real64_occurrences):
    for path, lineno, _ in real64_occurrences:
        if path.name != "e1071_kinds.f90":
            errors.append(f"{path}:{lineno}: real64 may only appear in e1071_kinds.f90")
if len(dp_defs) != 1 or dp_defs[0][0].name != "e1071_kinds.f90":
    errors.append(f"expected exactly one dp definition in e1071_kinds.f90, found {dp_defs}")
