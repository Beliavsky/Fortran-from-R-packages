program test_backend_socp
   ...
   ad = 0.0_dp
   ad(1,2) = 1.0_dp; ad(2,3) = 1.0_dp
   do i = 1, 3
      ad(i+2,i) = -1.0_dp
   end do
   q = [1.0_dp, 0.0_dp, 0.0_dp]
   b = [1.0_dp, 2.0_dp, 0.0_dp, 0.0_dp, 0.0_dp]
   target = [sqrt(5.0_dp), 1.0_dp, 2.0_dp]
   p = csc_empty(3,3)
   a = csc_from_dense(ad)
   ...
   if (.not. sol%solved()) error stop "SOCP status"
   if (maxval(abs(sol%x - target)) > 1.0e-6_dp) error stop "SOCP solution"
   print *, "test_backend_socp: PASS"
end program test_backend_socp
