...
contains
  subroutine acf_values(residuals,max_lag,acf,pairs,status,group)
    ...
    call unique_integers(grp,levels,status)
    if(status/=NLME_SUCCESS)return
    allocate(acf(0:max_lag),pairs(0:max_lag))
    acf=0.0_dp
    pairs=0
    mean_all=sum(residuals)/real(n,dp)
    var_all=sum((residuals-mean_all)**2)
    if(var_all<=0.0_dp)then
    status=NLME_INVALID_ARGUMENT
    return
    end if
    do g=1,size(levels)
  ...
  subroutine empirical_variogram(residuals,time,max_lag,result,status,group)
    ...
    real(dp) :: d,width
    n=size(residuals)
    if(size(time)/=n .or. n<2 .or. max_lag<=0.0_dp)then
    status=NLME_INVALID_ARGUMENT
    return
    end if
    if(present(group))then
    ...
    nbin=max(5,min(30,nint(sqrt(real(n,dp)))))
    width=max_lag/real(nbin,dp)
    allocate(dsum(nbin),vsum(nbin),result%pairs(nbin))
    dsum=0.0_dp
    vsum=0.0_dp
    result%pairs=0
    call unique_integers(grp,levels,status)
  ...
  function pooled_sd(values,group,status) result(sd)
    ...
    real(dp) :: ss,mu
    integer :: g,df
    if(size(values)/=size(group) .or. size(values)<2)then
    sd=0.0_dp
    status=NLME_DIMENSION_ERROR
    return
    end if
  ...
  subroutine simulate_lme(x,z,group,beta,random_covariance,sigma,y,status,seed,correlation,variance,time,&
    ...
    call cholesky_lower(r,lr,status)
    if(status/=NLME_SUCCESS)then
    allocate(y(0))
    return
    end if
    call cholesky_lower(random_covariance,lg,status)
    if(status/=NLME_SUCCESS)then
    allocate(y(0))
    return
    end if
    state=13579
    if(present(seed))state=max(1,mod(abs(seed),2147483646))
    allocate(noise(n),y(n))
    call fill_normals(noise,state)
    ...
    do g=1,size(levels)
      call find_group_indices(group,levels(g),idx)
      allocate(b(q))
      call fill_normals(b,state)
      b=matmul(lg,b)
      y(idx)=y(idx)+matmul(z(idx,:),b)
      deallocate(b)
    end do
    status=NLME_SUCCESS
  ...
  subroutine fill_normals(x,state)
    ...
    i=1
    do while(i<=size(x))
      u1=max(1.0e-12_dp,next_uniform(state))
      u2=next_uniform(state)
  ...
  function next_uniform(state) result(u)
    ...
    real(dp) :: u
    integer(kind=8) :: temp
    temp=mod(16807_8*int(state,8),2147483647_8)
    state=int(temp)
