...
contains
  subroutine initialize_pd(spec,q,y)
    ...
    case(PD_LOG_CHOL)
      ...
      do i=1,q
        k=k+1
        spec%par(k)=log(max(1.0e-4_dp,0.35_dp*sy))
        do j=1,i-1
          k=k+1
          spec%par(k)=0.0_dp
        end do
  ...
  subroutine fit_lme(y,x,z,group,result,random,correlation,variance,time,var_covariate,var_group,coordinates,control,fixed_sigma)
    ...
    pds=pd_spec()
    pds%kind=PD_LOG_CHOL
    if(present(random))pds=random
    ...
    ctx%reml=ctl%reml
    ctx%nlevels=max(1,maxval(ctx%var_group))
    ctx%max_group_size=1
    do it=1,size(ctx%levels)
    ctx%max_group_size=max(ctx%max_group_size,count(ctx%group==ctx%levels(it)))
    end do
    ctx%kp=merge(0,pd_parameter_count(pds),pds%fixed)
    ctx%ks=1
    if(present(fixed_sigma))then
      if(fixed_sigma<=0.0_dp)then
      result%status=NLME_INVALID_ARGUMENT
      return
      end if
      ctx%sigma_fixed=.true.
      ctx%fixed_sigma=fixed_sigma
      ctx%ks=0
    end if
    ...
    allocate(par(ctx%kp+ctx%ks+ctx%kc+ctx%kv))
    pos=0
    if(ctx%kp>0)then
    xp=pds%par
    par(1:ctx%kp)=xp
    ...
    if(ctx%ks>0)then
      ...
      par(pos+1)=log(max(1.0e-4_dp,0.7_dp*sy))
      pos=pos+1
    end if
    if(ctx%kc>0)then
    ...
    if(size(par)>0 .and. ctl%optimize_covariance)then
      ...
      objective=lme_objective(par,ctx)
      status=NLME_SUCCESS
      it=0
    end if
  ...
  subroutine unpack_lme(par,ctx,pds,sigma,corr,var,status)
    ...
    pos=0
    if(ctx%kp>0)then
      if(allocated(pds%par))deallocate(pds%par)
      allocate(pds%par(ctx%kp))
      pds%par=par(1:ctx%kp)
      pos=ctx%kp
    end if
    if(ctx%ks>0)then
    sigma=exp(max(-30.0_dp,min(30.0_dp,par(pos+1))))
    pos=pos+1
    else
    sigma=ctx%fixed_sigma
    end if
    if(ctx%kc>0)then
  ...
  subroutine build_lme_covariance(ctx,gmat,sigma,corr,var,v,status,rbase)
    ...
    do g=1,size(ctx%levels)
      ...
      zi=ctx%z(idx,:)
      block=matmul(matmul(zi,gmat),transpose(zi))
  ...
  subroutine evaluate_lme(ctx,pds,sigma,corr,var,nll,status,beta,beta_cov,gmat_out,v_out,random_effects,&
    ...
    integer :: n,p,gg
    call pd_matrix(pds,gmat,status)
    if(status/=NLME_SUCCESS)then
    nll=huge(1.0_dp)
    return
    end if
    call build_lme_covariance(ctx,gmat,sigma,corr,var,v,status)
    if(status/=NLME_SUCCESS)then
    nll=huge(1.0_dp)
    return
    end if
    call solve_spd(v,ctx%x,vinvx,status,ldv)
    if(status/=NLME_SUCCESS)then
    nll=huge(1.0_dp)
    return
    end if
    ...
    xtx=matmul(transpose(ctx%x),vinvx)
    xty=matmul(transpose(ctx%x),vinvy)
    call solve_spd(xtx,xty,b,status,ldx)
    if(status/=NLME_SUCCESS)then
    nll=huge(1.0_dp)
    return
    end if
    r=ctx%y-matmul(ctx%x,b)
    call solve_spd(v,r,vr,status)
    if(status/=NLME_SUCCESS)then
    nll=huge(1.0_dp)
    return
    end if
    quad=dot_product(r,vr)
    n=size(ctx%y)
    p=size(ctx%x,2)
    if(ctx%reml)then
    ...
    if(present(beta_cov))then
      call inverse_spd(xtx,bcov,status)
      if(status/=NLME_SUCCESS)return
      allocate(beta_cov(size(b),size(b)))
      beta_cov=bcov
    end if
    if(present(gmat_out))then
    allocate(gmat_out(size(gmat,1),size(gmat,2)))
    gmat_out=gmat
    end if
    ...
    if(present(resid_marginal))then
    allocate(resid_marginal(n))
    resid_marginal=r
    end if
    if(present(fitted_marginal))then
    allocate(fitted_marginal(n))
    fitted_marginal=matmul(ctx%x,b)
    end if
    if(present(random_effects) .or. present(fitted_conditional) .or. present(resid_conditional))then
      allocate(bre(size(ctx%levels),size(ctx%z,2)))
      bre=0.0_dp
      allocate(fitc(n))
      fitc=matmul(ctx%x,b)
      do gg=1,size(ctx%levels)
        ...
        vi=v(idx,idx)
        ri=r(idx)
        call solve_spd(vi,ri,vi_r,status)
        if(status/=NLME_SUCCESS)return
        bre(gg,:)=matmul(gmat,matmul(transpose(zi),vi_r))
        fitc(idx)=fitc(idx)+matmul(zi,bre(gg,:))
      end do
      if(present(random_effects))then
      allocate(random_effects(size(bre,1),size(bre,2)))
      random_effects=bre
      end if
      if(present(fitted_conditional))then
      allocate(fitted_conditional(n))
      fitted_conditional=fitc
      end if
      if(present(resid_conditional))then
      allocate(resid_conditional(n))
      resid_conditional=ctx%y-fitc
      end if
    end if
  ...
  subroutine fill_lme_result(par,ctx,result,opt_status,iterations)
    ...
    allocate(result%group_levels(size(ctx%levels)))
    result%group_levels=ctx%levels
    ...
    result%sigma=sigma
    k=size(result%beta)+ctx%kp+ctx%ks+ctx%kc+ctx%kv
