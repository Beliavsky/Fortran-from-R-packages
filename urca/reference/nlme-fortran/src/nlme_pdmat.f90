...
contains
  ...
  subroutine pd_matrix(spec,matrix,status)
    ...
    case(PD_DIAG)
      do i=1,n
      matrix(i,i)=exp(2.0_dp*spec%par(i))
      end do
    case(PD_LOG_CHOL)
      allocate(l(n,n))
      l=0.0_dp
      ...
      do i=1,n
        k=k+1
        l(i,i)=exp(spec%par(k))
        do j=1,i-1
          k=k+1
          l(i,j)=spec%par(k)
        end do
      end do
      matrix=matmul(l,transpose(l))
  ...
  subroutine pd_from_matrix(a,kind,spec,status)
    ...
    case(PD_IDENT)
      allocate(spec%par(1))
      spec%par(1)=0.5_dp*log(max(tiny(1.0_dp),sum([(a(i,i),i=1,n)])/real(n,dp)))
    case(PD_DIAG)
      allocate(spec%par(n))
      do i=1,n
      spec%par(i)=0.5_dp*log(max(tiny(1.0_dp),a(i,i)))
      end do
    case(PD_LOG_CHOL)
      call cholesky_lower(a,l,status)
      if (status/=NLME_SUCCESS) return
      allocate(spec%par(n*(n+1)/2))
      k=0
      do i=1,n
        k=k+1
        spec%par(k)=log(l(i,i))
        do j=1,i-1
          k=k+1
          spec%par(k)=l(i,j)
        end do
