...
contains
  logical function is_balanced(group,covariate) result(balanced)
    ...
    call find_group_indices(group,levels(1),first_idx)
    n0=size(first_idx)
    do g=2,size(levels)
  ...
  subroutine group_summary(values,group,levels,means,sds,counts,status)
    ...
    if(size(values)/=size(group))then
      allocate(levels(0),counts(0),means(0),sds(0))
  ...
  subroutine fit_lm_list(y,x,group,levels,coefficients,sigma,status)
    ...
    if(size(x,1)/=size(y) .or. size(group)/=size(y))then
      allocate(levels(0),coefficients(0,0),sigma(0))
      status=NLME_DIMENSION_ERROR
      return
    end if
    call unique_integers(group,levels,status)
    if(status/=NLME_SUCCESS)return
    p=size(x,2)
    allocate(coefficients(size(levels),p),sigma(size(levels)))
    do g=1,size(levels)
      call find_group_indices(group,levels(g),idx)
      call solve_least_squares(x(idx,:),y(idx),beta,cov,rss,status)
      if(status/=NLME_SUCCESS)return
      coefficients(g,:)=beta
      sigma(g)=sqrt(rss/real(max(1,size(idx)-p),dp))
    end do
  ...
  subroutine fit_nls_list(model,y,xdata,group,theta0,levels,parameters,sigma,status,control)
    ...
    if(size(xdata,1)/=size(y) .or. size(group)/=size(y))then
      allocate(levels(0),parameters(0,0),sigma(0))
      status=NLME_DIMENSION_ERROR
      return
    end if
    call unique_integers(group,levels,status)
    ...
    do g=1,size(levels)
      ...
      if(fit%status/=NLME_SUCCESS)then
      status=fit%status
      return
      end if
      parameters(g,:)=fit%parameters
      sigma(g)=fit%sigma
    end do
    status=NLME_SUCCESS
