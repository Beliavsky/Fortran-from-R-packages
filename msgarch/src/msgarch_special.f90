...
module msgarch_special
   use msgarch_kinds, only : dp, pi, sqrt_two
   implicit none
   private
...
contains
   ...
   function beta_cf(a,b,x) result(cf)
      ...
      real(dp) :: cf, aa, c, d, del, h, qab, qam, qap
      integer :: m, m2
   ...
   elemental function student_t_pdf(x,nu) result(f)
      ...
      real(dp) :: f
      f=exp(log_gamma(0.5_dp*(nu+1.0_dp))-log_gamma(0.5_dp*nu))/sqrt(nu*pi)* &
         (1.0_dp+x*x/nu)**(-0.5_dp*(nu+1.0_dp))
   ...
   function regularized_gamma_p(a,x) result(p)
      ...
      if(a<=0.0_dp)then;p=0.0_dp;return;end if
      if(x<a+1.0_dp)then
   ...
   function gamma_quantile(p,a) result(x)
      ...
      do while(regularized_gamma_p(a,hi)<p);hi=2.0_dp*hi;end do
      do i=1,100
         mid=0.5_dp*(lo+hi)
         if(regularized_gamma_p(a,mid)<p)then;lo=mid;else;hi=mid;end if
      end do
      x=0.5_dp*(lo+hi)
   end function gamma_quantile
end module msgarch_special
