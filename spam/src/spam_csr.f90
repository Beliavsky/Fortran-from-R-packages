...
allocate(a%entries(k),a%colindices(k))
k=0
do i=1,min(n,size(d))
   if(abs(d(i))>tol) then
      k=k+1
...
cnt=0
do k=1,nz
cnt(a%colindices(k))=cnt(a%colindices(k))+1
end do
at%rowpointers(1)=1
do i=1,at%nrow
at%rowpointers(i+1)=at%rowpointers(i)+cnt(i)
end do
next=at%rowpointers(:at%nrow)
do i=1,a%nrow
   do k=a%rowpointers(i),a%rowpointers(i+1)-1
      j=a%colindices(k)
      at%entries(next(j))=a%entries(k)
      at%colindices(next(j))=i
      next(j)=next(j)+1
   end do
end do
...
allocate(vals(cap),cols(cap),c%rowpointers(a%nrow+1))
c%nrow=a%nrow
c%ncol=a%ncol
nz=0
c%rowpointers(1)=1
do i=1,a%nrow
   ka=a%rowpointers(i)
   eaa=a%rowpointers(i+1)-1
   kb=b%rowpointers(i)
   ebb=b%rowpointers(i+1)-1
...
allocate(c%entries(nz),c%colindices(nz))
if(nz>0)then
c%entries=vals(:nz)
c%colindices=cols(:nz)
end if
...
allocate(c%entries(nz),c%colindices(nz),c%rowpointers(c%nrow+1))
k=0
c%rowpointers(1)=1
do i=1,a%nrow
   do nz=a%rowpointers(i),a%rowpointers(i+1)-1
      if(abs(s*a%entries(nz))>tol)then
      k=k+1
      c%entries(k)=s*a%entries(nz)
      c%colindices(k)=a%colindices(nz)
      end if
   end do
...
do i=1,a%nrow
   ...
   do while(ka<a%rowpointers(i+1) .and. kb<b%rowpointers(i+1))
      if(a%colindices(ka)<b%colindices(kb))then
      ka=ka+1
      else if(b%colindices(kb)<a%colindices(ka))then
      kb=kb+1
      else
         v=a%entries(ka)*b%entries(kb)
         if(abs(v)>tol)then
...
tol=0.0_dp
if(present(eps))tol=eps
allocate(mark(b%ncol),work(b%ncol),rowcols(max(1,b%ncol)))
mark=0
work=0.0_dp
cap=max(16,min(max(1,a%nnz()+b%nnz()),max(16,a%nrow*4)))
allocate(vals(cap),cols(cap),c%rowpointers(a%nrow+1))
c%nrow=a%nrow
c%ncol=b%ncol
nz=0
...
do i=1,a%nrow
   ...
   do ka=a%rowpointers(i),a%rowpointers(i+1)-1
      ...
      do kb=b%rowpointers(j),b%rowpointers(j+1)-1
         ...
         rowcols(nr)=col
         mark(col)=i
         work(col)=a%entries(ka)*b%entries(kb)
         else
         work(col)=work(col)+a%entries(ka)*b%entries(kb)
         end if
      end do
   end do
   call sort_int(rowcols,nr)
...
do i=1,a%nrow
   do k=a%rowpointers(i),a%rowpointers(i+1)-1
   y(i)=y(i)+a%entries(k)*x(a%colindices(k))
   end do
end do
...
allocate(y(a%nrow,size(x,2)))
y=0.0_dp
do i=1,a%nrow
 do k=a%rowpointers(i),a%rowpointers(i+1)-1
   do j=1,size(x,2)
   y(i,j)=y(i,j)+a%entries(k)*x(a%colindices(k),j)
   end do
 end do
end do
...
tol=0.0_dp
if(present(eps))tol=eps
cap=max(16,a%nnz()*max(1,b%nnz()))
allocate(vals(cap),cols(cap),c%rowpointers(a%nrow*b%nrow+1))
   ...
   do ka=a%rowpointers(ia),a%rowpointers(ia+1)-1
    do kb=b%rowpointers(ib),b%rowpointers(ib+1)-1
      ...
      if(abs(v)>tol)then
        ...
        call ensure_capacity(vals,cols,nz)
        vals(nz)=v
        col=(a%colindices(ka)-1)*b%ncol+b%colindices(kb)
        cols(nz)=col
      end if
    end do
   end do
...
if(nza>0)then
c%entries(:nza)=a%entries
c%colindices(:nza)=a%colindices
end if
if(nzb>0)then
c%entries(nza+1:)=b%entries
c%colindices(nza+1:)=b%colindices
end if
...
do i=1,c%nrow
 do ka=a%rowpointers(i),a%rowpointers(i+1)-1
 nz=nz+1
 c%entries(nz)=a%entries(ka)
 c%colindices(nz)=a%colindices(ka)
 end do
 do kb=b%rowpointers(i),b%rowpointers(i+1)-1
 nz=nz+1
 c%entries(nz)=b%entries(kb)
 c%colindices(nz)=b%colindices(kb)+a%ncol
 end do
 c%rowpointers(i+1)=nz+1
end do
...
map=0
do j=1,size(cols)
if(cols(j)>=1.and.cols(j)<=a%ncol)map(cols(j))=j
end do
cap=max(16,min(a%nnz(),max(1,size(rows)*4)))
allocate(outv(cap),outc(cap),c%rowpointers(size(rows)+1))
nz=0
c%nrow=size(rows)
c%ncol=size(cols)
c%rowpointers(1)=1
...
integer::k
v=0.0_dp
if(i<1.or.i>a%nrow.or.j<1.or.j>a%ncol)return
do k=a%rowpointers(i),a%rowpointers(i+1)-1
 if(a%colindices(k)==j)then
 v=a%entries(k)
 return
 else if(a%colindices(k)>j)then
 return
 end if
end do
...
allocate(dd(n))
dd=0.0_dp
dd(:min(n,size(d)))=d(:min(n,size(d)))
doadd=.false.
if(present(add))doadd=add
if(doadd)then
 dm=csr_diag(dd,a%nrow,a%ncol,eps)
 c=csr_add(a,dm,eps)
else
 c=a
 ! easiest robust replacement: add (new-old) diagonal
 do i=1,n
 dd(i)=dd(i)-csr_get(a,i,i)
 end do
 dm=csr_diag(dd,a%nrow,a%ncol,eps)
 c=csr_add(a,dm,eps)
end if
...
allocate(s(a%nrow))
do i=1,a%nrow
s(i)=sum(a%entries(a%rowpointers(i):a%rowpointers(i+1)-1))
end do
...
s=0.0_dp
do k=1,a%nnz()
s(a%colindices(k))=s(a%colindices(k))+a%entries(k)
end do
...
integer::i,n
sb=.false.
if(present(structure_based))sb=structure_based
s=csr_row_sums(a)
do i=1,a%nrow
...
if(sb)then
 ...
 do j=1,a%ncol
 if(cnt(j)>0)s(j)=s(j)/real(cnt(j),dp)
 end do
else
...
real(dp),allocatable::s(:)
t='m'
if(present(type))t=type(1:1)
select case(t)
case('o','O','1')
s=csr_col_sums_abs(a)
v=maxval(s)
case('i','I')
s=csr_row_sums_abs(a)
v=maxval(s)
case('f','F','h','H');v=sqrt(sum(a%entries*a%entries))
case default
if(a%nnz()>0)then
v=maxval(abs(a%entries))
else
v=0.0_dp
end if
end select
...
c%nrow=a%nrow
c%ncol=a%ncol
nz=count(abs(a%entries)>eps)
allocate(c%entries(nz),c%colindices(nz),c%rowpointers(c%nrow+1))
nz=0
c%rowpointers(1)=1
do i=1,a%nrow
 do k=a%rowpointers(i),a%rowpointers(i+1)-1
  if(abs(a%entries(k))>eps)then
  nz=nz+1
  c%entries(nz)=a%entries(k)
  c%colindices(nz)=a%colindices(k)
  end if
 end do
 c%rowpointers(i+1)=nz+1
end do
...
do i=1,a%nrow
 do k=a%rowpointers(i),a%rowpointers(i+1)-1
  if(abs(a%entries(k)-csr_get(a,a%colindices(k),i))>t)then
  ok=.false.
  return
  end if
 end do
...
lg=1
if(present(lag))lg=lag
nd=1
if(present(differences))nd=differences
c=a
allocate(cols(a%ncol))
cols=[(i,i=1,a%ncol)]
do i=1,nd
 if(c%nrow<=lg)then
 c=csr_zeros(0,c%ncol)
 exit
 end if
 allocate(rows1(c%nrow-lg),rows2(c%nrow-lg))
 rows1=[(i,i=1,c%nrow-lg)]
 rows2=rows1+lg
 d=csr_subtract(csr_subset(c,rows2,cols),csr_subset(c,rows1,cols),eps)
 call move_csr(d,c)
 deallocate(rows1,rows2)
end do
...
n=size(x)
if(present(y))then
if(size(y)/=n)error stop 'csr_toeplitz: y size'
end if
allocate(d(n,n))
do i=1,n
 do j=1,n
...
type(csr_matrix)::c
if(present(b))then
c=csr_matmul(csr_transpose(a),b,eps)
else
c=csr_matmul(csr_transpose(a),a,eps)
end if
...
type(csr_matrix)::c
if(present(b))then
c=csr_matmul(a,csr_transpose(b),eps)
else
c=csr_matmul(a,csr_transpose(a),eps)
end if
...
br=br+blocks(b)%nrow
bc=bc+blocks(b)%ncol
total=total+blocks(b)%nnz()
end do
c%nrow=br
c%ncol=bc
allocate(c%entries(total),c%colindices(total),c%rowpointers(br+1))
nz=0
br=0
bc=0
c%rowpointers(1)=1
do b=1,size(blocks)
 do i=1,blocks(b)%nrow
  do k=blocks(b)%rowpointers(i),blocks(b)%rowpointers(i+1)-1
   nz=nz+1
   c%entries(nz)=blocks(b)%entries(k)
   c%colindices(nz)=blocks(b)%colindices(k)+bc
  end do
  br=br+1
  c%rowpointers(br+1)=nz+1
 end do
 bc=bc+blocks(b)%ncol
end do
...
allocate(s(a%nrow))
do i=1,a%nrow
s(i)=sum(abs(a%entries(a%rowpointers(i):a%rowpointers(i+1)-1)))
end do
...
s=0.0_dp
do k=1,a%nnz()
s(a%colindices(k))=s(a%colindices(k))+abs(a%entries(k))
end do
...
nc=max(need,max(16,2*size(vals)))
allocate(v2(nc),c2(nc))
if(size(vals)>0)then
v2(:size(vals))=vals
c2(:size(cols))=cols
end if
call move_alloc(v2,vals)
call move_alloc(c2,cols)
