...
n=a%nrow
if(nev<1 .or. nev>=n)error stop 'spam_eigen_symmetric: require 1 <= nev < n'
nc=min(n,max(nev+2,2*nev+1))
if(present(ncv))nc=min(n,max(nev+1,ncv))
mi=300
if(present(maxitr))mi=maxitr
iw=which_sym(which)
allocate(v(n,nc),d(nev))
v=0
d=0
iparam=0
call ds_eigen_f(nev,nc,mi,n,iw,n,a%entries,a%colindices,a%rowpointers,v,d,iparam)
nv=max(0,min(nev,iparam(5)))
allocate(res%values(nv),res%vectors(n,nv))
if(nv>0)then
res%values=d(:nv)
res%vectors=v(:,:nv)
end if
...
n=a%nrow
if(nev<1 .or. nev>=n)error stop 'spam_eigen_general: require 1 <= nev < n'
nc=min(n,max(nev+2,2*nev+1))
if(present(ncv))nc=min(n,max(nev+1,ncv))
mi=300
if(present(maxitr))mi=maxitr
iw=which_gen(which)
allocate(v(n,nc),dr(nev+1),di(nev+1))
v=0
dr=0
di=0
iparam=0
...
integer::j,c
t='  '
t(1:min(2,len_trim(s)))=s(1:min(2,len_trim(s)))
do j=1,2
c=iachar(t(j:j))
if(c>=97.and.c<=122)t(j:j)=achar(c-32)
end do
end function
