subroutine amuxmat (n,m,p, x, y, a,ja,ia)
implicit none
integer n, m, p, ja(*), ia(*)
double precision  x(m,p), y(n,p), a(*)
!-----------------------------------------------------------------------
! Multiplies a sparse matrix by a full matrix using consecutive dot
! products, cf. subroutine amux from sparse kit.
! Matrix A is stored in compressed sparse row storage.
!
! on entry:
!----------
! n     = row dimension of A
! p     = column dimension of x
! x     = array of dimension mxp, m column dimension of A.
! a, ja,
!    ia = input matrix in compressed sparse row format.
!
! on return:
!-----------
! y     = array of dimension nxp, containing the product y=Ax
!
! Reinhard Furrer
!-----------------------------------------------------------------------
! local variables
!
double precision t
integer j, i, k
!-----------------------------------------------------------------------
do j = 1,p
do i = 1,n
!
!     compute the inner product of row i with vector x
!
t = 0.0d0
do  k=ia(i), ia(i+1)-1
t = t + a(k)*x(ja(k),j)
enddo
!
y(i,j) = t
enddo
enddo
!
return
!---------end-of-amuxmat------------------------------------------------
!-----------------------------------------------------------------------
end
!

subroutine notzero (ja,ia,nrow,ncol,nnz,nz,jao,iao)
! Return the structure of the zero entries in ra,ja,ia, in
!  compressed sparse row format via rao, jao, iao.
! INPUT:
!     ja, ia -- sparse structure of the matrix A
!     nrow -- number of rows in `a'
!     ncol -- number of columns in `a'
!     nnz -- number of non-zero elements
!     nz -- number of zero elements
! OUTPUT:
!     jao, iao --  sparse structure of the zero entries
! WORK ARRAY:
!     colmn -- logical vector of length ncol

implicit none
integer  nrow,ncol,nnz,nz,inz, &
& ja(nnz),ia(nrow+1),jao(nz),iao(nrow+1)
logical colmn(ncol)
integer i,j,k
inz = 0
iao(1) = 1
do i = 1,nrow
iao(i+1) = iao(i)
do k = 1,ncol
colmn(k) = .true.
enddo
do j = ia(i),ia(i+1)-1
colmn(ja(j)) = .false.
enddo
do k = 1,ncol
if(colmn(k)) then
inz = inz + 1
jao(inz) = k
iao(i+1) = iao(i+1) + 1
endif
enddo
enddo
return
end


subroutine setdiagmat (nrow, n, a, ja, ia, diag, iw)
implicit none
integer nrow, n
double precision a(*),  diag(n)
integer ja(*), ia(nrow+1), iw(nrow)
!-----------------------------------------------------------------------
! Sets the diagonal entries of a sparse matrix
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A
! n = integer. Smallest dimension of A
!
! a, ja, ia   = Matrix A in compressed sparse row format. Sorted.
! diag = diagonal matrix stored as a vector diag(1:n)
! iw   = n vector of zeros.
!
! on return:
!----------
! updated matrix A
! iw    = iw contains the positions of the diagonal entries in the
!         output matrix. (i.e., a(iw(k)), ja(iw(k)), k=1,...n,
!         are the values/column indices of the diagonal elements
!         of the output matrix. ).
!
! Reinhard Furrer
!-----------------------------------------------------------------
logical insert
integer i,j, k, k1, k2, icount

!
!     get positions of diagonal elements in data structure.
!
do  11 i=1,n
do 21 j= ia(i),ia(i+1)-1
if (ja(j) .ge. i) then
if (ja(j) .eq. i) then
iw(i) = j
endif
goto 11
endif
21 continue
11 continue
!
!     count number of holes in diagonal and add diag(*) elements to
!     valid diagonal entries.
!
icount = 0
do 31 i=1, n
if (iw(i) .eq. 0) then
icount = icount+1
else
a(iw(i)) = diag(i)
endif
31 continue
!
!     if no diagonal elements to insert return
!
if (icount .eq. 0) return
!
!     shift the nonzero elements if needed, to allow for created
!     diagonal elements.
!
!
!     copy rows backward
!
do 5 i=nrow, 1, -1
!
!     go through  row ii
!
k1 = ia(i)
k2 = ia(i+1)-1

ia(i+1) = ia(i+1)+icount

if ((i .gt. n) .or. (iw(i) .gt. 0)) then
!     iw(ii) equal to 0, means no diagonal element in a, we need to insert it
!     test is thus true.

!     no fill-in, only copying
do 4 k = k2,k1,-1
ja(k+icount)=ja(k)
a(k+icount)=a(k)
4 continue
iw(i)=-i
else
insert=.TRUE.
if (k2.lt.k1) then
ja(k2+icount)=i
a(k2+icount)=diag(i)
iw(i)=k2+icount
icount=icount-1
insert = .FALSE.
if (icount .eq. 0) return
else
do 6 k = k2,k1,-1
if (ja(k).gt. i) then
ja(k+icount)=ja(k)
a(k+icount)=a(k)
else  if  (insert) then
ja(k+icount)=i
a(k+icount)=diag(i)
iw(i)=k+icount
icount=icount-1
insert = .FALSE.
if (icount .eq. 0) return
endif
if (ja(k).lt. i) then
ja(k+icount)=ja(k)
a(k+icount)=a(k)
endif
6 continue
!     in case there is only one element, larger than i, we still need to
!     add the diagonal element
if  (insert) then
ja(k+icount)=i
a(k+icount)=diag(i)
iw(i)=k+icount
icount=icount-1
insert = .FALSE.
if (icount .eq. 0) return
endif
endif
endif
5 continue
return
!-----------------------------------------------------------------------
!------------end-of-diagaddmat------------------------------------------
end
subroutine diagaddmat (nrow, n, a, ja, ia, diag, iw)
implicit none
integer nrow, n
double precision a(*),  diag(n)
integer ja(*), ia(nrow+1), iw(nrow)
!-----------------------------------------------------------------------
! Adds a diagonal matrix to a sparse matrix:  A = Diag + A
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A
! n = integer. Smallest dimension of A
!
! a, ja, ia   = Matrix A in compressed sparse row format. Sorted.
! diag = diagonal matrix stored as a vector diag(1:n)
! iw   = n vector of zeros.
!
! on return:
!----------
! updated matrix A
! iw    = iw contains the positions of the diagonal entries in the
!         output matrix. (i.e., a(iw(k)), ja(iw(k)), k=1,...n,
!         are the values/column indices of the diagonal elements
!         of the output matrix. ).
!
! Reinhard Furrer
!-----------------------------------------------------------------
logical insert
integer i,j, k, k1, k2, icount

!
!     get positions of diagonal elements in data structure.
!
do  11 i=1,n
do 21 j= ia(i),ia(i+1)-1
if (ja(j) .ge. i) then
if (ja(j) .eq. i) then
iw(i) = j
endif
goto 11
endif
21 continue
11 continue
!
!     count number of holes in diagonal and add diag(*) elements to
!     valid diagonal entries.
!
icount = 0
do 31 i=1, n
if (iw(i) .eq. 0) then
icount = icount+1
else
a(iw(i)) = a(iw(i)) + diag(i)
endif
31 continue
!
!     if no diagonal elements to insert return
!
if (icount .eq. 0) return
!
!     shift the nonzero elements if needed, to allow for created
!     diagonal elements.
!
!
!     copy rows backward
!
do 5 i=nrow, 1, -1
!
!     go through  row ii
!
k1 = ia(i)
k2 = ia(i+1)-1

ia(i+1) = ia(i+1)+icount

if ((i .gt. n) .or. (iw(i) .gt. 0)) then
!     iw(ii) equal to 0, means no diagonal element in a, we need to insert it
!     test is thus true.

!     no fill-in, only copying
do 4 k = k2,k1,-1
ja(k+icount)=ja(k)
a(k+icount)=a(k)
4 continue
iw(i)=-i
else
insert=.TRUE.
if (k2.lt.k1) then
ja(k2+icount)=i
a(k2+icount)=diag(i)
iw(i)=k2+icount
icount=icount-1
insert = .FALSE.
if (icount .eq. 0) return
else
do 6 k = k2,k1,-1
if (ja(k).gt. i) then
ja(k+icount)=ja(k)
a(k+icount)=a(k)
else  if  (insert) then
ja(k+icount)=i
a(k+icount)=diag(i)
iw(i)=k+icount
icount=icount-1
insert = .FALSE.
if (icount .eq. 0) return
endif
if (ja(k).lt. i) then
ja(k+icount)=ja(k)
a(k+icount)=a(k)
endif
6 continue
!     in case there is only one element, larger than i, we still need to
!     add the diagonal element
if  (insert) then
ja(k+icount)=i
a(k+icount)=diag(i)
iw(i)=k+icount
icount=icount-1
insert = .FALSE.
if (icount .eq. 0) return
endif
endif
endif
5 continue
return
!-----------------------------------------------------------------------
!------------end-of-setdiagmat------------------------------------------
end
!-----------------------------------------------------------------------
subroutine diagmua (nrow, a, ia, diag)
implicit none
integer          nrow, ia(nrow+1)
double precision a(*),  diag(nrow), scal
!-----------------------------------------------------------------------
! performs the matrix by matrix product A = Diag * A  (in place)
! (diamua from sparsekit provides more functionality)
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A
! a, ia   = Matrix A in compressed sparse row format.
!           (ja is not needed)
!
! diag = diagonal matrix stored as a vector diag(1:n)
!
! on return:
!----------
! a, 	= resulting matrix A in compressed sparse row sparse format.
!
! Notes:
!-------
!     Reinhard Furrer 2007-06-21
!
!-----------------------------------------------------------------
!     local variables
integer          ii, k, k1, k2

do 1 ii=1,nrow
!
!     normalize each row
!
k1 = ia(ii)
k2 = ia(ii+1)-1
scal = diag(ii)
do 2 k=k1, k2
a(k) = a(k)*scal
2 continue
1 continue
!
return
!----------end-of-diagmua------------------------------------------------
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------

!-----------------------------------------------------------------------
subroutine getdiag (a,ja,ia,len,diag)

implicit none
double precision diag(*),a(*)
integer len, ia(*), ja(*)
!-----------------------------------------------------------------------
! This subroutine extracts the main diagonal.
! (getdia from sparsekit provides more functionality)
!-----------------------------------------------------------------------
!
! on entry:
!----------
!
! len= min(nrow, ncol) = min dimension of the matrix a.
! a,ja,ia = matrix stored in sorted compressed sparse row a,ja,ia,format
! diag  = array of zeros.
!
! on return:
!-----------
! diag  = array of length containing the wanted diagonal.
!
! Notes:
!-------
!     Reinhard Furrer 2006-11-02
!----------------------------------------------------------------------c
!     local variables
integer i, k
!
!     extract  diagonal elements
!
do 1 i=1, len
do k= ia(i),ia(i+1) -1
if (ja(k) .ge. i) then
!     we are at or beyond the diagonal.
if (ja(k) .eq. i) then
diag(i)= a(k)
endif
goto 1
endif
enddo
1 continue
return
!------------end-of-getdiag----------------------------------------------
!-----------------------------------------------------------------------
end




! Functions that are new or  modified.

subroutine subsparsefull(nrow,a,ja,ia,b)
!
!     subtracts a sparse matrix from a full one
!     algorithm is in-place, i.e. b is changed
!
!
! Notes:
!-------
!     Reinhard Furrer 2006-09-21
!-----------------------------------------------------------------------

implicit none
integer nrow,ja(*),ia(nrow+1)
double precision a(*), b(nrow,*)

integer i,k


do i=1,nrow
do k=ia(i),ia(i+1)-1
b(i,ja(k)) = b(i,ja(k))-a(k)
enddo
enddo
return
end

subroutine subfullsparse(nrow,ncol,a,ja,ia,b)
!
!     subtracts a full matrix from a sparse one
!     algorithm is in-place, i.e. b is changed
!
!
! Notes:
!-------
!     Reinhard Furrer 2006-09-21
!-----------------------------------------------------------------------

implicit none
integer nrow,ncol,ja(*),ia(nrow+1)
double precision a(*), b(nrow,*)

integer i,j,k


do i=1,nrow
do j=1,ncol
b(i,j) = -b(i,j)
enddo
do k=ia(i),ia(i+1)-1
b(i,ja(k)) = b(i,ja(k))+a(k)
enddo
enddo
return
end

subroutine addsparsefull(nrow,a,ja,ia,b)
!
!     adds a sparse matrix to a full one
!     algorithm is in-place, i.e. b is changed
!
!
! Notes:
!-------
!     Reinhard Furrer 2006-09-21
!-----------------------------------------------------------------------

implicit none
integer nrow,ja(*),ia(nrow+1)
double precision a(*), b(nrow,*)

integer i,k


do i=1,nrow
do k=ia(i),ia(i+1)-1
b(i,ja(k)) = b(i,ja(k))+a(k)
enddo
enddo
return
end

subroutine constructia(nrow,nir,ia,ir)
!
!     constructs from a regular row index vector a sparse ia vector.
!     note that a regular column index vector corresponds to the
!     sparse ja vector. for example:
!         A[ir,jc] =>  A@ja = jc, A@ia = constructia(nrow,nir,ia,ir)$ia
!
!     nrow: row dimension of A
!     nir:  length of ir
!     ir:   array of length nir+1!!!
!
! Notes:
!-------
!     _*Row indices have to be ordered!*_
!
!     Reinhard Furrer 2006-09-13
!-----------------------------------------------------------------------

implicit none
integer nrow,nir
integer ia(nrow+1),ir(*)

integer i,k

k=1
ia(1)=1
do i=1,nrow
5 continue
if (ir(k) .eq. i) then
k=k+1
if (k .le. nir) goto 5
endif
ia(i+1)=k
enddo

ia(nrow+1)=nir+1

return
end





subroutine disttospam(nrow,x,a,ja,ia,eps)

implicit none
integer nrow, ia(nrow+1), ja(*)
double precision x(*), a(*), eps
!
!     Convertion of an R dist object (removes zero entries as well).
!
! On entry:
!----------
!     nrow    -- row dimension of the matrix
!     x       -- elements of the dist object (is lower diagonal)
!                        n*(i-1) - i*(i-1)/2 + j-i  for i < j
!
!     a,ja,ia -- input matrix in CSR format
!
! On return:
!-----------
!     a,ja,ia -- cleaned matrix
!
! Notes:
!-------
!     Reinhard Furrer 2008-08-13
!-----------------------------------------------------------------------
!
!     Local
integer i,j,k, tmp


ia(1) = 1
k = 1
do i = 2, nrow
ia(i) = k
do j=1 , i-1
tmp = nrow*(j-1)-j*(j-1)/2+i-j
if (.not.(dabs(x(tmp)) .le. eps)) then
ja(k) = j
a(k) = x(tmp)
k = k + 1
endif

enddo
enddo

ia(nrow+1) = k
return

!---- end of disttospam -------------------------------------------------
!-----------------------------------------------------------------------
end

subroutine setdiaold (nrow,ncol,a,ja,ia,c,jc,ic,cmax,diag,eps)

implicit none

double precision  a(*),c(*),diag(*),eps
integer nrow, ncol, ia(*), ja(*), ic(*), jc(*), cmax
!
!     this routine sets the diagonal entries of a matix, provided they
!     are non-zero.
!
! On entry:
!----------
!     nrow,ncol    --  dimensions of the matrix
!     a,ja,ia -- input matrix in CSR format
!     c,jc,ic -- input matrix in CSR format with enough space, see below
!     diag -- diagonal values to set
!     eps  -- what is smaller than zero?
!
! On return:
!-----------
!     c,jc,ic -- matrix with modified diag in CSR format
!
! Notes:
!-------
!     Reinhard Furrer 2006-10-30
!-----------------------------------------------------------------------
!
!     Local
double precision b(nrow)
integer i,k, len, ib(nrow+1), jb(nrow)

!
len=0
ib(1)=1
do i=1,nrow
jb(i)=0
enddo


do 10 i=1,nrow
do 15 k= ia(i),ia(i+1) -1
if (ja(k) .eq. i) then
a(k)=diag(i)
c(k)=diag(i)
ib(i+1)=ib(i)
goto 10
endif
if (ja(k) .gt. i) then
if (diag(i).gt.eps) then
len=len+1
jb(len)=i
ib(i+1)=ib(i)+1
b(len)=diag(i)
else
ib(i+1)=ib(i)
endif
goto 10
endif
15 continue
10 continue

if (len .eq. 0) return
!
!     set nonexisiting elements.
!

call subass(nrow,ncol,a,ja,ia,b,jb,ib,c,jc,ic,cmax)

return
!------------end of setdia----------------------------------------------
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------


!
!-----------------------------------------------------------------------
subroutine subass(nrow,ncol,a,ja,ia,b,jb,ib,c,jc,ic,nzmax)
implicit none
integer nrow,ncol,nzmax
integer ja(*),jb(*),jc(*),ia(*),ib(*),ic(*)
double precision a(*), b(*), c(*)

!-----------------------------------------------------------------------
! replaces the elements of A with those of B for matrices in sorted CSR
! format. we assume that each row is sorted with increasing column
! indices.
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A and B
! ncol  = integer. The column dimension of A and B.
!
! a,ja,ia,
! b,jb,ib = Matrices A and B in compressed sparse row format with column
!           entries sorted ascendly in each row
!
! nzmax	= integer. The max length of the arrays c and jc.
!
! on return:
!----------
! c,jc,ic = resulting matrix C in compressed sparse row sparse format
!           with entries sorted ascendly in each row.
!
! Notes:
!-------
!     Reinhard Furrer 2006-09-13, based on sparsekit2 subroutine aplb1
!-----------------------------------------------------------------------

!     local variables
integer i,j1,j2,ka,kb,kc,kamax,kbmax

kc = 1
ic(1) = kc
!
!     looping over the rows:
do 6 i=1, nrow
ka = ia(i)
kb = ib(i)
kamax = ia(i+1)-1
kbmax = ib(i+1)-1
5 continue

!     If we have one or more entries then ka <= kamax
!     If we do not have any entries in both A and B
!     we will not enter the if clause. In which case
!     we repeatedly copy ic(i+1) <- ic(i).
if (ka .le. kamax .or. kb .le. kbmax) then

!     j1 and j2 are left hand pointers of the first entry
!     of A and B. If no entry, they are set to ncol+1
if (ka .le. kamax) then
j1 = ja(ka)
else
j1 = ncol+1
endif
if (kb .le. kbmax) then
j2 = jb(kb)
else
j2 = ncol+1
endif
!
!     Three cases:
!       j1=j2: copy element of b in c, incr. all three pointers
!       j1<j2: copy element of a in c, incr. a and c pointers
!       j1>j2: copy element of b in c, incr. b and c pointers
if (j1 .eq. j2) then
c(kc) = b(kb)
jc(kc) = j1
ka = ka+1
kb = kb+1
kc = kc+1
else if (j1 .lt. j2) then
jc(kc) = j1
c(kc) = a(ka)
ka = ka+1
kc = kc+1
else if (j1 .gt. j2) then
jc(kc) = j2
c(kc) = b(kb)
kb = kb+1
kc = kc+1
endif

!     the next four lines should not be required...
if (kc .gt. nzmax+1) then
!               write (*,*) "exceeding array capacities...",i,nzmax,
!     & ka,kb,kc,j1,j2,kamax,kbmax,ncol,jb(kb)
return
endif
goto 5
endif

ic(i+1) = kc
6 continue
return
!------------end-of-subass----------------------------------------------
!-----------------------------------------------------------------------
end

subroutine spamcsrdns(nrow,a,ja,ia,dns)

implicit none
integer i,k
integer nrow,ja(*),ia(*)
double precision dns(nrow,*),a(*)

!-----------------------------------------------------------------------
! Compressed Sparse Row    to    Dense
!-----------------------------------------------------------------------
!
! converts a row-stored sparse matrix into a densely stored one
!
! On entry:
!----------
!
! nrow  = row-dimension of a
! a,
! ja,
! ia    = input matrix in compressed sparse row format.
!         (a=value array, ja=column array, ia=pointer array)
! dns   = array where to store dense matrix
!
! on return:
!-----------
! dns   = the sparse matrix a, ja, ia has been stored in dns(nrow,*)
!
! changes:
!---------
! eliminated the ierr
! eliminated the filling of zeros: all done with
!-----------------------------------------------------------------------
do i=1,nrow
do  k=ia(i),ia(i+1)-1
dns(i,ja(k)) = a(k)
enddo
enddo
return
!---- end of csrdns ----------------------------------------------------
!-----------------------------------------------------------------------
end



!-----------------------------------------------------------------------
subroutine spamdnscsr(nrow,ncol,dns,ndns,a,ja,ia,eps)

implicit none
integer i,j,next
integer nrow,ncol,ndns,ia(*),ja(*)
double precision dns(ndns,*),a(*),eps
!-----------------------------------------------------------------------
! Converts a densely stored matrix into a CSR  sparse matrix.
!-----------------------------------------------------------------------
! on entry:
!---------
!
! nrow  = row-dimension of a
! ncol  = column dimension of a
! nzmax = maximum number of nonzero elements allowed. This
!         should be set to be the lengths of the arrays a and ja.
! dns   = input nrow x ncol (dense) matrix.
! ndns  = first dimension of dns.
!
! on return:
!----------
!
! a, ja, ia = value, column, pointer  arrays for output matrix
!
! changes:
!---------
! eliminated the ierr
! introduced epsilon
!-----------------------------------------------------------------------
next = 1
ia(1) = 1
do  i=1,nrow
do  j=1, ncol
if (.not.(dabs(dns(i,j)) .le. eps)) then
ja(next) = j
a(next) = dns(i,j)
next = next+1
endif
enddo
ia(i+1) = next
enddo
return
!---- end of dnscsr ----------------------------------------------------
!-----------------------------------------------------------------------
end

!-----------------------------------------------------------------------
subroutine getmask(nrow,nnz,ir,jc,jao,iao)
!-----------------------------------------------------------------------
implicit none
integer nrow,nnz,ir(*),jc(*),jao(*),iao(*)
integer k,k0,j,i,iad
!-----------------------------------------------------------------------
!  Gets Compressed Sparse Row indices from Coordinate ones
!-----------------------------------------------------------------------
!  Loosely based on coocsr from Sparsekit.
!
! on entry:
!---------
! nrow	= dimension of the matrix
! nnz	= number of nonzero elements in matrix
! ir,
! jc    = matrix in coordinate format. ir(k), jc(k) store the nnz
!         nonzero index. The order of the elements is arbitrary.
! iao   = vector of 0 of size nrow+1
!
! on return:
!-----------
! ir 	is destroyed
!
! jao, iao = matrix index in general sparse matrix format with
!       jao containing the column indices,
!	and iao being the pointer to the beginning of the row
!
!------------------------------------------------------------------------

! determine row-lengths.
do 2 k=1, nnz
iao(ir(k)) = iao(ir(k))+1
2 continue
! starting position of each row..
k = 1
do 3 j=1,nrow+1
k0 = iao(j)
iao(j) = k
k = k+k0
3 continue
! go through the structure  once more. Fill in output matrix.
do 4 k=1, nnz
i = ir(k)
j = jc(k)
iad = iao(i)
jao(iad) = j
iao(i) = iad+1
4 continue
! shift back iao
do 5 j=nrow,1,-1
iao(j+1) = iao(j)
5 continue
iao(1) = 1
return
!-----------------------------------------------------------------------
end


!-----------------------------------------------------------------------
subroutine getblock(a,ja,ia, nrw, rw, ncl, cl, bnz, b,jb,ib)
!-----------------------------------------------------------------------
!     purpose:
!     --------
!     this function returns the elements a(rw,cl) of a matrix a,
!     for any index vector rw and cl. the matrix is assumed to be stored
!     in compressed sparse row (csr) format.
!
!
!     Reinhard Furrer 2006-09-12
!-----------------------------------------------------------------------
!     parameters:
!     -----------
! on entry:
!----------
!     a,ja,ia = the matrix a in compressed sparse row format (input).
!     nrw,rw
!     ncl,cl  = length of and the vector containing the rows and columns
!               to extract
!
! on return:
!-----------
!     bnz     = nonzero elements of b
!     b,jb,ib = the matrix a(rw,cl) in compressed sparse row format.
!
! note:
!------
!     no error testing is done. It is assumed that b has enough space
!     allocated.
!-----------------------------------------------------------------------
implicit none

integer nrw,rw(*),  ncl, cl(*)
integer bnz, ia(*),ja(*), ib(*),jb(*)
double precision a(*),b(*)
!
!     local variables.
!
integer irw, jcl, jja
!
!      write(*,*) cl(1),cl(2)
bnz = 1
ib(1) = 1
do irw = 1,nrw
do jcl = 1,ncl

do jja = ia(rw(irw)),ia(rw(irw)+1)-1

if (cl(jcl) .eq. ja(jja)) then
!     we've found one...
b(bnz)  = a(jja)
jb(bnz) = jcl
bnz = bnz + 1
endif
enddo
enddo
ib(irw+1) = bnz
!     end irw, we've cycled over all lines
enddo
bnz = bnz - 1
!      write(*,*) cl(1),cl(2)

return
!--------end-of-getblock------------------------------------------------
!-----------------------------------------------------------------------
end


!-----------------------------------------------------------------------
subroutine getlines(a,ja,ia, nrw, rw, bnz, b,jb,ib)
!-----------------------------------------------------------------------
!     purpose:
!     --------
!     this function returns the lines rw of a matrix a.
!     the matrix is assumed to be stored
!     in compressed sparse row (csr) format.
!
!
!     Reinhard Furrer 2012-04-04
!-----------------------------------------------------------------------
!     parameters:
!     -----------
! on entry:
!----------
!     a,ja,ia = the matrix a in compressed sparse row format (input).
!     nrw,rw  = length of and the vector containing the rows and columns
!               to extract
!
! on return:
!-----------
!     bnz     = nonzero elements of b
!     b,jb,ib = the matrix a(rw,cl) in compressed sparse row format.
!
! note:
!------
!     no error testing is done. It is assumed that b has enough space
!     allocated.
!-----------------------------------------------------------------------
implicit none

integer nrw,rw(*)
integer bnz, ia(*),ja(*), ib(*),jb(*)
double precision a(*),b(*)
!
!     local variables.
!
integer irw, jja
!
bnz = 1
ib(1) = 1
do irw = 1,nrw
do jja = ia(rw(irw)),ia(rw(irw)+1)-1

b(bnz)  = a(jja)
jb(bnz) = ja(jja)
bnz = bnz + 1
enddo
ib(irw+1) = bnz
!     end irw, we've cycled over all lines
enddo
bnz = bnz - 1

return
!--------end-of-getlines------------------------------------------------
!-----------------------------------------------------------------------
end



!-----------------------------------------------------------------------
subroutine getelem(i,j,a,ja,ia,iadd,elem)
!-----------------------------------------------------------------------
!     purpose:
!     --------
!     this function returns the element a(i,j) of a matrix a,
!     for any pair (i,j).  the matrix is assumed to be stored
!     in compressed sparse row (csr) format. getelem performs a
!     binary search.
!     also returns (in iadd) the address of the element a(i,j) in
!     arrays a and ja when the search is successsful (zero if not).
!-----------------------------------------------------------------------
!     parameters:
!     -----------
! on entry:
!----------
!     i      = the row index of the element sought (input).
!     j      = the column index of the element sought (input).
!     a      = the matrix a in compressed sparse row format (input).
!     ja     = the array of column indices (input).
!     ia     = the array of pointers to the rows' data (input).
! on return:
!-----------
!     elem = value of a(i,j).
!     iadd   = address of element a(i,j) in arrays a, ja if found,
!              zero if not found. (output)
!
!     note: the inputs i and j are not checked for validity.
!-----------------------------------------------------------------------
!     noel m. nachtigal october 28, 1990 -- youcef saad jan 20, 1991.
!
!     Reinhard Furrer: converted to subroutine and eliminated sorted
!         many manipulations... last for 0.31; Sept 13
!-----------------------------------------------------------------------
implicit none

integer i, ia(*), iadd, j, ja(*)
double precision a(*),elem
!
!     local variables.
!
integer ibeg, iend, imid
!
!     initialization
!
iadd = 0
ibeg = ia(i)
iend = ia(i+1)-1

!     empty line! test at beginning
10 if (iend .lt. ibeg) return

!
!     begin binary search:
!     test of bounds
if (ja(ibeg).gt.j) return
if (ja(iend).lt.j) return

if (ja(ibeg).eq.j) then
iadd = ibeg
goto 20
endif
if (ja(iend).eq.j) then
iadd = iend
goto 20
endif

!     compute the middle index and test if found
imid = ( ibeg + iend ) / 2
if (ja(imid).eq.j) then
iadd = imid
goto 20
endif

!     update the interval bounds.
if (ja(imid).gt.j) then
iend = imid -1
else
ibeg = imid +1
endif

goto 10
!
!     set iadd and elem before returning
20 elem = a(iadd)
return
!--------end-of-getelem-------------------------------------------------
!-----------------------------------------------------------------------
end

subroutine getallelem(nir,ir,jr,a,ja,ia,alliadd,allelem)
!-----------------------------------------------------------------------
!     purpose:
!     --------
!     wrapper to getelem to retrieve several elements.
!-----------------------------------------------------------------------
!     Reinhard Furrer 2006-09-12
!-----------------------------------------------------------------------
implicit none

integer nir,ir(nir),jr(nir),ja(*),ia(*),alliadd(nir)
double precision a(*),allelem(nir)
!     local vars
integer i
do i = 1,nir
call getelem(ir(i),jr(i),a,ja,ia,alliadd(i),allelem(i))
enddo
return
!--------end-of-allgetelem----------------------------------------------
!-----------------------------------------------------------------------
end


!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!-
!- Modified by P. T. Ng from sparsekit
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!      subroutine aemub (nrow,ncol,a,ja,ia,amask,jmask,imask,
!     *                  c,jc,ic,iw,aw,nzmax,ierr)
subroutine aemub (nrow,ncol,a,ja,ia,amask,jmask,imask, &
& c,jc,ic,nzmax,ierr)
!---------------------------------------------------------------------
implicit none
integer nrow, ncol
real(8) a(*),c(*),amask(*),aw(ncol)
integer ia(nrow+1),ja(*),jc(*),ic(nrow+1),jmask(*)
integer imask(nrow+1), nzmax, ierr
logical iw(ncol)
!-----------------------------------------------------------------------
! further used variables
integer len, k, k1, k2, ii, j
!-----------------------------------------------------------------------
! Modified from amask by Pin T. Ng on 2/27/03 to perform
! element-wise multiplication
!-----------------------------------------------------------------------
! On entry:
!---------
! nrow  = integer. row dimension of input matrix
! ncol  = integer. Column dimension of input matrix.
!
! a,
! ja,
! ia    = the A matrix in Compressed Sparse Row format
!
! amask,
! jmask,
! imask = matrix defining mask stored in compressed
!         sparse row format. (This is the B matrix)
!
! nzmax = length of arrays c and jc. see ierr.
!
! On return:
!-----------
!
! a, ja, ia and amask, jmask, imask are unchanged.
!
! c
! jc,
! ic    = the output matrix in Compressed Sparse Row format.
!
! ierr  = integer. serving as error message.c
!         ierr = 1  means normal return
!         ierr .gt. 1 means that amask stopped when processing
!         row number ierr, because there was not enough space in
!         c, jc according to the value of nzmax.
!
! work arrays:
!-------------
! iw    = logical work array of length ncol.
! aw    = real work array of length ncol.
!
! note:
!------ the  algorithm is in place: c, jc, ic can be the same as
! a, ja, ia in which cas the code will overwrite the matrix c
! on a, ja, ia
!
!-----------------------------------------------------------------------
ierr = 0
len = 0
do 1 j=1, ncol
iw(j) = .false.
aw(j) = 0.0
1 continue
!     unpack the mask for row ii in iw
do 100 ii=1, nrow
!     save pointer and value in order to be able to do things in place
do 2 k=imask(ii), imask(ii+1)-1
iw(jmask(k)) = .true.
aw(jmask(k)) = amask(k)
2 continue
!     add umasked elemnts of row ii
k1 = ia(ii)
k2 = ia(ii+1)-1
ic(ii) = len+1
do 200 k=k1,k2
j = ja(k)
if (iw(j)) then
len = len+1
if (len .gt. nzmax) then
ierr = ii
return
endif
jc(len) = j
c(len) = a(k)*aw(j)
endif
200 continue
!
do 3 k=imask(ii), imask(ii+1)-1
iw(jmask(k)) = .false.
aw(jmask(k)) = 0.0
3 continue
100 continue
ic(nrow+1)=len+1
!
return
!-----end-of-aemub -----------------------------------------------------
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------
subroutine aemub1 (nrow,ncol,a,ja,ia,b,jb,ib,c,jc,ic, &
& nzmax,ierr)
implicit none
integer nrow, ncol, nzmax, ierr
real(8) a(*), b(*), c(*)
integer ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1),ic(nrow+1)
!-----------------------------------------------------------------------
! A modification of aplsb by Pin Ng on 6/12/02 to
! perform the element-wise operation C = A*B for matrices in
! sorted CSR format.
! the difference with aplsb is that the resulting matrix is such that
! the elements of each row are sorted with increasing column indices in
! each row, provided the original matrices are sorted in the same way.
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A and B
! ncol  = integer. The column dimension of A and B.
!
! a,
! ja,
! ia   = Matrix A in compressed sparse row format with entries sorted
!
! b,
! jb,
! ib	=  Matrix B in compressed sparse row format with entries sorted
!        ascendly in each row
!
! nzmax	= integer. The  length of the arrays c and jc.
!         amub will stop if the result matrix C  has a number
!         of elements that exceeds exceeds nzmax. See ierr.
!
! on return:
!----------
! c,
! jc,
! ic	= resulting matrix C in compressed sparse row sparse format
!         with entries sorted ascendly in each row.
!
! ierr	= integer. serving as error message.
!         ierr = 0 means normal return,
!         ierr .gt. 0 means that amub stopped while computing the
!         i-th row  of C with i=ierr, because the number
!         of elements in C exceeds nzmax.
!
! Notes:
!-------
!     this will not work if any of the two input matrices is not sorted
!-----------------------------------------------------------------------
integer i, ka, kb, kc, kamax, kbmax, j1, j2
ierr = 0
kc = 1
ic(1) = kc
!
!     the following loop does a merge of two sparse rows and
!     multiplies  them.
!
do 6 i=1, nrow
ka = ia(i)
kb = ib(i)
kamax = ia(i+1)-1
kbmax = ib(i+1)-1
5 continue
!
!     this is a while  -- do loop --
!
if (ka .le. kamax .or. kb .le. kbmax) then
!
if (ka .le. kamax) then
j1 = ja(ka)
else
!     take j1 large enough  that always j2 .lt. j1
j1 = ncol+1
endif
if (kb .le. kbmax) then
j2 = jb(kb)
else
!     similarly take j2 large enough  that always j1 .lt. j2
j2 = ncol+1
endif
!
!     three cases
!
if (j1 .eq. j2) then
c(kc) = a(ka)*b(kb)
jc(kc) = j1
ka = ka+1
kb = kb+1
kc = kc+1
else if (j1 .lt. j2) then
ka = ka+1
else if (j1 .gt. j2) then
kb = kb+1
endif
if (kc .gt. nzmax) goto 999
goto 5
!
!     end while loop
!
endif
ic(i+1) = kc
6 continue
return
999 ierr = i
return
!------------end-of-aemub1 ---------------------------------------------
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------
subroutine aedib (nrow,ncol,job,a,ja,ia,b,jb,ib, &
& c,jc,ic,nzmax,iw,aw,ierr)
implicit none
integer nrow, ncol, job, nzmax, ierr
real(8) a(*), b(*), c(*), aw(ncol)
integer ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1),ic(nrow+1), &
& iw(ncol)
!-----------------------------------------------------------------------
! performs the element-wise matrix division  C = A/B.
! Modified from aplsb by Pin Ng on 2/27/03
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A and B
! ncol  = integer. The column dimension of A and B.
! job   = integer. Job indicator. When job = 0, only the structure
!                  (i.e. the arrays jc, ic) is computed and the
!                  real values are ignored.
!
! a,
! ja,
! ia   = Matrix A in compressed sparse row format.
!
! b,
! jb,
! ib	=  Matrix B in compressed sparse row format.
!
! nzmax	= integer. The  length of the arrays c and jc.
!         amub will stop if the result matrix C  has a number
!         of elements that exceeds exceeds nzmax. See ierr.
!
! on return:
!----------
! c,
! jc,
! ic	= resulting matrix C in compressed sparse row sparse format.
!
! ierr	= integer. serving as error message.
!         ierr = 0 means normal return,
!         ierr .gt. 0 means that amub stopped while computing the
!         i-th row  of C with i=ierr, because the number
!         of elements in C exceeds nzmax.
!
! work arrays:
!------------
! iw	= integer work array of length equal to the number of
!         columns in A.
! aw	= real work array of length equal to the number of
!         columns in A.
!
!-----------------------------------------------------------------------
logical values
integer len, j, ii, ka, kb, k, jpos, jcol
values = (job .ne. 0)
ierr = 0
len = 0
ic(1) = 1
do 1 j=1, ncol
iw(j) = 0
1 continue
!
do 500 ii=1, nrow
!     row i
do 200 ka=ia(ii), ia(ii+1)-1
len = len+1
jcol    = ja(ka)
if (len .gt. nzmax) goto 999
jc(len) = jcol
if (values) c(len)  = a(ka)/0.0
iw(jcol)= len
aw(jcol) = a(ka)
200 continue
!
do 300 kb=ib(ii),ib(ii+1)-1
jcol = jb(kb)
jpos = iw(jcol)
if (jpos .eq. 0) then
len = len+1
if (len .gt. nzmax) goto 999
jc(len) = jcol
if (values) c(len)  = 0.0
iw(jcol)= len
else
if (values) c(jpos) = aw(jcol)/b(kb)
endif
300 continue
do 301 k=ic(ii), len
iw(jc(k)) = 0
301 continue
ic(ii+1) = len+1
500 continue
return
999 ierr = ii
return
!------------end of aedib -----------------------------------------------
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------
subroutine aeexpb (nrow,ncol,job,a,ja,ia,b,jb,ib, &
& c,jc,ic,nzmax,iw,aw,ierr)
implicit none
integer nrow, ncol, job, nzmax, ierr
real(8) a(*), b(*), c(*), aw(ncol)
integer ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1),ic(nrow+1), &
& iw(ncol)
!-----------------------------------------------------------------------
! performs the element-wise matrix division  C = A/B.
! Modified from aplsb by Pin Ng on 2/27/03
!-----------------------------------------------------------------------
! on entry:
! ---------
! nrow	= integer. The row dimension of A and B
! ncol  = integer. The column dimension of A and B.
! job   = integer. Job indicator. When job = 0, only the structure
!                  (i.e. the arrays jc, ic) is computed and the
!                  real values are ignored.
!
! a,
! ja,
! ia   = Matrix A in compressed sparse row format.
!
! b,
! jb,
! ib	=  Matrix B in compressed sparse row format.
!
! nzmax	= integer. The  length of the arrays c and jc.
!         amub will stop if the result matrix C  has a number
!         of elements that exceeds exceeds nzmax. See ierr.
!
! on return:
!----------
! c,
! jc,
! ic	= resulting matrix C in compressed sparse row sparse format.
!
! ierr	= integer. serving as error message.
!         ierr = 0 means normal return,
!         ierr .gt. 0 means that amub stopped while computing the
!         i-th row  of C with i=ierr, because the number
!         of elements in C exceeds nzmax.
!
! work arrays:
!------------
! iw	= integer work array of length equal to the number of
!         columns in A.
! aw	= real work array of length equal to the number of
!         columns in A.
!
!-----------------------------------------------------------------------
logical values
integer len, kb, ka, k, jpos, jcol, j, ii
values = (job .ne. 0)
ierr = 0
len = 0
ic(1) = 1
do 1 j=1, ncol
iw(j) = 0
1 continue
!
do 500 ii=1, nrow
!     row i
do 200 ka=ia(ii), ia(ii+1)-1
len = len+1
jcol    = ja(ka)
if (len .gt. nzmax) goto 999
jc(len) = jcol
if (values) c(len)  = 1.0
iw(jcol)= len
aw(jcol) = a(ka)
200 continue
!
do 300 kb=ib(ii),ib(ii+1)-1
jcol = jb(kb)
jpos = iw(jcol)
if (jpos .eq. 0) then
len = len+1
if (len .gt. nzmax) goto 999
jc(len) = jcol
if (values) c(len)  = 0.0**b(kb)
iw(jcol)= len
else
if (values) c(jpos) = aw(jcol)**b(kb)
endif
300 continue
do 301 k=ic(ii), len
iw(jc(k)) = 0
301 continue
ic(ii+1) = len+1
500 continue
return
999 ierr = ii
return
!------------end of aeexpb -----------------------------------------------
!-----------------------------------------------------------------------
end



SUBROUTINE CALCJA(nrow,nsuper, &
& xsuper,lindx,xlindx,xlnz, &
& cholcja)
!     small function to calculate ja for the cholesky factor
!     as they use a condensed format. GRATULIERU LIT!


!     INPUT:
!     nrow (integer)           number of rows
!     nsuper (integer)         number of supernodes
!     xsuper (integer)         supernode partition
!     xlindx,lindx  (integer)  row indices for each supernode
!     xlnz (integer)           ia for cholesky factor
!
!     OUTPUT:
!     cholcja (integer)        ja for cholesky factor

IMPLICIT NONE

INTEGER nrow,nsuper
INTEGER xsuper(nrow),lindx(*),xlindx(nrow+1),xlnz(nrow+1)
INTEGER cholcja(*)

INTEGER k, i, j, m, n

k=1
m=1
DO i=1,nsuper
DO j=1,( xsuper(i+1)-xsuper(i))
DO n=1,(xlnz(k+1)-xlnz(k))
cholcja(m)=lindx( xlindx(i)+j-2  + n)
m=m+1
ENDDO
k=k+1
ENDDO
ENDDO

RETURN
END






subroutine transpose(n,m,a,ja,ia,ao,jao,iao)

implicit none
integer n,m,ia(n+1),iao(m+1),ja(*),jao(*)
double precision  a(*),ao(*)


integer i,j,k,next
!-----------------------------------------------------------------------
!     Transposition
!     similar to csrcsc from sparsekit
!-----------------------------------------------------------------------
! on entry:
!----------
! n	= number of rows of CSR matrix.
! m    = number of columns of CSC matrix.
! a	= real array of length nnz (nnz=number of nonzero elements in input
!         matrix) containing the nonzero elements.
! ja	= integer array of length nnz containing the column positions
! 	  of the corresponding elements in a.
! ia	= integer of size n+1. ia(k) contains the position in a, ja of
!	  the beginning of the k-th row.
!
! on return:
! ----------
! ao	= real array of size nzz containing the "a" part of the transpose
! jao	= integer array of size nnz containing the column indices.
! iao	= integer array of size n+1 containing the "ia" index array of
!	  the transpose.
!
!-----------------------------------------------------------------------
!----------------- compute lengths of rows of transp(A) ----------------
do  i=1, n
do  k=ia(i), ia(i+1)-1
j = ja(k)+1
iao(j) = iao(j)+1
enddo
enddo
!---------- compute pointers from lengths ------------------------------
iao(1) = 1
do  i=1,m
iao(i+1) = iao(i) + iao(i+1)
enddo
!--------------- now do the actual copying -----------------------------
do  i=1,n
do  k=ia(i),ia(i+1)-1
j = ja(k)
next = iao(j)
ao(next) = a(k)
jao(next) = i
iao(j) = next+1
enddo
enddo
!-------------------------- reshift iao and leave ----------------------
do  i=m,1,-1
iao(i+1) = iao(i)
enddo
iao(1) = 1
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------

!-----------------------------------------------------------------------



subroutine reducedim(a,ja,ia,eps,bnrow,bncol,k,b,jb,ib)

implicit none
double precision a(*),b(*),eps
integer bnrow, bncol,k
integer ia(*),ja(*),ib(*),jb(*)

integer   i, j, jaj

!-----------------------------------------------------------------------
!  Reduces the dimension of A to (,bnrow,bncol) by copying it to B.
!  (Hence not in place - for R purposes).
!  Only elements smaller than eps are copied.
!-----------------------------------------------------------------------
! on entry:
!---------
!
!------------------------------------------------------------------------

k=1
do i=1,bnrow
ib(i)=k
do j=ia(i), ia(i+1)-1
jaj=ja(j)
if (jaj .le.bncol) then
if (abs( a(j)) .gt. eps) then
b(k)=a(j)
jb(k)=jaj
k=k+1
endif
endif
enddo
enddo
ib(bnrow+1)=k
return
!-----------------------------------------------------------------------
end

!-----------------------------------------------------------------------
! Currently not used...
subroutine reducediminplace(eps,nrow,ncol,k,a,ja,ia)

implicit none
double precision a(*),eps
integer nrow, ncol,k
integer ia(*),ja(*)

integer   i, j, jj, itmp

!-----------------------------------------------------------------------
!  Reduces the dimension of A to (nrow,ncol) _in place_
!  Only elements smaller than eps are copied.
!-----------------------------------------------------------------------
!  Reinhard Furrer, June 2008
!------------------------------------------------------------------------

k=1
do i=1,nrow
itmp = ia(i)
ia(i)=k
do j=itmp, ia(i+1)-1
jj=ja(j)
if (jj .le. ncol) then
if (abs( a(jj)) .gt. eps) then
a(k)=a(jj)
ja(k)=jj
k=k+1
endif
endif
enddo
enddo
ia(nrow+1)=k
return
!-----------------------------------------------------------------------
end
!-----------------------------------------------------------------------



!----------------------------------------------------------------------c
!      T R I A N G U L A R    S Y S T E M    S O L U T I O N S         c
!                                                                      c
!      spamforward and spamback                                        c
!----------------------------------------------------------------------c

subroutine spamforward (n,p,x,b,l,jl,il)
implicit none

integer n, p, jl(*),il(n+1)
double precision  x(n,p), b(n,p), l(*)

integer i, j, k
double precision  t

!-----------------------------------------------------------------------
!   solves    L x = y ; L = lower triang. /  CSR format
!                        sequential forward elimination
!-----------------------------------------------------------------------
!
! On entry:
!----------
! n,p      = integer. dimensions of problem.
! b      = real array containg the right side.
!
! l, jl, il,    = Lower triangular matrix stored in CSR format.
!
! On return:
!-----------
!       x  = The solution of  L x  = b.
!--------------------------------------------------------------------
!     Reinhard Furrer June 2008, April 2012, Sept 2016, Nov 2019


!     initialize k for next setting
k=0
!     if first diagonal element is zero, break
if (abs(l(1)) .le. 0.D0) goto 5

!     cycle over all columns of b
do i=1,p

!     first row has one element then cycle over all rows
x(1,i) = b(1,i) / l(1)
do 3 k = 2, n
t = b(k,i)
do 1 j = il(k), il(k+1)-1
if (jl(j) .lt. k) then
t = t-l(j)*x(jl(j),i)
else
if (jl(j) .eq. k) then
if (abs(l(j)) .le. 0.D0) goto 5
!     diagonal element is not zero, hence we divide and leave the loop
x(k,i) = t / l(j)
goto 3
endif
endif
1 continue
3 continue
enddo

return
5 n = -k
return
end
!-----------------------------------------------------------------------
subroutine spamback (n,p,x,b,r,jr,ir)
implicit none

integer n, p, jr(*),ir(n+1)
double precision  x(n,p), b(n,p), r(*)

integer l, j, k
double precision  t
!-----------------------------------------------------------------------
!             Solves   R x = b    R = upper triangular.
!-----------------------------------------------------------------------
!
! On entry:
!----------
! n,p      = integers. dimension of problem.
! b        = real array containg the right side.
!
! r, jr, ir,    = Upper triangular matrix stored in CSR format.
!
! On return:
!-----------
!       x = The solution of  R x = b .
!--------------------------------------------------------------------
!     Reinhard Furrer June 2008, April 2012, Sept 2016, Nov 2019

k = n+1
if (abs( r(ir(n+1)-1) ) .le. 0.D0) goto 5
do l=1,p
x(n,l) = b(n,l) / r(ir(n+1)-1)
do 3 k = n-1,1,-1
t = b(k,l)
do 1  j = ir(k+1)-1,ir(k),-1
if (jr(j) .gt. k) then
t = t - r(j)*x(jr(j),l)
else
if (jr(j) .eq. k) then
if (abs(r(j)) .le. 0.D0) goto 5
!     diagonal element is not zero, hence we divide and leave the loop
x(k,l) = t / r(j)
goto 3
endif
endif
1 continue
3 continue
enddo
return
5 n = -k
return
end
!-----------------------------------------------------------------------
