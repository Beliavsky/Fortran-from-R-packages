...
nnz=a%nnz()
cache=512
if(present(cache_kb))cache=cache_kb
piv='mmd'
if(present(pivot))piv=lower(trim(pivot))
select case(trim(piv))
...
nsubmax=max(n+1,4*max(nnz,n))
nnzlmax=max(n+1,6*max(nnz,n))
ierr=0
allocate(invp(n),perm(n),xlindx(n+1),xlnz(n+1),snode(n),xsuper(n+1))
do tries=1,8
 allocate(lindx(nsubmax),lnz(nnzlmax))
 lindx=0
 lnz=0.0_dp
...
f%colpointers=0
f%supernodes=0
f%colindices=lindx(:nsub)
f%colpointers(:nsuper+1)=xlindx(:nsuper+1)
f%supernodes(:nsuper+1)=xsuper(:nsuper+1)
...
allocate(bm(f%n,1),sol(f%n,1),work(f%n))
bm(:,1)=b
...
real(dp),intent(in)::b(:,:)
real(dp),allocatable::x(:,:)
real(dp),allocatable::work(:),bm(:,:),sol(:,:)
integer::j
...
v=0.0_dp
do i=1,f%n
 if(f%entries(f%rowpointers(i))<=0.0_dp)then
 v=-huge(1.0_dp)
 return
 end if
...
do s=1,f%nsuper
   firstcol=f%supernodes(s)
   lenrow=f%colpointers(s+1)-f%colpointers(s)
   do j=firstcol,f%supernodes(s+1)-1
      base=f%rowpointers(j)
      offset=j-firstcol
