...
logical,intent(in),optional::index_mode
type(csr_matrix)::b
integer,allocatable::rp(:),cq(:),ir(:),jc(:)
real(dp),allocatable::v(:)
integer::i,k,nz,newr,newc
logical::ind
if(.not.present(p).and..not.present(q))error stop 'permute_csr: P or Q required'
ind=.false.
if(present(index_mode))ind=index_mode
if(present(p))then
 if(size(p)/=a%nrow)error stop 'permute_csr: P length mismatch'
 if(ind)then
 rp=p
 else
 rp=inverse_permutation(p)
 end if
else
 allocate(rp(a%nrow))
 do i=1,a%nrow
...
do i=1,a%nrow
 ...
 do nz=a%rowpointers(i),a%rowpointers(i+1)-1
  ...
  newc=0
  do newc=1,a%ncol
  if(cq(newc)==a%colindices(nz))exit
  end do
  jc(k)=newc
  v(k)=a%entries(nz)
 end do
