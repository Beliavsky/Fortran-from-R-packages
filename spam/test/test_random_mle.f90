...
type(csr_matrix)::a,dist,q
type(mle_result)::fit
integer::i
sig=reshape([1d0,0.4d0,0.4d0,2d0],[2,2])
a=csr_from_dense(sig)
mu=[1d0,-2d0]
call set_seed_int(12345)
z=rmvnorm_cov(n,mu,a)
m=sum(z,dim=1)/real(n,dp)
s=0
do i=1,n
s=s+outer(z(i,:)-m,z(i,:)-m)
end do
s=s/real(n,dp)
call check(maxval(abs(m-mu))<0.07d0,'rmvnorm mean')
call check(maxval(abs(s-sig))<0.1d0,'rmvnorm covariance')
q=csr_from_dense(reshape([2d0,0.5d0,0.5d0,1.5d0],[2,2]))
call set_seed_int(777)
z=rmvnorm_prec(n,[0d0,0d0],q)
m=sum(z,dim=1)/real(n,dp)
s=0
do i=1,n
s=s+outer(z(i,:)-m,z(i,:)-m)
...
loc(:,1)=[0d0,1d0,2d0,3d0,4d0]
dist=nearest_dist(loc,delta=10d0,full=.true.)
y=[1d0,2d0,3d0,4d0,5d0]
xx(:,1)=1d0
fit=mle_spam(y,xx,dist,[2d0],[1d0],[0.01d0],[20d0],'nugget',maxit=100,reltol=1d-8)
call check(fit%convergence==0,'mle convergence')
call check(abs(fit%par(1)-3d0)<2d-3,'mle mean')
