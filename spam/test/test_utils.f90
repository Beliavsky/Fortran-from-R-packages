...
a=csr_subset(a,[1,2,3],[1,2,3])
c=permute_csr(a,p=p,q=p,index_mode=.true.)
z=csr_to_dense(c)
call check(all(abs(diagonal_dense(z)-[3d0,1d0,2d0])<1d-14),'permutation')
splits=[1,3,4]
fact=reshape([2d0,4d0,3d0,5d0],[2,2])
c=gmult(a,splits,fact)
z=csr_to_dense(c)
call check(abs(z(1,1)-2d0)<1d-14 .and. abs(z(3,3)-15d0)<1d-14,'gmult')
q=precmat_season(6,3)
z=csr_to_dense(q)
b=0
b(1:5,1:5)=0
