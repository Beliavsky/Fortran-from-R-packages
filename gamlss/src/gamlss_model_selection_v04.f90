! Parameter-general matrix-based stepwise GAIC selection.
! Extends v0.3 mu-only forward/backward/both search to sigma, nu and tau.
! SPDX-License-Identifier: GPL-3.0-only
module gamlss_model_selection_v04
   ...
   use gamlss_model_selection_v03, only : STEP_FORWARD,STEP_BACKWARD,STEP_BOTH
   implicit none
   private
   public :: stepwise_gaic_parameter
contains

   subroutine stepwise_gaic_parameter(y,x_base,x_candidates,family,parameter,result,direction, &
      start_selected,k,x_mu,x_sigma,x_nu,x_tau,weights,control,max_steps)
      ...
      real(dp),intent(in),optional::k,x_mu(:,:),x_sigma(:,:),x_nu(:,:),x_tau(:,:),weights(:)
      type(gamlss_control_t),intent(in),optional::control
      logical,allocatable::selected(:),trial(:)
      integer,allocatable::sel(:)
      real(dp),allocatable::path(:),tmp_path(:),xtarget(:,:)
      type(gamlss_result_t)::fit,bestfit
      integer::dir,ncand,j,step,nstep,istat,bestj,bestaction,np
      real(dp)::kk,current,best,val

      np=family_npar(family)
      if(parameter<1.or.parameter>np)then;result%status=1;return;end if
      if(size(x_base,1)/=size(y).or.size(x_candidates,1)/=size(y))then;result%status=2;return;end if
      dir=STEP_BOTH;if(present(direction))dir=direction
      if(dir<STEP_FORWARD.or.dir>STEP_BOTH)then;result%status=3;return;end if
      ...
      if(present(start_selected))then
         do j=1,size(start_selected)
            if(start_selected(j)<1.or.start_selected(j)>ncand)then;result%status=4;return;end if
            selected(start_selected(j))=.true.
         end do
      else if(dir==STEP_BACKWARD)then
         selected=.true.
      end if
      call logical_to_indices(selected,sel);call build_target(x_base,x_candidates,sel,xtarget)
      call fit_target(xtarget,fit,istat);if(istat/=0)then;result%status=5;return;end if
      ...
      do step=1,nstep
         ...
         do j=1,ncand
            if((dir==STEP_FORWARD.or.dir==STEP_BOTH).and..not.selected(j))then
               ...
               call build_target(x_base,x_candidates,sel,xtarget);call fit_target(xtarget,fit,istat)
               if(istat==0)then
   ...
   contains
      subroutine fit_target(xt,model,status)
         real(dp),intent(in)::xt(:,:)
         type(gamlss_result_t),intent(out)::model
         integer,intent(out)::status
         real(dp),allocatable::xm(:,:),xs(:,:),xn(:,:),xu(:,:)
         integer::n
         n=size(y)
         call optional_design(n,x_mu,xm);call optional_design(n,x_sigma,xs)
         call optional_design(n,x_nu,xn);call optional_design(n,x_tau,xu)
         ...
         call fit_gamlss_model(y,xm,family,model,x_sigma=xs,x_nu=xn,x_tau=xu,weights=weights,control=control)
         status=model%status
      end subroutine fit_target
   end subroutine stepwise_gaic_parameter
   ...
   subroutine logical_to_indices(mask,idx)
      ...
      do j=1,size(mask)
         if(mask(j))then;k=k+1;idx(k)=j;end if
      end do
   end subroutine logical_to_indices

   subroutine build_target(base,candidates,sel,x)
...
end module gamlss_model_selection_v04
