...
contains
  subroutine nelder_mead(fun, context, x, value, status, iterations, max_iter, tolerance, initial_step, verbose)
    ...
    n=size(x); miter=1000; if (present(max_iter)) miter=max_iter
    tol=1.0e-8_dp; if (present(tolerance)) tol=max(tolerance,epsilon(1.0_dp))
    step=0.1_dp; if (present(initial_step)) step=max(initial_step,sqrt(epsilon(1.0_dp)))
    trace=.false.; if (present(verbose)) trace=verbose
    if (n==0 .or. miter<1) then
      value=huge(1.0_dp); status=NLME_INVALID_ARGUMENT; iterations=0; return
    end if
    ...
    do iterations=1,miter
      ...
      if (spread_f<=tol .and. spread_x<=sqrt(tol)) then
        status=NLME_SUCCESS; exit
      end if
      centroid=(sum(simplex,dim=2)-simplex(:,ihi))/real(n,dp)
      xr=centroid+alpha*(centroid-simplex(:,ihi)); fr=fun(xr,context)
      if (.not.ieee_is_finite(fr)) fr=huge(1.0_dp)/100.0_dp
      if (fr<f(ilo)) then
        xe=centroid+gamma*(xr-centroid); fe=fun(xe,context)
    ...
    ilo=minloc(f,dim=1); x=simplex(:,ilo); value=f(ilo)
  ...
  subroutine finite_difference_gradient(fun,context,x,gradient,status,step)
    ...
    real(dp) :: h,fp,fm
    integer :: i
    h=epsilon(1.0_dp)**(1.0_dp/3.0_dp); if (present(step)) h=step
    allocate(gradient(size(x)),xp(size(x)),xm(size(x)))
    do i=1,size(x)
      xp=x; xm=x
      xp(i)=xp(i)+h*max(1.0_dp,abs(x(i))); xm(i)=xm(i)-h*max(1.0_dp,abs(x(i)))
      fp=fun(xp,context); fm=fun(xm,context)
      if (.not.ieee_is_finite(fp) .or. .not.ieee_is_finite(fm)) then
        gradient=0.0_dp; status=NLME_NONFINITE; return
      end if
  ...
  subroutine finite_difference_hessian(fun,context,x,hessian,status,step)
    ...
    integer :: i,j,n
    h=epsilon(1.0_dp)**0.25_dp; if (present(step)) h=step
    n=size(x); allocate(hessian(n,n),xpp(n),xpm(n),xmp(n),xmm(n)); hessian=0.0_dp
    f0=fun(x,context)
    if (.not.ieee_is_finite(f0)) then; status=NLME_NONFINITE; return; end if
    do i=1,n
      hi=h*max(1.0_dp,abs(x(i)))
      xpp=x; xmm=x; xpp(i)=x(i)+hi; xmm(i)=x(i)-hi
      fp=fun(xpp,context); fm=fun(xmm,context)
      if (.not.ieee_is_finite(fp) .or. .not.ieee_is_finite(fm)) then; status=NLME_NONFINITE; return; end if
      ...
      do j=1,i-1
        ...
        fpp=fun(xpp,context); fpm=fun(xpm,context); fmp=fun(xmp,context); fmm=fun(xmm,context)
        if (any(.not.ieee_is_finite([fpp,fpm,fmp,fmm]))) then; status=NLME_NONFINITE; return; end if
