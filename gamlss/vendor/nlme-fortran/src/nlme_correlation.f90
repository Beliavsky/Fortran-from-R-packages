...
contains
  ...
  subroutine correlation_matrix(spec, covariate, matrix, status, coordinates)
    ...
    n = size(covariate)
    if (n < 1) then
      allocate(matrix(0,0)); status = NLME_DIMENSION_ERROR; return
    end if
    ...
    case (COR_AR1)
      ...
      do j = 2, n
        do i = 1, j - 1
          ...
          matrix(i,j) = rho**lag; matrix(j,i) = matrix(i,j)
        end do
      end do
      status = NLME_SUCCESS
    ...
    case (COR_CAR1)
      ...
      do j = 2, n
        do i = 1, j - 1
          matrix(i,j) = rho**abs(covariate(j)-covariate(i)); matrix(j,i) = matrix(i,j)
        end do
      end do
      status = NLME_SUCCESS
    ...
    case (COR_ARMA)
      ...
      do j = 2, n
        do i = 1, j - 1
          lag = nint(abs(covariate(j)-covariate(i)))
          matrix(i,j) = acf(lag+1); matrix(j,i) = matrix(i,j)
        end do
      end do
    ...
    case (COR_COMPOUND_SYMM)
      ...
      rho = spec%par(1); lower = -1.0_dp / real(max(1,n-1),dp)
      if (rho <= lower .or. rho >= 1.0_dp) then
        status = NLME_INVALID_ARGUMENT; return
      end if
      do j = 2, n
        do i = 1, j - 1
          matrix(i,j) = rho; matrix(j,i) = rho
        end do
      end do
      status = NLME_SUCCESS
    ...
    case (COR_EXPONENTIAL, COR_GAUSSIAN, COR_LINEAR, COR_RATIO, COR_SPHERICAL)
      ...
      do j = 2, n
        do i = 1, j - 1
          ...
          matrix(i,j) = ratio*rho; matrix(j,i) = matrix(i,j)
        end do
      end do
      status = NLME_SUCCESS
    ...
    case (COR_UNSTRUCTURED)
      ...
      allocate(l(n,n),raw(n,n)); l = 0.0_dp
      do i = 1, n
        l(i,i) = 1.0_dp
      end do
      lag = 0
      ...
      matrix = symmetrize(matrix); status = NLME_SUCCESS
  ...
  subroutine spatial_distance_matrix(coordinates, dist)
    ...
    integer :: n, i, j
    n = size(coordinates,1); allocate(dist(n,n)); dist = 0.0_dp
    do j = 2, n
      do i = 1, j - 1
  ...
  subroutine arma_autocorrelation(phi, theta, maxlag, acf, status)
    ...
    real(dp) :: tail
    p = size(phi); q = size(theta)
    if (maxlag < 0 .or. any(abs(phi) >= 1.0_dp) .or. any(.not. ieee_is_finite(phi)) .or. &
        any(.not. ieee_is_finite(theta))) then
      allocate(acf(0)); status = NLME_INVALID_ARGUMENT; return
    end if
    trunc = max(maxlag + 200,2000)
    allocate(psi(0:trunc+maxlag),gamma(0:maxlag)); psi = 0.0_dp; psi(0)=1.0_dp
    do k = 1, ubound(psi,1)
    ...
    allocate(acf(maxlag+1)); acf = gamma/gamma(0); acf(1)=1.0_dp
    status = NLME_SUCCESS
  ...
  subroutine correlation_to_unconstrained(spec, n, x, status)
    ...
    integer :: k
    real(dp) :: lower, z
    k = correlation_parameter_count(spec,n); allocate(x(k))
    if (k == 0) then
      status = NLME_SUCCESS; return
    end if
    ...
    select case (spec%kind)
    case (COR_AR1)
      if (abs(spec%par(1)) >= 1.0_dp) then; status=NLME_INVALID_ARGUMENT; return; end if
      x(1)=atanh(spec%par(1))
    case (COR_CAR1)
      if (spec%par(1)<=0.0_dp .or. spec%par(1)>=1.0_dp) then; status=NLME_INVALID_ARGUMENT; return; end if
      x(1)=log(spec%par(1)/(1.0_dp-spec%par(1)))
    case (COR_ARMA)
      if (any(abs(spec%par)>=1.0_dp)) then; status=NLME_INVALID_ARGUMENT; return; end if
      x=atanh(spec%par)
    case (COR_COMPOUND_SYMM)
      lower=-1.0_dp/real(max(1,n-1),dp); z=(spec%par(1)-lower)/(1.0_dp-lower)
      if (z<=0.0_dp .or. z>=1.0_dp) then; status=NLME_INVALID_ARGUMENT; return; end if
      x(1)=log(z/(1.0_dp-z))
    case (COR_EXPONENTIAL,COR_GAUSSIAN,COR_LINEAR,COR_RATIO,COR_SPHERICAL)
      if (spec%par(1)<=0.0_dp) then; status=NLME_INVALID_ARGUMENT; return; end if
      x(1)=log(spec%par(1))
      if (spec%nugget) then
        if (spec%par(2)<=0.0_dp .or. spec%par(2)>=1.0_dp) then; status=NLME_INVALID_ARGUMENT; return; end if
  ...
  subroutine correlation_from_unconstrained(template, n, x, spec, status)
    ...
    integer :: k
    real(dp) :: lower, z
    spec=template; k=correlation_parameter_count(template,n)
    if (size(x)/=k) then; status=NLME_DIMENSION_ERROR; return; end if
    if (allocated(spec%par)) deallocate(spec%par); allocate(spec%par(k))
    ...
    case (COR_ARMA)
      spec%par=tanh(x)
    case (COR_COMPOUND_SYMM)
      lower=-1.0_dp/real(max(1,n-1),dp); z=1.0_dp/(1.0_dp+exp(-max(-40.0_dp,min(40.0_dp,x(1)))))
