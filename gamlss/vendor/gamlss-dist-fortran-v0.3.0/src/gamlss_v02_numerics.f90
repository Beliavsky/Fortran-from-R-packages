...
module gamlss_v02_numerics
   ...
   real(dp), save :: bk_x = 1.0_dp, bk_nu = 0.0_dp
...
contains
   ...
   function bessel_k_integral(x,nu) result(k)
      ...
      bk_x=x
      bk_nu=nu
      k=adaptive_integral(bessel_k_integrand,0.0_dp,tmax,2.0e-11_dp,24)
   end function bessel_k_integral

   function bessel_k_integrand(t) result(v)
      real(dp), intent(in) :: t
      real(dp) :: v,a,b,m
      a=-bk_x*cosh(t)+bk_nu*t
      b=-bk_x*cosh(t)-bk_nu*t
      m=max(a,b)
      if (m<-745.0_dp) then
         v=0.0_dp
      else
   ...
   end function bessel_k_integrand
