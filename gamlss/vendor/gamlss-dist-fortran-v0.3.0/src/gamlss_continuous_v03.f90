...
module gamlss_continuous_v03
   ...
   integer,parameter :: CB_SN1=1,CB_GT=2,CB_EXGAUS=3
...
contains
   ...
   real(dp) function v03_pdf_callback(x) result(y)
      real(dp),intent(in)::x
      select case(pdf_callback_family)
      case(CB_SN1)
         y=dSN1(x,0.0_dp,1.0_dp,pdf_callback_par(3))
      case(CB_GT)
         y=dGT(pdf_callback_par(1)+pdf_callback_par(2)*x,pdf_callback_par(1), &
            pdf_callback_par(2),pdf_callback_par(3),pdf_callback_par(4))*pdf_callback_par(2)
      case default
         y=0.0_dp
      end select
   end function v03_pdf_callback

   real(dp) function v03_cdf_callback(x) result(y)
      real(dp),intent(in)::x
      select case(cdf_callback_family)
      case(CB_SN1)
         y=pSN1(x,cdf_callback_par(1),cdf_callback_par(2),cdf_callback_par(3))
      case(CB_GT)
         y=pGT(x,cdf_callback_par(1),cdf_callback_par(2),cdf_callback_par(3),cdf_callback_par(4))
      case(CB_EXGAUS)
         y=pexGAUS(x,cdf_callback_par(1),cdf_callback_par(2),cdf_callback_par(3))
      case default
         y=0.0_dp
      end select
   end function v03_cdf_callback
   ...
   real(dp) function pSN1(q,mu,sigma,nu,lower_tail,log_p) result(v)
      ...
      if(z<=-12.0_dp)then
         ...
         a=-12.0_dp;pdf_callback_family=CB_SN1;pdf_callback_par=[0.0_dp,1.0_dp,nu,0.0_dp]
         raw=adaptive_integral(v03_pdf_callback,a,z,5.0e-10_dp,22)
      end if
   ...
   real(dp) function qSN1(p,mu,sigma,nu,lower_tail,log_p) result(v)
      ...
      cdf_callback_family=CB_SN1;cdf_callback_par=[mu,sigma,nu,0.0_dp]
      lo=mu-8.0_dp*sigma; hi=mu+8.0_dp*sigma
      do while(v03_cdf_callback(lo)>pp); lo=mu-2.0_dp*(mu-lo); end do
      do while(v03_cdf_callback(hi)<pp); hi=mu+2.0_dp*(hi-mu); end do
      v=bisection_root(v03_cdf_callback,pp,lo,hi,2.0e-9_dp,120)
   end function qSN1
   ...
   real(dp) function pGT(q,mu,sigma,nu,tau,lower_tail,log_p) result(v)
      ...
      if(abs(z)==0.0_dp)then
         ...
         pdf_callback_family=CB_GT;pdf_callback_par=[mu,sigma,nu,tau]
         if(z<0.0_dp)then
            raw=0.5_dp-adaptive_integral(v03_pdf_callback,z,0.0_dp,2.0e-10_dp,20)
         else
            raw=0.5_dp+adaptive_integral(v03_pdf_callback,0.0_dp,z,2.0e-10_dp,20)
         end if
      end if
      v=finish_prob(raw,lower_tail,log_p)
   end function pGT
   ...
   real(dp) function qgt_numeric(pp,mu,sigma,nu,tau) result(v)
      ...
      real(dp)::lo,hi
      cdf_callback_family=CB_GT;cdf_callback_par=[mu,sigma,nu,tau]
      lo=mu-8.0_dp*sigma; hi=mu+8.0_dp*sigma
      do while(v03_cdf_callback(lo)>pp); lo=mu-2.0_dp*(mu-lo); end do
      do while(v03_cdf_callback(hi)<pp); hi=mu+2.0_dp*(hi-mu); end do
      v=bisection_root(v03_cdf_callback,pp,lo,hi,2.0e-9_dp,100)
   ...
   real(dp) function qexGAUS(p,mu,sigma,nu,lower_tail,log_p) result(v)
      ...
      if(pp<=0.0_dp)then; v=-infv(); return; end if
      if(pp>=1.0_dp)then; v=infv(); return; end if
      cdf_callback_family=CB_EXGAUS;cdf_callback_par=[mu,sigma,nu,0.0_dp]
      lo=mu-8.0_dp*sigma; hi=mu+8.0_dp*sigma+12.0_dp*nu
      do while(v03_cdf_callback(lo)>pp); lo=lo-4.0_dp*sigma; end do
      do while(v03_cdf_callback(hi)<pp); hi=hi+max(4.0_dp*sigma,4.0_dp*nu); end do
      v=bisection_root(v03_cdf_callback,pp,lo,hi,2.0e-9_dp,120)
