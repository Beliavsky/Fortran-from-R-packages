...
module gamlss_student_t
   use, intrinsic :: ieee_arithmetic, only : ieee_is_finite, ieee_value, ieee_quiet_nan
   use gamlss_kinds, only : dp, pi
   use gamlss_special, only : regularized_beta
   use gamlss_random, only : random_normal, random_gamma
   ...
   type :: bvt_fit_context_t
      real(dp), allocatable :: x1(:), x2(:), xdf(:, :), xrho(:, :), w(:)
      integer :: pd = 0, pr = 0, np = 0
   end type bvt_fit_context_t

   type :: tcop_fit_context_t
      real(dp), allocatable :: u(:), v(:), xrho(:, :), w(:), xu(:), xv(:)
      real(dp) :: df0 = 8.0_dp
      integer :: pr = 0, np = 0
      logical :: estimate_df = .true.
   end type tcop_fit_context_t

   type(bvt_fit_context_t), save :: bvt_ctx
   type(tcop_fit_context_t), save :: tcop_ctx
...
contains
   ...
   subroutine fit_bivariate_student_t(x1, x2, xdf, xrho, result, weights, max_iter, tol)
      ...
      bvt_ctx%x1 = x1; bvt_ctx%x2 = x2; bvt_ctx%xdf = xdf; bvt_ctx%xrho = xrho; bvt_ctx%w = w
      bvt_ctx%pd = pd; bvt_ctx%pr = pr; bvt_ctx%np = np
      call bfgs_minimize(bvt_objective, par, fval, stat, max_iter=niter, tol=tolerance)
      ...
      allocate(hess(np, np)); call numerical_hessian(bvt_objective, par, hess)
   ...
   subroutine fit_student_t_copula(u, v, xrho, result, estimate_df, initial_df, weights, max_iter, tol)
      ...
      tolerance = 1.0e-7_dp; if (present(tol)) tolerance = tol
      tcop_ctx%u = u; tcop_ctx%v = v; tcop_ctx%xrho = xrho; tcop_ctx%w = w
      tcop_ctx%xu = xu; tcop_ctx%xv = xv; tcop_ctx%df0 = df0
      tcop_ctx%pr = pr; tcop_ctx%np = np; tcop_ctx%estimate_df = estdf
      call bfgs_minimize(tcop_objective, par, fval, stat, max_iter=niter, tol=tolerance)
      result%status = stat; result%converged = stat == 0
      ...
      result%loglik = -fval; result%aic = 2.0_dp*fval + 2.0_dp*real(np, dp)
      allocate(hess(np, np)); call numerical_hessian(tcop_objective, par, hess)
      call invert_matrix(hess, cov, stat2)
      if (stat2 == 0) then
         result%covariance = cov
      else
   ...
   real(dp) function bvt_objective(theta) result(nll)
      real(dp), intent(in) :: theta(:)
      real(dp) :: dfv, rhov, ld
      integer :: j
      nll = 0.0_dp
      do j = 1, size(bvt_ctx%x1)
         dfv = 1.0_dp + exp(min(30.0_dp, max(-30.0_dp, &
            dot_product(bvt_ctx%xdf(j, :), theta(1:bvt_ctx%pd)))))
         rhov = tanh(dot_product(bvt_ctx%xrho(j, :), theta(bvt_ctx%pd + 1:bvt_ctx%np)))
         ld = bivariate_student_t_logpdf(bvt_ctx%x1(j), bvt_ctx%x2(j), dfv, rhov)
         if (.not. ieee_is_finite(ld)) then
            nll = huge(1.0_dp)/100.0_dp
            return
         end if
         nll = nll - bvt_ctx%w(j)*ld
      end do
   end function bvt_objective

   real(dp) function tcop_objective(theta) result(nll)
      real(dp), intent(in) :: theta(:)
      real(dp) :: dfv, rhov, dens
      integer :: j
      dfv = tcop_ctx%df0
      if (tcop_ctx%estimate_df) dfv = exp(min(20.0_dp, max(-2.0_dp, theta(tcop_ctx%np))))
      nll = 0.0_dp
      do j = 1, size(tcop_ctx%u)
         rhov = tanh(dot_product(tcop_ctx%xrho(j, :), theta(1:tcop_ctx%pr)))
         if (tcop_ctx%estimate_df) then
            dens = student_t_copula_pdf(tcop_ctx%u(j), tcop_ctx%v(j), dfv, rhov)
         else
            dens = exp(min(700.0_dp, bivariate_student_t_logpdf(tcop_ctx%xu(j), tcop_ctx%xv(j), dfv, rhov) &
               - student_t_logpdf(tcop_ctx%xu(j), dfv) - student_t_logpdf(tcop_ctx%xv(j), dfv)))
         end if
         if (dens <= tiny(1.0_dp) .or. .not. ieee_is_finite(dens)) then
            nll = huge(1.0_dp)/100.0_dp
            return
         end if
         nll = nll - tcop_ctx%w(j)*log(dens)
      end do
   end function tcop_objective
