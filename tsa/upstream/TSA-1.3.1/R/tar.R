...
# Revised on 6/26/2016
# This version fixes a bug so that is.constant1 and is.constant2 work properly.
# Thanks to Dong Li for pointing out the bug.
#
    cvar <- function(x, df = 1) {
        sum(x^2)/(length(x) - df)
    }
    fmaic <- function(R, n, order.select = FALSE) {
        k <- dim(R)[2]
        v <- R[-1, k]^2
        v <- rev(cumsum(rev(v)))
        k <- k - 1
        ...
        else return(list(MAIC = AIC[length(AIC)], order = k - 
            1, AIC = AIC, like = like[length(AIC)]))
    }
    cat1 <- function(..., print = TRUE, file = "", sep = " ", 
    ...
    makedata <- function(dataf, p1, p2, d, is.constant1 = TRUE, 
        is.constant2 = TRUE, thd.by.phase = FALSE) {
        n <- dim(dataf)[1]
        nseries <- dim(dataf)[2]
    ...
    makexy <- function(x, p, start, d, thd.by.phase = FALSE) {
        n <- length(x)
        xy <- NULL
    ...
    setxy <- function(old.xy, p, p1, nseries, is.coefficient = TRUE) {
        if (p1 >= 1) 
            s <- 1:p1
        else s <- NULL
        n <- dim(old.xy)[1]
    ...
    findstart <- function(x, nseries, indexid, p) {
        m <- dim(x)[1]
        amax <- 0
        for (i in (1:nseries)) {
    ...
    if (is.null(lab)) 
        lab <- ""
    if(is.constant1) sublab1=c("intercept", sublab) else sublab1=sublab
    if (lab == "") 
        dimnames(xy1)[[2]] <- as.vector(c(outer(sublab1, lab, paste, sep = " "), "", "", ""))
    else dimnames(xy1)[[2]] <- as.vector(c(outer(sublab1, lab, paste, sep = "-"), "", "", ""))
    s <- (q2 - 3 - p2):(q2 - 3)
    temp <- xy2[, s]
    ...
    if(is.constant2) sublab2=c("intercept", sublab) else sublab2=sublab   
    if (lab == "") 
        dimnames(xy2)[[2]] <- as.vector(c(outer(sublab2, lab, paste, sep = " "), "", "", ""))
    else dimnames(xy2)[[2]] <- as.vector(c(outer(sublab2, lab, paste, sep = "-"), "", "", ""))
    ...
    R1 <- qr.R(qr1)
    if (MAIC) {
        order1 = fmaic(R1, thdindex, order.select = order.select)$order + 
            1
        subset1 = 1:order1
        p1 = order1 - is.constant1
    }
    else subset1 = -((q1 - 2):q1)
    qr1 <- lsfit(x.regime1 <- xy1[1:thdindex, subset1, drop = FALSE], 
        y.regime1 <- xy1[1:thdindex, q1 - 2], intercept = FALSE)
    dxy1 = xy1[, subset1, drop = FALSE]
    dxy1[-(1:thdindex), ] = 0
    s <- -((q2 - 1):q2)
    qr2 <- qr(xy2[(thdindex + 1):m, s])
    R2 <- qr.R(qr2)
    if (MAIC) {
        order2 = fmaic(R2, m - thdindex, order.select = order.select)$order + 
            1
        subset2 = 1:order2
        p2 = order2 - is.constant1
    }
    else subset2 = -((q2 - 2):q2)
    qr2 <- lsfit(x.regime2 <- xy2[-(1:thdindex), subset2, drop = FALSE], 
        y.regime2 <- xy2[-(1:thdindex), q2 - 2], intercept = FALSE)
    dxy2 = xy2[, subset2, drop = FALSE]
    dxy2[(1:thdindex), ] = 0
    ...
    cat1(print = print, "The estimated threshold is the ", signif(thdindex/m * 
        100, 3), " percentile of\nall data.\n")
    ...
    cat1(print = print, rbind(lab, signif((rms1 * (n1 - p1 - 
        1))/n1, 4)), "\n")
    cat1(print = print, "\n\n upper regime: \n")
    if (print) 
        ls.print(qr2)
    ...
    cat1(print = print, "\n\n (max. likelihood) RMS for each series (denominator=sample size in the regime)\n")
    cat1(print = print, signif((rms2 * (n2 - p2 - 1))/n2, 4), 
        "\n")
    AIC <- signif(n1 * log((rms1 * (n1 - p1 - is.constant1))/n1) + 
        n2 * log((rms2 * (n2 - p2 - is.constant2))/n2) + +(n1 + 
        n2) * (1 + log(2 * pi)) + 2 * (p1 + p2 + is.constant1 + 
        is.constant2 + 1), 4)
    cat1(print = print, "\n Nominal AIC is ", AIC, "\n")
    ...
    dxy1[sort.l, ] = dxy1
    dxy2[sort.l, ] = dxy2
    res = list(dxy1 = dxy1, dxy2 = dxy2, p1 = p1, q1 = q1, d = d, 
        qr1 = qr1, qr2 = qr2, x.regime1 = x.regime1, y.regime1 = y.regime1, 
        ...
        p1 = p1, p2 = p2, rms1 = rms1, rms2 = rms2, is.constant1 = is.constant1, 
        is.constant2 = is.constant2, residuals = residuals, AIC = AIC, 
        aic.no.thd = aic.no.thd, y = y, like = max(like1 + like2), 
        method = method)
    class(res) = "TAR"
    invisible(res)
}
