...
#'
#' @param ... One or more simpler [Manifold definitions]
#'
...
get.product.defn <- function(...) {
	...
	for (i in 1:L) {
		if ("ManifoldOptim::manifold.defn" %notin% class(x[[i]])) {
...
#' \itemize{
#' \item{`get.stiefel.defn`}: Stiefel manifold
#'    \eqn{\{X \in R^{n \times p} : X^T X = I\}}
#' \item{`get.grassmann.defn`}: Grassmann manifold of \eqn{p}-dimensional
#'   subspaces in \eqn{R^n}
#' \item{`get.spd.defn`}: Manifold of \eqn{n \times n} symmetric positive
#'    definite matrices
#' \item{`get.sphere.defn`}: Manifold of \eqn{n}-dimensional vectors on
#'    the unit sphere
#' \item{`get.euclidean.defn`}: Euclidean \eqn{R^{n \times m}} space
#' \item{`get.lowrank.defn`}: Low-rank manifold
#'    \eqn{\{ X \in R^{n \times m} : \textrm{rank}(X) = p \}}
#' \item{`get.orthgroup.defn`}: Orthonormal group
