...
contains
  ...
  function tsprofile(specification,sizes,repetitions,rng,max_iterations) result(summary)
    ...
    if(ns==0.or.repetitions<1.or.any(sizes<5))then
      ...
      summary%message='positive repetitions and sample sizes >= 5 are required'
      return
    end if
    allocate(summary%sizes(ns))
    summary%sizes=sizes
    allocate(summary%rmse(7,ns),summary%mae(7,ns),summary%mape(7,ns))
    summary%rmse=0.0_dp
    summary%mae=0.0_dp
    summary%mape=0.0_dp
    allocate(errors(7,repetitions,ns))
    errors=0.0_dp
    ...
    do k=1,ns
      do j=1,repetitions
        ...
        if(fit%status==tsd_success)then
          ...
          errors(:,j,k)=estimate-actual
          nok=nok+1
          summary%successful_fits=summary%successful_fits+1
        else
          errors(:,j,k)=0.0_dp
        end if
      end do
    ...
    if(summary%successful_fits>0)then
    summary%status=tsd_success
    summary%message='simulation profile completed'
    else
      summary%status=tsd_invalid_argument
      summary%message='all simulated fits failed'
      end if
