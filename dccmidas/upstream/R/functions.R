...
H_t <- array(NA, dim=c(Num_col,Num_col,TT))
...
#' @examples
#' \donttest{
#' # This example is computationally intensive because it estimates
#' # three univariate GARCH-MIDAS models followed by a DCC-MIDAS model
#' require(xts)
#' # daily log-returns
#' # close to close daily log-returns
#' r_t_s<-diff(log(sp500['2018/2019'][,3]))
#' r_t_s[1]<-0
#' r_t_n<-diff(log(nasdaq['2018/2019'][,3]))
#' r_t_n[1]<-0
#' r_t_f<-diff(log(ftse100['2018/2019'][,3]))
#' r_t_f[1]<-0
#' db_m<-xts::merge.xts(r_t_s,r_t_n,r_t_f)
#' db_m<-db_m[complete.cases(db_m),]
#' colnames(db_m)<-c("S&P500","NASDAQ","FTSE100")
...
#' require(rumidas)
#' mv_m<-mv_into_mat(r_t[[1]],diff(indpro),K=24,"monthly")
#' # list of MV
...
#' MV=MV,K=24,corr_model="DCCMIDAS",N_c=N_c,K_c=K_c)
#' dccmidas_est
