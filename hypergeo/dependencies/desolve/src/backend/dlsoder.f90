...
!
!  Original author: Alan C. Hindmarsh
! 
!  Created by merging DLSODE with DLSODAR by Karline Soetaert
!    for use in R package deSolve.
!


!DECK DLSODER
! DLSODER was created by merging DLSODE with DLSODAR - Karline Soetaert
SUBROUTINE DLSODER (F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL, ITASK, &
& ISTATE, IOPT, RWORK, LRW, IWORK, LIW, JAC, MF, &
& G, NG, JROOT, rpar, ipar)
IMPLICIT NONE
EXTERNAL F, JAC, G
!KS: added rpar, ipar, and G
INTEGER ipar(*)
...
!-----------------------------------------------------------------------
!***BEGIN PROLOGUE  DLSODER
!***PURPOSE  Livermore Solver for Ordinary Differential Equations.
!            DLSODER solves the initial-value problem for stiff or
...
!             Livermore, CA 94551.
!           Root function added by Karline Soetaert
!***DESCRIPTION - see DLSODE and DLSODAR
!  Note: length of RWORK array = 20 + NYH*(MAXORD + 1) + 3*NEQ + LWM + 3*NG.
!
!-----------------------------------------------------------------------
...
! The block DLS001 is declared in subroutines DLSODER, DINTDY, DSTODE,
! DPREPJ, and DSOLSY.
...
& LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER, &
& MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
! karline: added next common block
...
IHIT = .TRUE.
!***FIRST EXECUTABLE STATEMENT  DLSODER
IF (ISTATE .LT. 1 .OR. ISTATE .GT. 3) GO TO 601
IF (ITASK .LT. 1 .OR. ITASK .GT. 5) GO TO 602
! Karline: added next  sentence
ITASKC = ITASK
...
! this in lsoda, not in lsode...       IF (IHIT) T = TCRIT + karline added next line
...
!-----------------------------------------------------------------------
! Block G.
! The following block handles all successful returns from DLSODER.
...
! Karline: added next error messages
...
RETURN
!----------------------- END OF SUBROUTINE DLSODER ----------------------
END


!DECK DLSODESR
! DLSODESR was created by merging DLSODES with DLSODAR - Karline Soetaert
SUBROUTINE DLSODESR (F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL, ITASK, &
& ISTATE, IOPT, RWORK, LRW, IWORK, LIW, IWK, JAC, MF, &
& G, NG, JROOT, rpar, ipar)
IMPLICIT NONE
EXTERNAL F, JAC, G
!KS: added rpar, ipar, and G
...
! the name of the segment.  E.g., the segment YH starts at RWORK(LYH).
! Segments of RWORK (in order) are denoted  G0, G1, GX, WM, YH, SAVF, EWT, ACOR.
