! modified to avoid printing for calls from Fortran within R
...
! jun 2019 renamed function from dgamma to avoid warning with intrinsic
!          function already named dgamma
  double precision x, gamcs(42), dxrel, pi, sinpiy, sq2pil, xmax, &
  &xmin, y, d9lgmc, dcsevl, d1mach, xsml
...
! modified to omit priniting for calls from Fortran within R
...
! added by CF to avoid uninitialized warnings
  b2 = 0
!
  twox = 2.0d0*x
  b1 = 0.d0
...
!
! dlog (dabs (dgam(x)) ) for dabs(x) .le. 10.0
!
  dlngam = log (abs (dgam(x)) )
  return
!
! dlog ( dabs (dgam(x)) ) for dabs(x) .gt. 10.0
!
...
! subtract mean and form sum of squares
  ss = zero
  do j = 1, p
...
!     double precision   x(n,p), u(p), v(p,p), w(p,p), work(lwork),
...
!     if (.false.) then
! this gets the eigenvectors but x is overwritten
...
!     double precision   s(l), O(l,l,m), w(l,l)
...
!     double precision   T(abs(n), abs(n))
...
!     double precision   x(n), mu(G), pro(G[+1]), z(n,G[+1])
...
!     double precision    x(n), d(ng*(ng-1)/2)
...
!     call intpr( 'ic', -1, ic, n)
!     call intpr( 'no. of groups', -1, lg, 1)

! group heads should be first among rows of x
...
!     double precision    x(n), z(n,G[+1]), mu(G), sigsq, pro(G[+1])
...
!       ViLog = log(Vinv)
  ...
  nz = G
  if (EQPRO) then
!         call dcopy( nz, one/dble(nz), 0, pro, 1)
  dummy(1) = one/dble(nz)
...
!     double precision    x(n), z(n,G), mu(G), sigsq, pro(G)
  double precision    x(*), z(n,*), mu(*), sigsq, pro(*)
...
!     double precision   x(n,p), w(p), z(n,G[+1])
  double precision   x(n,*), w(*), z(n,  *  )

!     double precision   mu(p,G), Sigma(p,p), pro(G[+1])
...
!     double precision   x(n,p), z(n,G), w(p)
  double precision   x(n,*), z(n,*), w(*)

!     double precision   mu(p,G), U(p,p), pro(G)
...
!     double precision   pshrnk, pmu(p), pscale(p,p), pdof
...
!------------------------------------------------------------------------------
!
!  x       double    (input) (n,p) matrix of observations.
!  z       double    (input) (n,G) conditional probabilities. 
!  n       integer   (input) number of observations.
!  p       integer   (input) dimension of the data.
!  G       integer   (input) number of Gaussian clusters in the mixture.
!  w       double    (scratch) (p) 
!  mu      double    (output) (p,G) mean for each group.
!  U       double    (output) (p,p) upper triangular Cholesky factor of the 
!         common covariance matrix for the groups: transpose(U) * U = Sigma.
!  pro     double    (output) (G) mixing proportions (ignore result if equal).
...
!     double precision   x(n,p), z(n,G[+1])
  double precision   x(n,*), z(n,  *  )

!     double precision   mu(p,G), shape(p), pro(G[+1])
...
!     double precision    pshrnk, pmu(p), pscale, pdof
  double precision    pshrnk, pmu(*), pscale, pdof
...
!     double precision   mu(p,G), scale, shape(p), pro(G)
...
!     double precision    mu(p,G), scale, shape(p), pro(G[+1])
  double precision    mu(p,*), scale, shape(*), pro(  *  )
...
!     double precision   x(n,p), v(p), w(p), z(n,G[+1])
  double precision   x(n,*), v(*), w(*), z(n,  *  )

!     double precision   mu(p,G), shape(p), O(p,p,G), pro(G[+1])
...
!     double precision   x(n,p), z(n,G[+1]), w(lwork), s(p)
  double precision   x(n,*), z(n,  *  ), w(  *  ), s(*)
...
!     double precision   x(n,p), z(n,G), w(lwork)
  double precision   x(n,*), z(n,*), w(  *  )

!     double precision   shape(p), O(p,p,G), mu(p,G), pro(G)
...
!     double precision   mu(p,G), shape(p), O(p,p,G), pro(G)
  double precision   mu(p,*), shape(*), O(p,p,*), pro(*)
...
!     double precision   x(n,p), mu(p,G), pro(G[+1]), z(n,G[+1])
...
!     double precision    x(n,p), v(p), d(ng*(ng-1)/2)
...
!     double precision    x(n,p), z(n,G[+1]), mu(p,G), pro(G[+1])
...
!     double precision   x(n,p), z(n,G), mu(p,G), sigsq, pro(G)
...
!     double precision   mu(p,G), shape(p,G), pro(G[+1])
...
!       ViLog = log(Vinv)
  ...
  nz = G
!       if (EQPRO) call dscal( G, one/dble(G), pro, 1)  wrong?
  if (EQPRO) then
!         call dcopy( G, one/dble(G), 0, pro, 1)
  dummy(1) = one/dble(G)
  call dcopy( G, dummy, 0, pro, 1)
...
!     double precision   mu(p,G), scale, shape(p,G), pro(G)
...
!     double precision   x(n), mu(G), sigsq(G), pro(G[+1]), z(n,G[+1])
...
!     double precision    x(n), ALPHA, d(ng*(ng-1)/2)
...
!     call intpr( 'ns', -1, ns, 1) 
!     call dblepr( 'alpha', -1, alpha, 1) 
!     call intpr( 'nd', -1, nd, 1) 
...
! store terms also so as not to recompute them
...
!       do j = 2, n
!         nj = ic(j)
!         if (nj .ne. 0 .and. abs(nj) .le. n) then
!           call dblepr( 'dij', -1, d(ij), jj)
!           ij = ij + jj 
!           jj = jj + 1
!         end if
!       end do
...
!     double precision     x(n), z(n,G[+1]), mu(G), sigsq(G), pro(G[+1])
  double precision     x(*), z(n,  *  ), mu(*), sigsq(*), pro(  *  )
...
!     double precision   x(n), z(n,G), mu(G), sigsq(G), pro(G)
  double precision   x(*), z(n,*), mu(*), sigsq(*), pro(*)
...
!     double precision   mu(p,G), scale(G), shape(p), pro(G[+1])
  double precision   mu(p,*), scale(*), shape(*), pro(  *  )
...
!     double precision    x(n,p), z(n,G[+1]), scl(G), shp(p), w(p,G)
...
! start with shape and scale equal to 1
...
!     double precision   x(n,p), z(n,G), scl(G), shp(p), w(p,G)
  double precision   x(n,*), z(n,*), scl(*), shp(*), w(p,*)

!     double precision   mu(p,G), scale(G), shape(p), pro(G)
  double precision   mu(p,*), scale(*), shape(*), pro(*)
...
! start with the equal volume and shape estimate
...
!     double precision   v(p), w(p)
  double precision   v(*), w(*)

!     double precision   scale(G), shape(p), O(p,p,G)
...
! inner iteration to estimate scale and shape
! pro now contains n*pro
...
!     double precision   mu(p,G), pro(G[+1])
  double precision   mu(p,*), pro(  *  )
...
!     double precision   x(n,p), z(n,G), w(max(4*p,5*p-4,p+G))
  double precision   x(n,*), z(n,*), w(*)

!     double precision   scale(G), shape(p), O(p,p,G), mu(p,G), pro(G)
...
! inner iteration estimates scale and shape
! pro now contains n*pro
...
! iteration to estimate scale and shape
! pro now contains n*pro
...
!     double precision   mu(p,G), sigsq(G), pro(G[+1])
...
!     double precision    x(n,p), v(p). d(*), ALPHA
...
! store terms also so as not to recompute them
  ...
  ni     = ic(i)
  ri     = dble(ni)
!         x(i,2) = ri*log(x(i,1)+ALPHA)
  x(i,2) = ri*log((x(i,1)+ALPHA)/ri)
  end if
  end do

!     call intpr( 'ic', -1, ic, n)
!     call dblepr( 'trace', -1, x(1,1), n)
!     call dblepr( 'term', -1, x(1,2), n)
...
! compute change in likelihood and determine minimum
  ...
  call dscal( p, sj, v, 1)
  call daxpy( p, (-si), x(j,1), n, v, 1)
!         trcij = (qi*tracei + qj*tracej) + qij*ddot(p,v,1,v,1)
  trcij = (tracei + tracej) + ddot(p,v,1,v,1)
!         trmij = rij*log(trcij+ALPHA)
  trmij = rij*log((trcij+ALPHA)/rij)
  dij   = trmij - (termi + termj)
  ij    = ij + 1
  d(ij) = dij
...
! update d and find max
  ...
  call dscal( p, sjop, v, 1)
  call daxpy( p, (-siop), x(jopt,1), n, v, 1)
!       trop  = (qi*tracei + qj*tracej) + qij*ddot(p,v,1,v,1)
  trop  = (tracei + tracej) + ddot(p,v,1,v,1)
!       tmop  = rij*log(trop+ALPHA)
  tmop  = rij*log((trop+ALPHA)/rij)
  end if
...
!         sigsq(k) = sigsqk/(pdof+(sumz+one)*dble(p)+two)
  temp   = pdof+sumz*dble(p)+two
...
!     double precision   x(n,p), z(n,G), mu(p,G), sigsq(G), pro(G)
  double precision   x(n,*), z(n,*), mu(p,*), sigsq(*), pro(*)
...
!     double precision    mu(p,G), sigsq(G), pro(G)
  double precision    mu(p,*), sigsq(*), pro(*)
...
!     double precision   mu(p,G), scale(G), shape(p,G), pro(G[+1])
  double precision   mu(p,*), scale(*), shape(p,*), pro(  *  )
...
!     double precision   mu(p,G), scale(G), shape(p,G), pro(G)
  double precision   mu(p,*), scale(*), shape(p,*), pro(*)
...
!     double precision   mu(p,G), Sigma(p,p,G), pro(G[+1])
...
!     double precision   x(n,p+1), v(p), u(p,p), s(p,p)
!     double precision   r(p,p), d(ng*(ng-1)/2)
...
! initialize by simulating merges       
...
20 continue
!         d(ll+k) = trcij
! copy triangular factor into the rows of x
  j  = k
  m  = p
  do i = 1, min(l-1,p)
  j  = ic(j)
...
!     call intpr( 'ic', -1, ic, n)
!     call dblepr( '', -1, x(1,pp1), n)
!     call dblepr( 'trac', -1, d(ll+1), ng)
!     call dblepr( 'term', -1, term, n)
...
!             trcij = half*ddot( p, v, 1, v, 1)
  trcij =      ddot( p, v, 1, v, 1)
  call dcopy( p, v, 1, u, p)
!             trmij = rij*log(BETA*trcij+ALPHA)
...
!             traci = d(ll+i)
!             traci = trac(i)
!             termi = vvvtrm(i,ni,n,p,ic,x,traci)
!             termi = term(i)
  traci = x(   i , pp1)
  termi = x(ic(i), pp1)
  ri  = dble(ni)
...
120 continue
  ...
  nj    = l - n
!         tracj = d(ll+j)
!         termj = vvvtrm(j,nj,n,p,ic,x,tracj)
  tracj = x(    j , pp1)
  termj = x( ic(j), pp1)
  rj    = dble(nj)
  do i = 1, (j-1)
...
310 continue
  ...
  if (l .le. n) goto 310
  nj  = l - n
!           call vvvget(j,nj,n,p,ic,x,tracj,termj)
  tracj = x(   j ,pp1)
  termj = x(ic(j),pp1)
  rj    = dble(nj)
  nij   = ni + nj
...
!     call dblepr("d", 1, d, nd)
!     call dblepr("d", 1, d, ld)
...
610 continue
  ...
  if (l .le. n) goto 610
  ni  = l - n
!           call vvvget(i,ni,n,p,ic,x,traci,termi)
  traci = x(   i , pp1)
  termi = x(ic(i), pp1)
  ri  = dble(ni)
  nij = ni + 1
...
!     double precision   mu(p,G), U(p,p,G), pro(G), S(p,p)
...
!
!  x       double  (input) (n,p) matrix of observations.
!  z       double  (input/output) (n,G[+1]) conditional probabilities. 
!  n       integer (input) number of observations.
!  p       integer (input) dimension of the data.
!  G       integer (input) number of Gaussian clusters in the mixture.
!  mu      double  (output) (p,G) mean for each group.
!  U       double  (output) (p,p,G)
!  pro     double  (output) (G) mixing proportions (used even if equal).
!  w       double  (scratch) (max(p,G))
...
!------------------------------------------------------------------------------
!
!  x       double  (input) (n) matrix of observations (destroyed).
!  n       integer (input) number of observations.
!  mu      double  (output) mean.
!  sigsq   double  (output) variance.
!  hood    double  (output) loglikelihood.
...
!     double precision   x(n,p), mu(p), shape(p)
...
! log posterior computation not yet available
  pdof = FLMAX
...
!     double precision   x(n,p), w(p), mu(p), U(p,p)
...
! mu contains ybar; U contains Cholesky factor of inverse Wishart scale
...
!     double precision   x(n,p), mu(p), U(p,p)
...
!     do j = 1, p
!       do i = 1, j
!         x(i,j) = ddot(i,U(1,i),1,U(1,j),1)
!         if (i .ne. j) x(j,i) = x(i,j)
!        end do
!     end do
!     do j = 1, p
!       call dcopy( p, x(1,j), 1, U(1,j), 1)
!     end do

  return
  end

! Luca: add to check if compile ok
...
! Gaussian model-based clustering algorithm in clusters share a common
! variance (shape, volume, and orientation are the same for all clusters).

  implicit NONE

  integer            n, p, ic(n), ng, ns, io(*), jo(*)

!     double precision   x(n,p), v(p), s(p,p), u(p,p), r(p,p)
  double precision   x(n,*), v(*), s(*), u(*), r(*)

!------------------------------------------------------------------------------
!
!  x       double  (input/output) On input, the (n by p) matrix containing
!                   the observations. On output, the first two columns
!                   and ns rows contain the determinant and trace of the
!                   sum of the sample cross product matrices. Columns 3 and 4
!                   contain the merge indices if p .ge. 4
!  n       integer (input) number of observations
!  p       integer (input) dimension of the data
!  ic      integer (input) (n) Initial partitioning of the data; groups must
!                   be numbered consecutively.
!  ng      integer (input) Number of groups in initial partition.
!  ns      integer (input) Desired number of stages of clustering.
!  io,jo   integer (output [p .le. 3]) If p .lt. 3, both io and jo must be of
!                   length ns and contain the indices of the merged pairs on
!                   output. If p .eq. 3, jo must be of length ns and contains
!                   an index of each merged on output pair. Otherwise io and
!                   jo are not used and can be of length 1.
!  v       double  (scratch/output) (p) On output, algorithm breakpoints;
!                   tells where the algorithm switches from using trace
!                   to trace + det, and from trace + det to det as criterion.
!  s       double  (scratch/output) (p,p) On output the first column contains
!                   the initial trace and determinant of the sum of sample
!                   cross product matrices.
!  u,r      double  (scratch) (p,p)
...
!     call dcopy( lw, zero, 0, r, 1)
  ...
  i = ic(j)
  if (i .ne. j) then
! update trace and Cholesky factor as if a merge
  q     = q + 2
  ni    = ic(i)
  ri    = dble(ni)
  rij   = dble(ni+1)
