...
gcc -std=c11 -O0 -g -Wall -Wextra -c "%ROOT%\src\highs_dynamic_loader.c" -o "%BUILD%\highs_dynamic_loader.o"
...
for %%F in ("%ROOT%\test\*.f90" "%ROOT%\example\*.f90" "%ROOT%\app\*.f90") do (
  gfortran -std=f2018 -O0 -g -Wall -Wextra -Wpedantic -fcheck=all -fbacktrace -J . -I . "%%~fF" *.o -o "%%~nF.exe"
  if errorlevel 1 (popd & exit /b 1)
  pushd "%ROOT%"
  "%BUILD%\%%~nF.exe"
