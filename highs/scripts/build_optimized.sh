...
rm -rf "$BUILD"
mkdir -p "$BUILD"
cc -std=c11 -O3 -DNDEBUG -Wall -Wextra -Wpedantic -c "$ROOT/src/highs_dynamic_loader.c" -o "$BUILD/highs_dynamic_loader.o"
cd "$BUILD"
...
for src in "$ROOT"/test/*.f90 "$ROOT"/example/*.f90 "$ROOT"/app/*.f90; do
  ...
  gfortran -std=f2018 -O3 -DNDEBUG -Wall -Wextra -Wpedantic -J . -I . "$src" ./*.o -o "$exe"
  "$exe"
done
