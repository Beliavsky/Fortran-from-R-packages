...
contains
   ...
   elemental function normal_quantile(p) result(x)
      ...
      real(dp),parameter::a(6)=[-3.969683028665376e1_dp,2.209460984245205e2_dp,-2.759285104469687e2_dp,1.383577518672690e2_dp,-3.066479806614716e1_dp,2.506628277459239_dp]
      real(dp),parameter::b(5)=[-5.447609879822406e1_dp,1.615858368580409e2_dp,-1.556989798598866e2_dp,6.680131188771972e1_dp,-1.328068155288572e1_dp]
      real(dp),parameter::c(6)=[-7.784894002430293e-3_dp,-3.223964580411365e-1_dp,-2.400758277161838_dp,-2.549732539343734_dp,4.374664141464968_dp,2.938163982698783_dp]
      ...
      if(p<0.02425_dp) then
         ...
         q=sqrt(-2.0_dp*log(1.0_dp-p));x=-(((((c(1)*q+c(2))*q+c(3))*q+c(4))*q+c(5))*q+c(6))/((((d(1)*q+d(2))*q+d(3))*q+d(4))*q+1.0_dp)
      else
