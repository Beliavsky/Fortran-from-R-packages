...
contains
   ...
   subroutine cov_comed(x,result,n_iter,reweight)
      ...
      allocate(result%center(p),result%covariance(p,p),result%raw_center(p), &
         result%raw_covariance(p,p),result%distances(n),result%weights(n))
      ...
      cut=chi_square_quantile(0.975_dp,real(p,dp))
      result%weights=d2<=cut; result%distances=sqrt(max(d2,0.0_dp))
   ...
   subroutine cov_mcd(x,result,alpha,n_starts,max_csteps)
      ...
      allocate(subset(h),idx(n),best_subset(h),center(p),cov(p,p),invc(p,p),d(n), &
         ds(n),vals(p),vecs(p,p),bestc(p),bestcov(p,p),mask(n))
   ...
   subroutine adjusted_outlyingness(x,outlyingness,center,n_directions)
      ...
      do d=1,nd
         ...
         sl=qn_scale(pack(proj,proj<=med)); sr=qn_scale(pack(proj,proj>=med))
         if(sl<=0.0_dp) sl=mad_scale(proj)
         if(sr<=0.0_dp) sr=mad_scale(proj)
         do i=1,n
