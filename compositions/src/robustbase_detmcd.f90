...
contains
   subroutine cov_detmcd(x, result, alpha, max_csteps, save_orderings)
      ...
      allocate(z(n,p),loc(p),sc(p),orient(p,p,6),orderings(n,6),subset(h),previous(h),idx(n),best_subset(h), &
               center(p),cov(p,p),best_center(p),best_cov(p,p),invcov(p,p),dist(n), &
               sorted_dist(n),eigvals(p),eigvecs(p,p),mask(n),result%csteps(6), &
   ...
   subroutine initial_order(z,p_matrix,ordering)
      ...
      real(dp),allocatable::projected(:,:),lambda(:),sqrtcov(:,:),sqrtinv(:,:), &
         transformed(:,:),estloc(:),score(:,:),distance(:),sorted(:)
      integer::n,p,i,j,k
      n=size(z,1);p=size(z,2)
