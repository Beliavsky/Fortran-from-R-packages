...
!   Geore D. Byrne (see references and author statement below)
!
...
& MF, IPAR
! KARLINE: CHANGED RTOL(1),ATOL(1) : was: RTOL(LRW),ATOL(LIW)!!!
! Thomas: changed (1) to (*)
...
!  XERRWD, LUNSAV, and MFLGSV handle the printing of all
...
JSV = SIGN(1,MF)
! Karline: applied changes from 941222
MFA = ABS(MF)
METH = MFA/10
MITER = MFA - 10*METH
...
GO TO 200
! Karline: correction 19981111 added
!-----------------------------------------------------------------------
! Block C.
...
500 call rprintf( &
& 'dvode -- At current T (=R1), MXSTEP (=I1) steps' // char(0))
 call rprintf( &
& '      taken on this call before reaching TOUT'  // char(0))
 call rprintfdi( &
...
call rprintf( &
& 'dvode -- At T (=R1), EWT(=I1) has become < 0 ' // char(0))
call rprintfdi( &
...
520 call rprintf( &
& 'dvode -- At T (=R1), too much accuracy requested' // char(0))
call rprintf( &
& '      for precision of machine.. see TOLSF (=R2)' // char(0))
call rprintfd2( &
...
530 call rprintf( &
& 'dvode -- At T (=R1), and step size H (=R2) the error'//char(0))
call rprintf( &
& '      test failed repeatedly or with abs(H) = HMIN' //char(0))
call rprintfd2( &
...
540 call rprintf( &
& 'dvode -- At T (=R1), and step size H (=R2) the' // char(0))
call rprintf( &
& '      corrector converged failed repeatedly' // char(0))
call rprintf( &
& '      or with abs(H) = HMIN  ' // char(0))
 call rprintfd2( &
& '      with: R1= %g, R2 = %g' // char(0), TN, H)
ISTATE = -5
...
!-----------------------------------------------------------------------
601 call rprintfi1( &
& 'dvode -- ISTATE (=I1) illegal %i' // char(0),  ISTATE)
IF (ISTATE .LT. 0) GO TO 800
GO TO 700
602 call rprintfi1( &
& 'dvode -- ITASK (=I1) illegal %i' // char(0), ITASK)
GO TO 700
603 call rprintfi1( &
& 'dvode -- ISTATE (=I1) >1 but dvode not initialised %i' &
& // char(0), ISTATE)
GO TO 700
604 call rprintfi1( &
& 'dvode -- NEQ (=I1) <1 %i' // char(0), NEQ)
GO TO 700
605 call rprintfi2( &
& 'dvode -- ISTATE =3 and NEQ increased (I1 to I2), %i, %i' &
& // char(0), N, NEQ)
GO TO 700
606 call rprintfi1( &
& 'dvode -- ITOL (=I1) illegal %i' // char(0), ITOL)
GO TO 700
607 call rprintfi1( &
& 'dvode -- IOPT (=I1) illegal %i' // char(0), IOPT)
GO TO 700
608 call rprintfi1( &
& 'dvode -- MF (=I1) illegal %i' // char(0), MF)
GO TO 700
609 call rprintfi2( &
& 'dvode -- ML (=I1) illegal: <0 or >=neq (+I2) %i, %i' &
& // char(0), ML,NEQ)
GO TO 700
610 call rprintfi2( &
& 'dvode -- MU (=I1) illegal: <= 0 or > neq (=I2) %i, %i' &
& // char(0), MU,NEQ)
GO TO 700
611 call rprintfi1( &
& 'dvode -- MAXORD (=I1) < 0 %i' // char(0),  MAXORD)
GO TO 700
612 call rprintfi1( &
& 'dvode -- MXSTEP (=I1) < 0 %i' // char(0),  MXSTEP)
GO TO 700
613 call rprintfi1( &
& 'dvode -- MXHNIL (=I1) < 0 %i' // char(0), MXHNIL)
GO TO 700
614 call rprintfd2( &
& 'dvode -- TOUT (=R1) behind T (=R2) %g, %g' &
& // char(0), TOUT, T )
GO TO 700
615 call rprintfd1( &
& 'dvode -- HMAX (=R1) <= 0 %g' // char(0), HMAX)
GO TO 700
616 call rprintfd1( &
& 'dvode -- HMIN (=R1) <=0 %g' // char(0), HMIN)
GO TO 700
617 CONTINUE
call rprintfi2( &
...
GO TO 700
619 call rprintfid( &
& 'dvode -- RTOL(I1) is R1 < 0 %i, %g' // char(0), I, RTOLI)
GO TO 700
620 call rprintfid( &
& 'dvode -- ATOL (I1) is R1 < 0 %i, %g' // char(0), I, ATOLI )
GO TO 700
621 EWTI = RWORK(LEWT+I-1)
call rprintfid( &
& 'dvode -- EWT (I1) is R1 <= 0  %i, %g' // char(0), I, EWTI)
GO TO 700
622 CONTINUE
call rprintfd2( &
...
623 CONTINUE
call rprintfi1( &
& 'dvode -- ITASK = I1 %i', ITASK)
call rprintfd2( &
& 'and TOUT (=R1) behind TCUR-HU (=R2) %g, %g' &
& // char(0), TOUT, TP)
GO TO 700
624 CONTINUE
 call rprintfd2( &
...
GO TO 700
626 call rprintf( &
& 'dvode -- at start of problem, too much accuracy' // char(0))
 call rprintfd1( &
...
GO TO 700
627 call rprintfid( &
& 'dvode -- trouble from DVINDY. ITASK = I1, TOUT = R1 %i, %g' &
& // char(0), ITASK, TOUT)
...
!
800 call rprintf( &
& 'dvode -- run aborted.. apparent infinite loop' // char(0))
RETURN
...
!
80 call rprinti1( &
& 'dvode -- DVINDY -- K (=I1) illegal ',  K)
IFLAG = -1
RETURN
90 call rprintd1( &
& 'dvode -- DVINDY -- T (=R1) illegal ',  T)
    call rprintd2( &
& 'dvode -- T not in interval TCUR-HU (=R1) to TCUR (=R2)       ', &
& TP,TN)
IFLAG = -2
RETURN
...
IF (MAXORD .LT. NQ-1) THEN
  ...
  ENDIF
!KS: value ETAQ used before its value defined
IF (MAXORD .EQ. NQ .AND. NEWQ .EQ. NQ+1) ETA = ETAQ
IF (MAXORD .EQ. NQ-1 .AND. NEWQ .EQ. NQ+1) THEN
!KS: value ETAMQ1 used before its value defined
  ETA = ETAQM1
  CALL DVJUST (YH, LDYH, -1)
  ENDIF
