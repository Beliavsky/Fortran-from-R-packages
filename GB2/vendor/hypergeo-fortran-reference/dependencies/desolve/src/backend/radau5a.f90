!  Original author: Ernst Hairer, see copyright statement in radau5.f
!  Adapted for use in R package deSolve by the deSolve authors.
!

!-----------------------------------------------------------------------
!     additional linear algebra routines required by RADAU5
!-----------------------------------------------------------------------

! KS: changed sol -> solradau , ... 
! KS: write statements rewritten
! ******************************************
...
!
 ...
 ELSE IF (IJOB .EQ. 12 .OR. IJOB .EQ. 14) THEN
   GOTO 12
 ENDIF
!  karline: the above was changed from this computed goto
...
!           SINGULAR AT STAGE K.
!  USE  solradau  TO OBTAIN SOLUTION OF LINEAR SYSTEM.
!  DETERM(A) = IP(N)*A(1,1)*A(2,2)*...*A(N,N).
!  IF IP(N)=O, A IS SINGULAR, solradau WILL DIVIDE BY ZERO.
!
!  REFERENCE..
...
!    N = ORDER OF MATRIX.
!    NDIM = DECLARED DIMENSION OF ARRAY  A .
!    A = TRIANGULARIZED MATRIX OBTAINED FROM DECradau.
!    B = RIGHT HAND SIDE VECTOR.
!    IP = PIVOT VECTOR OBTAINED FROM DECradau.
!  DO NOT USE IF DECradau HAS SET IER .NE. 0.
!  OUTPUT..
!    B = SOLUTION VECTOR, X .
!-----------------------------------------------------------------------
IF (N .EQ. 1) GO TO 50
...
!    N = ORDER OF MATRIX.
!    NDIM = DECLARED DIMENSION OF ARRAYS  AR AND AI.
!    (AR,AI) = TRIANGULARIZED MATRIX OBTAINED FROM DECradau.
...
!     IER     = 0 IF MATRIX A IS NONSINGULAR, OR  = K IF FOUND TO BE
!                SINGULAR AT STAGE K.
!  USE  SOLradB  TO OBTAIN SOLUTION OF LINEAR SYSTEM.
!  DETERM(A) = IP(N)*A(MD,1)*A(MD,2)*...*A(MD,N)  WITH MD=ML+MU+1.
!  IF IP(N)=O, A IS SINGULAR, SOLradB WILL DIVIDE BY ZERO.
!
!  REFERENCE..
...
IP(N) = 0
RETURN
!----------------------- END OF SUBROUTINE DECradB ------------------------
END
!
!
SUBROUTINE SOLradB (N, NDIM, A, ML, MU, B, IP)
REAL(KIND=KIND(0.0d0)) A,B,T
DIMENSION A(NDIM,N), B(N), IP(N)
!-----------------------------------------------------------------------
!  SOLUTION OF LINEAR SYSTEM, A*X = B .
!  INPUT..
!    N      ORDER OF MATRIX A.
!    NDIM   DECLARED DIMENSION OF ARRAY  A .
!    A      TRIANGULARIZED MATRIX OBTAINED FROM DECradB.
!    ML     LOWER BANDWIDTH OF A (DIAGONAL IS NOT COUNTED).
!    MU     UPPER BANDWIDTH OF A (DIAGONAL IS NOT COUNTED).
!    B      RIGHT HAND SIDE VECTOR.
!    IP     PIVOT VECTOR OBTAINED FROM DECradB.
!  DO NOT USE IF DECradB HAS SET IER .NE. 0.
!  OUTPUT..
!    B      SOLUTION VECTOR, X .
!-----------------------------------------------------------------------
MD = ML + MU + 1
...
!    N      ORDER OF MATRIX A.
!    NDIM   DECLARED DIMENSION OF ARRAY  A .
!    AR, AI TRIANGULARIZED MATRIX OBTAINED FROM DECradB (REAL AND IMAG. PART).
!    ML     LOWER BANDWIDTH OF A (DIAGONAL IS NOT COUNTED).
!    MU     UPPER BANDWIDTH OF A (DIAGONAL IS NOT COUNTED).
...
real(kind=kind(0.0d0)) a(nm,n)
real(kind=kind(0.0d0)) x,y
real(kind=kind(0.0d0)) dabs
integer int(igh)
!
