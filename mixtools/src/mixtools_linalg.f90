...
module mixtools_linalg
  use, intrinsic :: ieee_arithmetic, only : ieee_is_finite
  use mixtools_kinds, only : dp
  use mixtools_status, only : MIXTOOLS_SUCCESS, MIXTOOLS_SINGULAR, MIXTOOLS_NOT_POSITIVE_DEFINITE, MIXTOOLS_DIMENSION_ERROR
  implicit none
  private
...
contains
  ...
  subroutine cholesky_lower(a, l, status, jitter)
    ...
    if (size(a,2) /= n) then
      allocate(l(0,0))
      status = MIXTOOLS_DIMENSION_ERROR
      return
    end if
    ...
    do attempt = 0, 7
      ...
      do j = 1, n
        ...
        if (.not. ieee_is_finite(s) .or. s <= epsilon(1.0_dp) * max(1.0_dp, abs(work(j,j)))) then
          status = MIXTOOLS_NOT_POSITIVE_DEFINITE
          exit
        end if
        l(j,j) = sqrt(s)
        do i = j + 1, n
  ...
  subroutine solve_lower(l, b, x, status)
    ...
    if (size(l,2) /= n .or. size(b) /= n) then
      allocate(x(0)); status = MIXTOOLS_DIMENSION_ERROR; return
    end if
    allocate(x(n)); x = 0.0_dp
    do i = 1, n
      if (abs(l(i,i)) <= tiny(1.0_dp)) then
        status = MIXTOOLS_SINGULAR; return
      end if
      s = b(i)
      do j = 1, i - 1
        s = s - l(i,j) * x(j)
  ...
  subroutine solve_spd_vector(a, b, x, status, logdet)
    ...
    if (status /= MIXTOOLS_SUCCESS) then
      allocate(x(0)); if (present(logdet)) logdet = huge(1.0_dp); return
    end if
  ...
  subroutine solve_spd_matrix(a, b, x, status, logdet)
    ...
    call cholesky_lower(a, l, status)
    if (status /= MIXTOOLS_SUCCESS) then
      allocate(x(0,0)); if (present(logdet)) logdet = huge(1.0_dp); return
    end if
  ...
  subroutine inverse_spd(a, ainv, status, logdet)
    ...
    n = size(a,1)
    if (size(a,2) /= n) then
      allocate(ainv(0,0)); status = MIXTOOLS_DIMENSION_ERROR; return
    end if
  ...
  subroutine general_inverse(a, ainv, status)
    ...
    do k = 1, n
      ...
      if (abs(aug(imax,k)) <= epsilon(1.0_dp) * max(1.0_dp, maxval(abs(a)))) then
        allocate(ainv(0,0)); status = MIXTOOLS_SINGULAR; return
      end if
      if (imax /= k) then
        temp = aug(k,:); aug(k,:) = aug(imax,:); aug(imax,:) = temp
      end if
    ...
    allocate(ainv(n,n)); ainv = aug(:,n+1:2*n); status = MIXTOOLS_SUCCESS
  ...
  subroutine mean_columns(x, mu, status)
    ...
    integer, intent(out) :: status
    if (size(x,1) < 1) then
      allocate(mu(0)); status = MIXTOOLS_DIMENSION_ERROR; return
    end if
    allocate(mu(size(x,2))); mu = sum(x,dim=1) / real(size(x,1),dp)
    status = MIXTOOLS_SUCCESS
  ...
  subroutine covariance_matrix(x, cov, status, center)
    ...
    n = size(x,1)
    if (n < 2) then
      allocate(cov(0,0)); status = MIXTOOLS_DIMENSION_ERROR; return
    end if
    do_center = .true.; if (present(center)) do_center = center
    allocate(xc(size(x,1),size(x,2)))
    if (do_center) then
      call mean_columns(x, mu, status); if (status /= MIXTOOLS_SUCCESS) return
    ...
    cov = symmetrize(cov); status = MIXTOOLS_SUCCESS
  ...
  subroutine unique_integers(values, unique, status)
    ...
    if (size(values) == 0) then
      allocate(unique(0)); status = MIXTOOLS_SUCCESS; return
    end if
    allocate(work(size(values))); n = 0
    do i = 1, size(values)
      seen = .false.
      do j = 1, n
      ...
      if (.not. seen) then
        n = n + 1; work(n) = values(i)
      end if
    end do
    allocate(unique(n)); unique = work(:n); status = MIXTOOLS_SUCCESS
  ...
  subroutine find_group_indices(group, level, indices)
    ...
    integer :: i, n
    n = count(group == level); allocate(indices(n)); n = 0
    do i = 1, size(group)
  ...
  subroutine solve_least_squares(x, y, beta, covariance, rss, status, weights)
    ...
    if (size(y) /= n .or. n < p) then
      allocate(beta(0),covariance(0,0)); rss = huge(1.0_dp); status = MIXTOOLS_DIMENSION_ERROR; return
    end if
    allocate(xw(n,p),yw(n)); xw = x; yw = y
    if (present(weights)) then
      if (size(weights) /= n .or. any(weights < 0.0_dp)) then
        allocate(beta(0),covariance(0,0)); rss = huge(1.0_dp); status = MIXTOOLS_DIMENSION_ERROR; return
      end if
      do i = 1, n
        xw(i,:) = sqrt(weights(i)) * x(i,:); yw(i) = sqrt(weights(i)) * y(i)
      end do
    end if
    xtx = matmul(transpose(xw),xw); xty = matmul(transpose(xw),yw)
    call solve_spd(xtx,xty,beta,status)
    if (status /= MIXTOOLS_SUCCESS) then
      allocate(covariance(0,0)); rss = huge(1.0_dp); return
    end if
    ...
    allocate(covariance(p,p)); covariance = inv
  end subroutine solve_least_squares
end module mixtools_linalg
