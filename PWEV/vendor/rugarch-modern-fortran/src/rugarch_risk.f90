module rugarch_risk
   ...
   use rugarch_distributions, only : distribution_quantile
   implicit none
   private
...
contains
   ...
   function value_at_risk(probability, location, scale, kind, shape, skew, lambda) result(value)
      real(dp), intent(in) :: probability, location, scale, shape, skew
      real(dp), intent(in), optional :: lambda
      integer, intent(in) :: kind
      real(dp) :: value

      value = location+scale*distribution_quantile(probability,kind,shape,skew,lambda)
   end function value_at_risk

   function expected_shortfall(probability, location, scale, kind, shape, skew, lambda) result(value)
      real(dp), intent(in) :: probability, location, scale, shape, skew
      real(dp), intent(in), optional :: lambda
      integer, intent(in) :: kind
      ...
      do i = 0, ngrid
         ...
         sumq = sumq+real(weight,dp)*distribution_quantile(p,kind,shape,skew,lambda)
      end do
