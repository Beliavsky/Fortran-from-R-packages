...
program test_unified
   ...
   asymmetry = maxval(abs(ism%covariance-transpose(ism%covariance)))
   if (asymmetry > 0.0_dp) then
      print '(a,es14.6)', 'ism_vcov maximum asymmetry: ', asymmetry
      error stop 1
   end if
