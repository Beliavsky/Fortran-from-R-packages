program test_criteria
  ...
  o%kappa=0.2_dp; o%delta=0.01_dp
  do i=1,size(names)
    call evaluate_criterion(trim(names(i)),l,r,o)
    if(r%info/=0 .or. .not.(abs(r%f)<huge(1.0_dp))) then
      print *, 'criterion failure: ',trim(names(i)),r%info,r%f; fails=fails+1
    end if
  end do
...
contains
  subroutine gradient_check(name,x,o,tol,fails)
    ...
    integer::i,j
    h=1.0e-6_dp; call evaluate_criterion(name,x,r,o); err=0.0_dp
    do j=1,size(x,2); do i=1,size(x,1)
      xp=x; xp(i,j)=xp(i,j)+h; call evaluate_criterion(name,xp,rp,o)
      xp=x; xp(i,j)=xp(i,j)-h; call evaluate_criterion(name,xp,rm,o)
      num=(rp%f-rm%f)/(2.0_dp*h); err=max(err,abs(num-r%gq(i,j)))
    end do; end do
    if(err>tol) then; print *, 'gradient failure ',trim(name),err; fails=fails+1; end if
