 ...
 if( fuzz < max(abs(fa.none$loadings - tst))) {
    ...
    print(unclass(fa.none$loadings), digits=18)
    cat("difference:\n")
    print(unclass(fa.none$loadings - tst), digits=18)
    all.ok <- FALSE  
    } 
...
# with eps=1e-5
 ...
 if( fuzz < max(abs(fa.varimax$loadings - tst))) {
    ...
    print(unclass(fa.varimax$loadings), digits=18)
    cat("difference:\n")
    print(unclass(fa.varimax$loadings - tst), digits=18)
    all.ok <- FALSE  
    } 
...
# with eps=1e-5
 ...
 if( fuzz < max(abs(fa.oblimin$loadings - tst))) {
    ...
    print(unclass(fa.oblimin$loadings), digits=18) 
    cat("difference:\n")
    print(unclass(fa.oblimin$loadings - tst), digits=18)
    all.ok <- FALSE  
    } 
