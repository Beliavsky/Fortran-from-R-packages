...
contains
  ...
  subroutine gpfrsorth(a,method,nstarts,r,crit,opts)
    real(dp),intent(in)::a(:,:); character(len=*),intent(in)::method; integer,intent(in)::nstarts
    type(rotation_result),intent(out)::r; type(criterion_options),intent(in),optional::crit
  ...
  subroutine random_start(k,t,info)
    integer,intent(in)::k; real(dp),intent(out)::t(k,k); integer,intent(out)::info
    call random_orthogonal(k,t,info)
  end subroutine random_start

  subroutine oblimin(a,r,gam,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r
    real(dp),intent(in),optional::gam; type(rotation_options),intent(in),optional::opts
  ...
  subroutine quartimin(a,r,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    call gpfoblq(a,'quartimin',r,c,opts)
  end subroutine quartimin

  subroutine target_t(a,target,r,opts)
    real(dp),intent(in)::a(:,:),target(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    c%target=target; call gpforth(a,'target',r,c,opts)
  end subroutine target_t

  subroutine target_q(a,target,r,opts)
    real(dp),intent(in)::a(:,:),target(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    c%target=target; call gpfoblq(a,'target',r,c,opts)
  end subroutine target_q

  subroutine pst_t(a,w,target,r,opts)
    real(dp),intent(in)::a(:,:),w(:,:),target(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    c%weight=w; c%target=target; call gpforth(a,'pst',r,c,opts)
  end subroutine pst_t

  subroutine pst_q(a,w,target,r,opts)
    real(dp),intent(in)::a(:,:),w(:,:),target(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    c%weight=w; c%target=target; call gpfoblq(a,'pst',r,c,opts)
  ...
  subroutine simplimax(a,r,kzero,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; integer,intent(in),optional::kzero
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
  ...
  subroutine geomin_t(a,r,delta,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; real(dp),intent(in),optional::delta
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    if(present(delta)) c%delta=delta; call gpforth(a,'geomin',r,c,opts)
  end subroutine geomin_t

  subroutine geomin_q(a,r,delta,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; real(dp),intent(in),optional::delta
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    if(present(delta)) c%delta=delta; call gpfoblq(a,'geomin',r,c,opts)
  end subroutine geomin_q

  subroutine bigeomin_t(a,r,delta,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; real(dp),intent(in),optional::delta
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    if(present(delta)) c%delta=delta; call gpforth(a,'bigeomin',r,c,opts)
  end subroutine bigeomin_t

  subroutine bigeomin_q(a,r,delta,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; real(dp),intent(in),optional::delta
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    if(present(delta)) c%delta=delta; call gpfoblq(a,'bigeomin',r,c,opts)
  end subroutine bigeomin_q

  subroutine cf_t(a,r,kappa,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; real(dp),intent(in),optional::kappa
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    if(present(kappa)) c%kappa=kappa; call gpforth(a,'cf',r,c,opts)
  end subroutine cf_t

  subroutine cf_q(a,r,kappa,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r; real(dp),intent(in),optional::kappa
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    if(present(kappa)) c%kappa=kappa; call gpfoblq(a,'cf',r,c,opts)
  end subroutine cf_q
  ...
  subroutine equamax(a,r,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    c%kappa=real(size(a,2),dp)/(2.0_dp*real(size(a,1),dp)); call gpforth(a,'cf',r,c,opts)
  end subroutine equamax

  subroutine parsimax(a,r,opts)
    real(dp),intent(in)::a(:,:); type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; type(criterion_options)::c
    c%kappa=real(size(a,2)-1,dp)/real(size(a,2)+size(a,1)-2,dp); call gpforth(a,'cf',r,c,opts)
  ...
  subroutine lp_t(a,p,r,opts)
    real(dp),intent(in)::a(:,:),p; type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; call lp_rotate(a,p,.true.,r,opts)
  end subroutine lp_t

  subroutine lp_q(a,p,r,opts)
    real(dp),intent(in)::a(:,:),p; type(rotation_result),intent(out)::r
    type(rotation_options),intent(in),optional::opts; call lp_rotate(a,p,.false.,r,opts)
  end subroutine lp_q

  subroutine eiv(a,identity_rows,r)
    real(dp),intent(in)::a(:,:); integer,intent(in)::identity_rows(:); type(rotation_result),intent(out)::r
    call eiv_rotate(a,identity_rows,r)
  end subroutine eiv

  subroutine echelon(a,reference_rows,r)
    real(dp),intent(in)::a(:,:); integer,intent(in)::reference_rows(:); type(rotation_result),intent(out)::r
