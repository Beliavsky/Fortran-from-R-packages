...
contains
  ...
  subroutine inverse_matrix(a, ainv, info)
    ...
    do k = 1, n
      ...
      if (imax /= k) then
        tmp = aug(k,:); aug(k,:) = aug(imax,:); aug(imax,:) = tmp
      end if
      aug(k,:) = aug(k,:) / aug(k,k)
      do i = 1, n
        if (i == k) cycle
  ...
  function logabsdet(a, sign_det, info) result(v)
    ...
    ierr = 0
    if (size(a,2) /= n) then
      ierr = -1; v = -huge(1.0_dp); sgn = 0.0_dp
      if (present(sign_det)) sign_det = sgn
      if (present(info)) info = ierr
      return
    end if
    allocate(m(n,n),tmp(n)); m = a; v = 0.0_dp; sgn = 1.0_dp
    do k=1,n
      imax=k; piv=abs(m(k,k))
      do i=k+1,n
        if (abs(m(i,k))>piv) then
          piv=abs(m(i,k)); imax=i
        end if
      end do
      scale=max(1.0_dp,maxval(abs(m)))
      if (piv <= sqrt(epsilon(1.0_dp))*scale) then
        ierr=k; v=-huge(1.0_dp); sgn=0.0_dp; exit
      end if
      if (imax/=k) then
        tmp=m(k,:); m(k,:)=m(imax,:); m(imax,:)=tmp; sgn=-sgn
      end if
  ...
  subroutine symmetric_eigen_jacobi(a, eval, evec, info, tol, max_sweeps)
    ...
    n=size(a,1); m=0.5_dp*(a+transpose(a)); evec=eye(n)
    eps0=1.0e-13_dp
    ...
    info=1
    do sweep=1,ms
      thresh=0.0_dp; p=1; q=min(2,n)
      do j=1,n
        if (j<n) then
          if (maxval(abs(m(j,j+1:n)))>thresh) then
            q=j+maxloc(abs(m(j,j+1:n)),dim=1); p=j; thresh=abs(m(p,q))
          end if
        end if
      end do
      ...
      do j=1,n
        if (j/=p .and. j/=q) then
          mpj=m(p,j); mqj=m(q,j)
          m(p,j)=c*mpj-s*mqj; m(j,p)=m(p,j)
          m(q,j)=s*mpj+c*mqj; m(j,q)=m(q,j)
        end if
      end do
      ...
      do j=1,n
        vjp=evec(j,p); vjq=evec(j,q)
        evec(j,p)=c*vjp-s*vjq; evec(j,q)=s*vjp+c*vjq
      end do
    end do
    do j=1,n
      eval(j)=m(j,j)
  ...
  subroutine polar_orthogonal(x, q, info)
    ...
    if (info/=0 .or. minval(eval)<=sqrt(epsilon(1.0_dp))) then
      q=0.0_dp; info=max(1,info); return
    end if
    invsqrt=0.0_dp
    do j=1,size(eval)
  ...
  contains
    pure function outer_col(a,b) result(m)
      ...
      integer :: ii,jj
      do jj=1,size(b); do ii=1,size(a); m(ii,jj)=a(ii)*b(jj); end do; end do
  ...
  subroutine random_orthogonal(k, q, info)
    ...
    do j=1,k
      q(:,j)=z(:,j)
      do i=1,j-1
        proj=dot_product(q(:,i),q(:,j)); q(:,j)=q(:,j)-proj*q(:,i)
      end do
      nrm=sqrt(sum(q(:,j)*q(:,j))); rdiag(j)=nrm
      if(nrm<=sqrt(epsilon(1.0_dp))) then; info=j; return; end if
      q(:,j)=q(:,j)/nrm
