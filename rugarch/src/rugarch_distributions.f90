...
module rugarch_distributions
   use rugarch_kinds, only : dp
   use rugarch_math, only : pi, normal_pdf, normal_cdf, normal_quantile, &
      student_t_pdf, student_t_cdf, student_t_quantile, regularized_gamma_p, &
      inverse_regularized_gamma_p, beta_fn
   use rugarch_rng, only : random_normal, random_gamma, random_student_t
   use rugarch_gh, only : dsnig, psnig, qsnig, rsnig, dsgh, psgh, qsgh, rsgh, &
      dsghst, psghst, qsghst, rsghst
   implicit none
   private
   ...
   integer, parameter, public :: dist_jsu = 40
   integer, parameter, public :: dist_nig = 50
   integer, parameter, public :: dist_ghyp = 51
   integer, parameter, public :: dist_ghst = 52
...
contains
   ...
   pure elemental subroutine jsu_standardization(skew, shape, center, scale)
      real(dp), intent(in) :: skew, shape
      real(dp), intent(out) :: center, scale
      real(dp) :: rtau, w, omega

      rtau = 1.0_dp/max(shape,1.0e-8_dp)
      if (rtau < 1.0e-7_dp) then
         w = 1.0_dp
      else
         w = exp(min(100.0_dp,rtau*rtau))
      end if
      omega = -skew*rtau
      scale = sqrt(1.0_dp/max(1.0e-30_dp,0.5_dp*(w-1.0_dp)*(w*cosh(2.0_dp*omega)+1.0_dp)))
      center = scale*sqrt(w)*sinh(omega)
   end subroutine jsu_standardization

   pure elemental function djsu(x, mean, sd, skew, shape, log_density) result(value)
      real(dp), intent(in) :: x, mean, sd, skew, shape
      logical, intent(in), optional :: log_density
      real(dp) :: value, z, r, center, scale, rtau
      logical :: want_log

      want_log = .false.
      if (present(log_density)) want_log = log_density
      if (sd <= 0.0_dp .or. shape <= 0.0_dp) then
         value = merge(-huge(1.0_dp),0.0_dp,want_log)
         return
      end if
      call jsu_standardization(skew,shape,center,scale)
      rtau = 1.0_dp/shape
      z = ((x-mean)/sd-center)/scale
      r = -skew+asinh(z)/rtau
      value = -log(sd)-log(scale)-log(rtau)-0.5_dp*log(z*z+1.0_dp)-0.5_dp*log(2.0_dp*pi)-0.5_dp*r*r
      if (.not. want_log) value = exp(value)
   end function djsu

   pure elemental function pjsu(q, mean, sd, skew, shape) result(value)
      ...
      call jsu_standardization(skew,shape,center,scale)
      rtau = 1.0_dp/shape
      z = ((q-mean)/sd-center)/scale
      r = -skew+asinh(z)/rtau
      value = normal_cdf(r)
   end function pjsu

   pure elemental function qjsu(p, mean, sd, skew, shape) result(value)
      real(dp), intent(in) :: p, mean, sd, skew, shape
      real(dp) :: value, center, scale, rtau, r

      call jsu_standardization(skew,shape,center,scale)
      rtau = 1.0_dp/max(shape,1.0e-8_dp)
      r = normal_quantile(p)
      value = mean+sd*(center+scale*sinh(rtau*(r+skew)))
   end function qjsu

   function rjsu(mean, sd, skew, shape) result(value)
      real(dp), intent(in) :: mean, sd, skew, shape
      real(dp) :: value, u

      call random_number(u)
      value = qjsu(max(1.0e-12_dp,min(1.0_dp-1.0e-12_dp,u)),mean,sd,skew,shape)
   end function rjsu

   function distribution_pdf(x, kind, shape, skew, lambda) result(value)
      real(dp), intent(in) :: x, shape, skew
      integer, intent(in) :: kind
      real(dp), intent(in), optional :: lambda
      real(dp) :: value, glambda

      glambda = 1.0_dp
      if (present(lambda)) glambda=lambda
      select case (kind)
      case (dist_norm)
         value = normal_pdf(x)
      case (dist_snorm)
      ...
      case (dist_jsu)
         value = djsu(x,0.0_dp,1.0_dp,skew,shape)
      case (dist_nig)
         value = dsnig(x,0.0_dp,1.0_dp,skew,shape)
      case (dist_ghyp)
         value = dsgh(x,0.0_dp,1.0_dp,skew,shape,glambda)
      case (dist_ghst)
         value = dsghst(x,0.0_dp,1.0_dp,skew,shape)
      case default
         value = normal_pdf(x)
      end select
   end function distribution_pdf

   function distribution_cdf(x, kind, shape, skew, lambda) result(value)
      real(dp), intent(in) :: x, shape, skew
      integer, intent(in) :: kind
      real(dp), intent(in), optional :: lambda
      real(dp) :: value, glambda
      ...
      case (dist_jsu)
         value = pjsu(x,0.0_dp,1.0_dp,skew,shape)
      case (dist_nig)
         value = psnig(x,0.0_dp,1.0_dp,skew,shape)
      case (dist_ghyp)
         value = psgh(x,0.0_dp,1.0_dp,skew,shape,glambda)
      case (dist_ghst)
         value = psghst(x,0.0_dp,1.0_dp,skew,shape)
      case default
         value = normal_cdf(x)
      end select
   end function distribution_cdf

   function distribution_quantile(p, kind, shape, skew, lambda) result(value)
      ...
      case (dist_jsu)
         value = qjsu(p,0.0_dp,1.0_dp,skew,shape)
      case (dist_nig)
         value = qsnig(p,0.0_dp,1.0_dp,skew,shape)
      case (dist_ghyp)
         value = qsgh(p,0.0_dp,1.0_dp,skew,shape,glambda)
      case (dist_ghst)
         value = qsghst(p,0.0_dp,1.0_dp,skew,shape)
      case default
         value = normal_quantile(p)
      end select
   end function distribution_quantile

   function random_innovation(kind, shape, skew, lambda) result(value)
      integer, intent(in) :: kind
      ...
      case (dist_jsu)
         value = rjsu(0.0_dp,1.0_dp,skew,shape)
      case (dist_nig)
         value = rsnig(0.0_dp,1.0_dp,skew,shape)
      case (dist_ghyp)
         value = rsgh(0.0_dp,1.0_dp,skew,shape,glambda)
      case (dist_ghst)
         value = rsghst(0.0_dp,1.0_dp,skew,shape)
      case default
         value = random_normal()
      end select
   end function random_innovation

   function absolute_moment(order, kind, shape, skew, lambda) result(value)
      real(dp), intent(in) :: order, shape
      integer, intent(in) :: kind
      real(dp), intent(in), optional :: skew, lambda
      real(dp) :: value, sk, glambda, u, x, h, pdf
      integer :: i, n, weight
      ...
      case default
         n=2000
         h=(1.0_dp-2.0e-9_dp)/real(n,dp)
         value=0.0_dp
         do i=0,n
            u=1.0e-9_dp+real(i,dp)*h
            x=tan(pi*(u-0.5_dp))
            pdf=distribution_pdf(x,kind,shape,sk,glambda)*pi*(1.0_dp+x*x)*abs(x)**order
            if(i==0 .or. i==n)then
               weight=1
            ...
            value=value+real(weight,dp)*pdf
         end do
   ...
   pure function distribution_name(kind) result(name)
      ...
      case (dist_jsu);   name = 'jsu'
      case (dist_nig);   name = 'nig'
      case (dist_ghyp);  name = 'ghyp'
      case (dist_ghst);  name = 'ghst'
      case default;      name = 'unknown'
      end select
   end function distribution_name

end module rugarch_distributions
