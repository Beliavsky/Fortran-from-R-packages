...
contains
  ...
  subroutine gpforth(a, method, result, crit_opts, rot_opts, tinit)
    ...
    do iter=0,o%maxit
      m=matmul(transpose(t),g)
      sym=0.5_dp*(m+transpose(m))
      gp=g-matmul(t,sym)
      s=frobenius_norm(gp)
      ...
      j0=max(1,iter+2-max(1,o%fwindow))
      targetf=maxval(hist(j0:iter+1,2))
      do ii=0,10
    ...
    result%iterations=min(iter,o%maxit)
    result%converged=(s<o%eps)
    result%th=t
    result%phi=eye(k)
    result%objective=cr%f
    result%gq=cr%gq
    result%method=cr%method
    result%table=hist(1:nused,:)
    result%info=merge(0,1,result%converged)
    l=matmul(b,t)
    do ii=1,size(a,1)
    result%loadings(ii,:)=l(ii,:)*wrow(ii)
    end do
  ...
  subroutine gpfoblq(a, method, result, crit_opts, rot_opts, tinit)
    ...
    if(present(crit_opts)) co=crit_opts
    if(trim(lower(o%algorithm))=='cayley') then
      call init_rotation_result(result,a,size(a,2),.false.)
      result%info=-3
      return
    end if
    k=size(a,2)
    call init_rotation_result(result,a,k,.false.)
    if(k<2) then
    result%info=-1
    return
    end if
    ...
    call inverse_matrix(t,ti,info)
    if(info/=0) then
    result%info=info
    return
    end if
    l=matmul(b,transpose(ti))
    call evaluate_criterion(method,l,cr,co)
    if(cr%info/=0) then
    result%info=cr%info
    return
    ...
    result%converged=(s<o%eps)
    result%th=t
    result%phi=matmul(transpose(t),t)
    result%objective=cr%f
    result%gq=cr%gq
    result%method=cr%method
    result%table=hist(1:nused,:)
  ...
  subroutine lp_rotate(a,p,orthogonal,result,rot_opts,tinit)
    ...
    if(present(rot_opts)) o=rot_opts
    k=size(a,2)
    allocate(t(k,k),lprev(size(a,1),k),w(size(a,1),k))
    if(present(tinit)) then
    t=tinit
    else
    t=eye(k)
    end if
    epslp=max(o%eps,1.0e-10_dp)
    lprev=huge(1.0_dp)
    inner_o=o
    inner_o%maxit=5
    inner_o%random_starts=0
    do it=1,o%maxit
      if(it==1) then
        if(orthogonal) then
  ...
  contains
    subroutine oblique_loadings(x,tt,ll)
      ...
      real(dp)::invtt(size(tt,1),size(tt,2))
      integer::ierr
      call inverse_matrix(tt,invtt,ierr)
      if(ierr==0) then
      ll=matmul(x,transpose(invtt))
      else
      ll=0.0_dp
      end if
  ...
  subroutine normalizing_weights(a,mode,w)
    ...
    m=size(a,2)
    acosi=acos(real(m,dp)**(-0.5_dp))
    do i=1,size(a,1)
      h=max(sqrt(sum(a(i,:)*a(i,:))),epsilon(1.0_dp))
      if(mode==1) then
        w(i)=h
      else
        fpls=max(-1.0_dp,min(1.0_dp,a(i,1)/h))
        alpha=acos(abs(fpls))
        if(abs(fpls)<real(m,dp)**(-0.5_dp)) then
        dem=acosi-0.5_dp*acos(-1.0_dp)
        else
        dem=acosi
        end if
        num=acosi-alpha
        if(abs(dem)<=epsilon(1.0_dp)) then
        wt=0.001_dp
        else
        wt=cos((num/dem)*0.5_dp*acos(-1.0_dp))**2+0.001_dp
        end if
        w(i)=h/wt
      end if
    end do
