...
contains
  ...
  subroutine evaluate_criterion(name,l,r,opts)
    ...
    case('target')
      if(.not.allocated(o%target)) then
    ...
    case('lp.wls','lp_wls')
      if(.not.allocated(o%weight)) then
      call init_result(l,r)
      r%info=-2
      return
      end if
  ...
  subroutine vgq_oblimin(l,gam,r)
    ...
    if(abs(gam)>tiny(1.0_dp)) then
      cs=sum(x,dim=1)
      do j=1,size(l,2)
      x(:,j)=x(:,j)-(gam/real(size(l,1),dp))*cs(j)
      end do
    end if
    r%gq=l*x
    r%f=sum(l*l*x)/4.0_dp
    r%method='Oblimin'
  ...
  subroutine vgq_target(l,target,r)
    ...
    real(dp)::d
    call init_result(l,r)
    r%method='Target rotation'
    if(any(shape(l)/=shape(target))) then
    r%info=-2
    return
    end if
    do j=1,size(l,2)
  ...
  subroutine vgq_pst(l,w,target,r)
    ...
    call init_result(l,r)
    r%method='Partially specified target'
    if(any(shape(l)/=shape(w)) .or. any(shape(l)/=shape(target))) then
    r%info=-2
    return
    end if
    d=w*l-w*target
  ...
  subroutine vgq_entropy(l,r)
    ...
    call init_result(l,r)
    l2=l*l
    lg=log(l2+epsilon(1.0_dp))
    r%gq=-(l*lg+l)
    r%f=-sum(l2*lg)/2.0_dp
    r%method='Minimum entropy'
  ...
  subroutine vgq_infomax(l,r)
    ...
    a0=sum(s2*h)/ss
    a1=sum(sum(s2,dim=2)*h1)/ss
    a2=sum(sum(s2,dim=1)*h2)/ss
    g0=(h-a0)/ss
    do j=1,size(l,2)
    do i=1,size(l,1)
      g1(i,j)=(h1(i)-a1)/ss
    ...
    r%gq=2.0_dp*l*(g0-g1-g2)
    r%f=log(real(size(l,2),dp))+q0-q1-q2
    r%method='Infomax'
  ...
  subroutine vgq_mccammon(l,r)
    ...
    cs=sum(s,dim=1)
    ts=sum(s)
    if(minval(cs)<=0.0_dp .or. ts<=0.0_dp) then
    r%info=1
    return
    end if
    do j=1,size(l,2)
    ...
    r%gq=2.0_dp*l*(g1/q1-g2/q2)
    r%f=log(q1)-log(q2)
    r%method='McCammon entropy'
  ...
  subroutine vgq_geomin(l,delta,r)
    ...
    l2=l*l+delta
    do i=1,size(l,1)
    pro(i)=exp(sum(log(l2(i,:)))/real(k,dp))
    end do
    do j=1,k
    r%gq(:,j)=(2.0_dp/real(k,dp))*(l(:,j)/l2(:,j))*pro
    end do
    r%f=sum(pro)
    r%method='Geomin'
  ...
  subroutine vgq_simplimax(l,kin,r)
    ...
    n=size(l)
    k=kin
    if(k<0) k=size(l,1)*(size(l,2)-1)
    k=max(0,min(k,n))
    allocate(vals(n),pick(n))
    vals=reshape(l*l,[n])
    pick=.false.
    do j=1,k
      idx=minloc(vals,dim=1,mask=.not.pick)
      pick(idx)=.true.
    end do
    r%gq=reshape(merge(2.0_dp*reshape(l,[n]),0.0_dp,pick),shape(l))
    r%f=sum(vals,mask=pick)
    r%method='Simplimax'
  ...
  subroutine vgq_bifactor(l,r)
    ...
    integer::j
    call init_result(l,r)
    r%method='Bifactor Biquartimin'
    if(size(l,2)<2) then
    r%info=-1
    return
    end if
    lt2=l(:,2:)**2
    rows=sum(lt2(:,1:size(l,2)-1),dim=2)
    do j=2,size(l,2)
    r%gq(:,j)=4.0_dp*l(:,j)*(rows-l(:,j)**2)
    end do
  ...
  subroutine vgq_tandem1(l,r)
    ...
    ll2=ll*ll
    mp=matmul(ll2,l2)
    tc=matmul(l2,transpose(l2))
    r%f=-sum(l2*mp)
    r%gq=-(4.0_dp*l*mp+4.0_dp*matmul(ll*tc,l))
    r%method='Tandem I'
  ...
  subroutine vgq_oblimax(l,r)
    ...
    s2=sum(l*l)
    s4=sum(l**4)
    if(s2<=0.0_dp .or. s4<=0.0_dp) then
    r%info=1
    return
    end if
    r%gq=-(4.0_dp*l**3/s4-4.0_dp*l/s2)
    r%f=-(log(s4)-2.0_dp*log(s2))
    r%method='Oblimax'
  ...
  subroutine vgq_bentler(l,r)
    ...
    call init_result(l,r)
    l2=l*l
    m=matmul(transpose(l2),l2)
    d=0.0_dp
    do j=1,size(l,2)
    d(j)=m(j,j)
    end do
    if(minval(d)<=0.0_dp) then
    r%info=1
    return
    end if
    ldm=logabsdet(m,info=info)
    if(info/=0) then
    r%info=info
    return
    end if
    ldd=sum(log(d))
    call inverse_matrix(m,mi,info)
    if(info/=0) then
    r%info=info
    return
    end if
  ...
  subroutine vgq_varimax(l,r)
    ...
    call init_result(l,r)
    do j=1,size(l,2)
    cm=sum(l(:,j)**2)/real(size(l,1),dp)
    ql(:,j)=l(:,j)**2-cm
    end do
    r%gq=-l*ql
    r%f=-sum(ql*ql)/4.0_dp
    r%method='Varimax'
  ...
  subroutine vgq_binormamin(l,r)
    ...
    l2=l*l
    u=sum(l2,dim=1)
    if(minval(u)<=0.0_dp) then
    r%info=1
    return
    end if
    invu=1.0_dp/u
    nmat=matmul(transpose(l2),l2)
    do j=1,m
    do i=1,m
    c(i,j)=nmat(i,j)/(u(i)*u(j))
    end do
    end do
    ...
    sumrow=sum(l2u,dim=2)
    do j=1,m
    t1(:,j)=(sumrow-l2u(:,j))*invu(j)
    noveru(:,j)=nmat(:,j)*invu(j)
    end do
    do i=1,m
    sumk(i)=sum(noveru(i,:))-noveru(i,i)
    t2(i)=sumk(i)*invu(i)**2
    end do
    do j=1,m
    r%gq(:,j)=4.0_dp*l(:,j)*(t1(:,j)-t2(j))
    end do
    r%method='Binormamin'
  ...
  subroutine vgq_lp_wls(l,w,r)
    ...
    type(criterion_result),intent(out)::r
    call init_result(l,r)
    r%method='Weighted least squares for Lp rotation'
    if(any(shape(l)/=shape(w))) then
    r%info=-2
    return
    end if
    r%gq=2.0_dp*w*l/real(size(l,1),dp)
    r%f=sum(w*l*l)/real(size(l,1),dp)
