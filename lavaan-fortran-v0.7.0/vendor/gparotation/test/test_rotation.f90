program test_rotation
  ...
  if(.not.ro%converged) then
  print *,'varimax did not converge',ro%table(size(ro%table,1),3)
  fails=fails+1
  end if
  if(ro%objective>c0%f+1.0e-9_dp) then
  print *,'varimax objective did not improve'
  fails=fails+1
  end if
  if(maxval(abs(matmul(transpose(ro%th),ro%th)-eye(3)))>2.0e-6_dp) then
    print *,'orthogonality failure'
    fails=fails+1
  end if
  rec=matmul(a,ro%th)
  if(maxval(abs(rec-ro%loadings))>1.0e-8_dp) then
  print *,'orth reconstruction failure'
  fails=fails+1
  end if
  call evaluate_criterion('quartimin',a,c0,co)
  call gpfoblq(a,'quartimin',rq,co,opt)
  if(rq%objective>c0%f+1.0e-8_dp) then
  print *,'quartimin objective did not improve'
  fails=fails+1
  end if
  rec=matmul(rq%loadings,transpose(rq%th))
  if(maxval(abs(rec-a))>2.0e-6_dp) then
  print *,'oblique reconstruction failure',maxval(abs(rec-a))
  fails=fails+1
  end if
  if(maxval(abs([(rq%phi(info,info)-1.0_dp,info=1,3)]))>2.0e-6_dp) then
    print *,'oblique phi diagonal failure'
    fails=fails+1
  end if
  call random_orthogonal(3,q,info)
  if(info/=0 .or. maxval(abs(matmul(transpose(q),q)-eye(3)))>2.0e-10_dp) then
    print *,'random orthogonal failure'
    fails=fails+1
  end if
  call normalizing_weights(a,1,w)
  if(minval(w)<=0.0_dp) then
  print *,'normalization failure'
  fails=fails+1
  end if
  ...
  fails=fails+1
    ...
    if(maxval(abs(rr%loadings(idx,:)-eye(3)))>1.0e-8_dp) then
    print *,'eiv identity failure'
    fails=fails+1
    end if
  end if
  call echelon_rotate(a,idx,re)
  if(re%info/=0) then
  print *,'echelon failed'
  fails=fails+1
  else
    if(abs(re%loadings(idx(1),2))+abs(re%loadings(idx(1),3))+abs(re%loadings(idx(2),3))>1.0e-8_dp) then
      print *,'echelon triangular failure'
      fails=fails+1
    end if
  end if
  opt%maxit=100
  ...
  if(.not.(rlp%objective<huge(1.0_dp))) then
  print *,'lp rotation failure'
  fails=fails+1
  end if
  phi=eye(3)
  rmat=matmul(matmul(ro%loadings,phi),transpose(ro%loadings))
  do info=1,9
  rmat(info,info)=1.0_dp
  end do
  call calc_fitstats(rmat,ro%loadings,phi,500,fs)
  if(fs%srmr>1.0e-10_dp) then
  print *,'fit stats SRMR failure',fs%srmr
  fails=fails+1
  end if
  ...
  if(minval(fsi)<-1.0e-10_dp .or. minval(auc)<0.0_dp .or. hpt<0 .or. ss%gini<0.0_dp) then
    print *,'simplicity diagnostics failure'
    fails=fails+1
  end if
