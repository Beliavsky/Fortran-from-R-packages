...
allocate(x(n,n)); x=0.0_dp
x(1,1)=1.0_dp; x(n,n)=1.0_dp
if(n>2) then
   do i=2,n-1; x(i,i)=2.0_dp; end do
end if
do i=1,n-1; x(i,i+1)=-1.0_dp; x(i+1,i)=-1.0_dp; end do
q=csr_from_dense(x)
...
ord=1; if(present(order))ord=order
an=1.0_dp; if(present(anisotropy))an=anisotropy
if(ord==1) then
   if(an<0.0_dp .or. an>2.0_dp) error stop 'precmat_igmrf_reglat: anisotropy not in [0,2]'
   a=precmat_rw1(m); b=csr_identity(n,2.0_dp-an)
   c=csr_identity(m,an); d=precmat_rw1(n)
else if(ord==2) then
   a=precmat_rw2(m); b=csr_identity(n)
   c=csr_identity(m); d=precmat_rw2(n)
else
   if(ord<1 .or. ord>=min(n,m)) error stop 'precmat_igmrf_reglat: order out of range'
   a=precmat_rwn(m,ord); b=csr_identity(n)
   c=csr_identity(m); d=precmat_rwn(n,ord)
end if
...
tol=epsilon(1.0_dp)*100.0_dp; if(present(eps))tol=eps*100.0_dp
a=adj
if(allocated(a%entries)) then
   where(abs(a%entries)>0.0_dp) a%entries=1.0_dp
end if
allocate(deg(a%nrow)); deg=0.0_dp
do i=1,a%nrow
...
modl='m1p1'; if(present(model))modl=trim(adjustl(model))
tol=0.0_dp;if(present(eps))tol=eps
allocate(x(n),y(max(n,n*m)),z(m),w(n),zero(m)); x=0;y=0;z=0;w=0;zero=0
im=csr_identity(m)
select case(trim(modl))
case('m1p1')
   if(size(par)<1) error stop 'precmat_gmrf_reglat: par too short'
   x(1:2)=[1.0_dp,-par(1)]; y(n+1)=-par(1)
   p1=csr_kronecker(im,csr_toeplitz(x,eps=tol)); p2=csr_toeplitz(y(:n*m),eps=tol)
   q=csr_add(p1,p2,tol)
case('m1p2')
   if(size(par)<2) error stop 'precmat_gmrf_reglat: par too short'
   x(1:2)=[1.0_dp,-par(1)]; y(n+1)=-par(2)
...
case('m2p3')
   ...
   x(1:2)=[1.0_dp,-par(1)]; y(1:2)=[-par(2),-par(3)]; z(2)=1.0_dp
...
case('m2p4')
   ...
   x(1:2)=[1.0_dp,-par(1)]; y(1:2)=[-par(2),-par(3)]; w(1:2)=[-par(2),-par(4)]; z(2)=1.0_dp
   p1=csr_kronecker(im,csr_toeplitz(x,eps=tol))
