!     closestdistXY,  distance between x and x or between x and y
!
!
!     We have four distances implemented:
!       c("euclidean", "maximum", "minkowski", "greatcircle")
!
!     In case we need the distance matrix between x and x, then the
!     following parameters are used as well:
!     if part=-1, lower tri, part=0 the entire matrix
!        part= 1, upper tri only.
!     only values smaller than eta are considered.
!     p power for minkowski
...
! cycle over all rows of x (independent of part)
...
! Start calculating the distance (until delta is exceeded)
tmp = 0.0
do  k = 1, ncol
tmp = tmp + distfcn(x(i,k),y(j,k),p)
if( tmp.gt.etap) goto 10
enddo
! Delta is not exceeded.

!     in case nnz was too small, recall line to get a better estimate
if( jja .gt. nnz) then
iflag = i
goto 20
endif
...
! Delta is not exceeded.
!     (i,j) has a distance smaller than eta.
...
!     Great savings if we know that x=y.
!     Changes for archaic d... to ... trigonometric fcn
if (p .lt. 0) then
equi=.TRUE.
p=-p
else
...
!     x2 is missing if equi=.TRUE. and we reuse the y stuff
...
!     Due to numerical instabilities, we need the following... 0.15-2:
!     Patch suggested at code clinics.
