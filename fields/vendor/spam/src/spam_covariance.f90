...
integer,intent(in)::i
real(dp)::x
if(i<=size(theta))then;x=abs(theta(i));else;x=default;end if
...
integer::i
allocate(v(size(h)))
do i=1,size(h);v(i)=cov_value(h(i),theta,kind,eps);end do
...
do k=1,size(v%entries);v%entries(k)=cov_value(v%entries(k),theta,kind,eps);end do
...
tol=epsilon(one);if(present(eps))tol=eps
select case(kind)
case(4) ! nugget has a one-parameter convention
   sill=theta_at(theta,1,1.0_dp)
   if(h<tol)then;v=sill;else;v=0.0_dp;end if
   return
case default
...
if(range<tol)then
   if(h<tol)then;v=sill+nug;else;v=0.0_dp;end if
   return
end if
x=h/range
if(h<tol)then
...
case(1) ! exponential / Matern 1/2
   v=sill*exp(-x)
case(2) ! spherical
   if(x>=1.0_dp)then;v=0.0_dp;else;v=sill*(1.0_dp-1.5_dp*x+0.5_dp*x*x*x);end if
case(3) ! spherical correlation
   if(x>=1.0_dp)then;v=0.0_dp;else;v=1.0_dp-1.5_dp*x+0.5_dp*x*x*x;end if
case(5) ! Wu1
   if(x>=1.0_dp)then;v=0.0_dp;else;v=sill*(1.0_dp-x)**3*(1.0_dp+3.0_dp*x+x*x);end if
case(6) ! Wu2
   if(x>=1.0_dp)then;v=0.0_dp;else;v=sill*(1.0_dp-x)**4*(4.0_dp+16.0_dp*x+12.0_dp*x*x+3.0_dp*x**3)/4.0_dp;end if
case(7) ! Wu3
   if(x>=1.0_dp)then
      v=0.0_dp
   else
...
case(8) ! Wendland 1
   if(x>=1.0_dp)then;v=0.0_dp;else;v=sill*(1.0_dp-x)**4*(4.0_dp*x+1.0_dp);end if
case(9) ! Wendland 2
   if(x>=1.0_dp)then;v=0.0_dp;else;v=sill*(1.0_dp-x)**6*(35.0_dp*x*x+18.0_dp*x+3.0_dp)/3.0_dp;end if
case(10) ! general Matern: theta=(range,sill,nu,nugget)
   nu=theta_at(theta,3,1.0_dp);nug=theta_at(theta,4,0.0_dp)
   if(h<tol)then
      v=sill+nug
   else
...
real(dp),intent(in)::h(:),theta(:);real(dp),intent(in),optional::eps;real(dp),allocatable::v(:)
v=cov_apply_vec(h,theta,1,eps)
end function
function cov_exp_csr(h,theta,eps) result(v)
type(csr_matrix),intent(in)::h;real(dp),intent(in)::theta(:);real(dp),intent(in),optional::eps;type(csr_matrix)::v
...
real(dp),intent(in)::h(:);real(dp),intent(in)::range;real(dp),intent(in),optional::eps;real(dp),allocatable::v(:)
real(dp)::th(3);th=[range,1.0_dp,0.0_dp];v=cov_apply_vec(h,th,3,eps)
end function
function cor_sph_csr(h,range,eps) result(v)
type(csr_matrix),intent(in)::h;real(dp),intent(in)::range;real(dp),intent(in),optional::eps;type(csr_matrix)::v
real(dp)::th(3);th=[range,1.0_dp,0.0_dp];v=cov_apply_csr(h,th,3,eps)
end function
