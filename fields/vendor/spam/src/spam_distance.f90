...
logical::same
n1=size(x,1); nd=size(x,2); same=.not.present(y)
if(same) then
   allocate(yy(n1,nd)); yy=x; n2=n1
else
   if(size(y,2)/=nd) error stop 'nearest_dist: x/y column mismatch'
   n2=size(y,1); allocate(yy(n2,nd)); yy=y
end if
meth='euclidean'; if(present(method))meth=lower(trim(method))
del=1.0_dp; if(present(delta))del=abs(delta)
...
r=3963.34_dp; if(present(radius))r=radius
if(r<=0.0_dp) error stop 'nearest_dist: radius must be positive'
cap=max(16,min(n1*n2,1024)); allocate(ir(cap),jc(cap),val(cap)); nz=0
...
do i=1,n1
   do j=1,n2
      ...
      case('euclidean','euclidian')
         tmp=0.0_dp
         do k=1,nd; tmp=tmp+(x(i,k)-yy(j,k))**2; if(tmp>del*del)exit; end do
         if(tmp>del*del)cycle; dd=sqrt(tmp)
      case('maximum')
         tmp=0.0_dp
         do k=1,nd;tmp=max(tmp,abs(x(i,k)-yy(j,k)));if(tmp>del)exit;end do
         if(tmp>del)cycle;dd=tmp
      case('minkowski')
         if(pp<=0.0_dp) error stop 'nearest_dist: p must be positive'
         tmp=0.0_dp
         do k=1,nd;tmp=tmp+abs(x(i,k)-yy(j,k))**pp;if(tmp>del**pp)exit;end do
         if(tmp>del**pp)cycle;dd=tmp**(1.0_dp/pp)
      case('greatcircle')
         ...
         if(dotv>=0.99999999999_dp)then;dd=0.0_dp;else;dd=acos(dotv)*r;end if
      case default
         error stop 'nearest_dist: invalid method'
      end select
      nz=nz+1; call grow_triplet(ir,jc,val,nz)
      ir(nz)=i;jc(nz)=j;val(nz)=dd
   end do
end do
...
real(dp),intent(in)::x(:,:),y(:,:);real(dp),intent(in),optional::delta
type(csr_matrix)::a
if(present(delta))then;a=nearest_dist(x,y,delta=delta);else;a=nearest_dist(x,y);end if
end function rdist
...
type(csr_matrix)::a
real(dp)::d,r;logical::mi
d=1.0_dp;if(present(delta))d=delta
...
integer,allocatable::ii(:),jj(:);real(dp),allocatable::vv(:);integer::n
if(need<=size(ir))return
n=max(need,2*size(ir));allocate(ii(n),jj(n),vv(n));ii=0;jj=0;vv=0.0_dp
ii(:size(ir))=ir;jj(:size(jc))=jc;vv(:size(val))=val
call move_alloc(ii,ir);call move_alloc(jj,jc);call move_alloc(vv,val)
