...
abstract interface
 function random_values(n) result(x)
  import dp;integer,intent(in)::n;real(dp),allocatable::x(:)
 end function
end interface
contains
...
type(csr_matrix)::a
integer::nc,i,j,k,nz;real(dp)::rho;logical::sy,pd
real(dp),allocatable::u(:),v(:),d(:,:);integer,allocatable::ir(:),jc(:)
nc=nrow;if(present(ncol))nc=ncol;rho=0.5_dp;if(present(density))rho=density
sy=.false.;if(present(sym))sy=sym;pd=.false.;if(present(spd))pd=spd
...
allocate(u(max(1,nrow*nc)));u=runif_vec(size(u));nz=count(u(:nrow*nc)<=rho)
if(nz==0)then
 allocate(d(nrow,nc));d=0.0_dp
 if(sy)then;do i=1,min(nrow,nc);d(i,i)=1.0_dp;end do;end if
 a=csr_from_dense(d);return
end if
allocate(ir(nz),jc(nz),v(nz));k=0
do i=1,nrow;do j=1,nc
 if(u((i-1)*nc+j)<=rho)then;k=k+1;ir(k)=i;jc(k)=j;v(k)=1.0_dp;end if
end do;end do
...
if(sy.or.pd)then
 ...
 do i=1,nrow
  do j=i+1,nrow
   if(d(i,j)/=0.0_dp)then;d(j,i)=d(i,j);else;d(i,j)=d(j,i);end if
  end do
 end do
 if(pd)then
  do i=1,nrow
