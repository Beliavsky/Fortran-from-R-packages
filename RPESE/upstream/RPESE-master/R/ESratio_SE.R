...
#' @import RPEIF
#'
#' @title Standard Error Estimate for Expected Shortfall Ratio (ESratio) of Returns
#'
#' @description \code{ESratio.SE} computes the standard error of the expected shortfall ratio of the returns.
#'
...
ESratio.SE = function(data, alpha = 0.1, rf = 0,
  ...
  if(is.null(dim(data)) || ncol(data) == 1)
    point.est <- ESratio(data, alpha = alpha, rf = rf) else
      point.est <- apply(data, 2, function(x) ESratio(x, alpha = alpha, rf = rf))

    # SE Computation
    if(is.null(se.method)){
      return(point.est)
    } else{
      SE.out <- list(ESratio = point.est)
      for(mymethod in se.method){
        SE.out[[mymethod]] <- EstimatorSE(data, estimator.fun = "ESratio", alpha = alpha, rf = rf,
                                          se.method = mymethod,
      ...
      SE.out <- Add_Correlations(SE.out = SE.out, data = data, cleanOutliers = cleanOutliers, corOut = corOut, IF.func = IF.ESratio, ...)
