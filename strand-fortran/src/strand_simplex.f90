...
! The self-contained two-phase simplex implementation is adapted from the
! GPL-3.0-only PortfolioOptim Fortran translation generated in this project series.
module strand_simplex
  use strand_kinds, only : dp
  use strand_types, only : lp_result
  implicit none
  private
  public :: solve_lp, solve_bounded_lp

contains


  function solve_bounded_lp(c, a, sense, b, lower, upper, tol, maxiter) result(res)
    ! Minimize c^T x subject to row-wise constraints and variable bounds.
    ! sense = -1 for <=, 0 for equality, +1 for >=.
    real(dp), intent(in) :: c(:), a(:, :), b(:), lower(:), upper(:)
    integer, intent(in) :: sense(:)
    real(dp), intent(in), optional :: tol
    integer, intent(in), optional :: maxiter
    type(lp_result) :: res
    type(lp_result) :: shifted
    real(dp), allocatable :: aa(:, :), bb(:), shifted_b(:)
    integer :: i, row, mnew, n

    n = size(c)
    allocate(res%x(n))
    res%x = 0.0_dp
    if (size(a, 2) /= n .or. size(a, 1) /= size(b) .or. size(sense) /= size(b) .or. &
        size(lower) /= n .or. size(upper) /= n) then
      res%message = 'bounded LP dimension mismatch'
      return
    end if
    if (any(.not. (lower < huge(1.0_dp)))) then
      res%message = 'all lower bounds must be finite'
      return
    end if
    if (any(upper < lower)) then
      res%message = 'inconsistent variable bounds'
      return
    end if

    mnew = count(sense /= 0) + 2 * count(sense == 0) + count(upper < huge(1.0_dp) / 4.0_dp)
    allocate(aa(mnew, n), bb(mnew), shifted_b(size(b)))
    shifted_b = b - matmul(a, lower)
    row = 0
    do i = 1, size(b)
      select case (sense(i))
      case (-1)
      ...
      case (0)
        ...
        bb(row) = -shifted_b(i)
      case default
        res%message = 'invalid constraint sense'
        return
      end select
    end do
    do i = 1, n
      if (upper(i) < huge(1.0_dp) / 4.0_dp) then
        row = row + 1
        aa(row, :) = 0.0_dp
        aa(row, i) = 1.0_dp
        bb(row) = upper(i) - lower(i)
      end if
    end do

    if (present(tol) .and. present(maxiter)) then
      shifted = solve_lp(c, aa, bb, tol, maxiter)
    else if (present(tol)) then
      shifted = solve_lp(c, aa, bb, tol=tol)
    else if (present(maxiter)) then
      shifted = solve_lp(c, aa, bb, maxiter=maxiter)
    else
      shifted = solve_lp(c, aa, bb)
    end if
    res = shifted
    if (allocated(shifted%x)) then
      res%x = lower + shifted%x
      res%objective = dot_product(c, res%x)
    end if
  end function solve_bounded_lp
