...
contains
  subroutine numerical_model_jacobian(model,theta,x,yhat,jacobian,status,step)
    ...
    real(dp) :: h
    integer :: j,n,p,st
    n=size(x,1);p=size(theta);h=epsilon(1.0_dp)**(1.0_dp/3.0_dp);if(present(step))h=step
    allocate(yhat(n),jacobian(n,p),tp(p),tm(p),yp(n),ym(n))
    call model(theta,x,yhat,st)
    if(st/=NLME_SUCCESS .or. any(.not.ieee_is_finite(yhat)))then;status=NLME_CALLBACK_ERROR;return;end if
    do j=1,p
      tp=theta;tm=theta
      tp(j)=theta(j)+h*max(1.0_dp,abs(theta(j)));tm(j)=theta(j)-h*max(1.0_dp,abs(theta(j)))
      call model(tp,x,yp,st);if(st/=NLME_SUCCESS)then;status=NLME_CALLBACK_ERROR;return;end if
      call model(tm,x,ym,st);if(st/=NLME_SUCCESS)then;status=NLME_CALLBACK_ERROR;return;end if
      if(any(.not.ieee_is_finite(yp)) .or. any(.not.ieee_is_finite(ym)))then;status=NLME_CALLBACK_ERROR;return;end if
  ...
  subroutine fit_gnls(model,y,xdata,theta0,result,correlation,variance,time,group,var_covariate,var_group,&
    ...
    n=size(y);p=size(theta0)
    if(size(xdata,1)/=n .or. n<=p .or. p<1)then;result%status=NLME_DIMENSION_ERROR;return;end if
    ...
    allocate(tvec(n),vcov(n),theta(p),candidate(p),fc(n));theta=theta0
    if(present(time))then;if(size(time)/=n)then;result%status=NLME_DIMENSION_ERROR;return;end if;tvec=time
    else;tvec=[(real(it,dp),it=1,n)];end if
    if(present(group))then;if(size(group)/=n)then;result%status=NLME_DIMENSION_ERROR;return;end if;allocate(grp(n));grp=group
    else;call default_group_vector(n,grp);end if
    ...
    if(present(var_group))then
      ...
      allocate(vgrp(n));vgrp=var_group
    else
      allocate(vgrp(n));vgrp=1
    end if
    ...
    lambda=1.0e-3_dp;result%converged=.false.
    do it=1,ctl%max_iter
      call numerical_model_jacobian(model,theta,xdata,f,jmat,status,ctl%fd_step)
      if(status/=NLME_SUCCESS)then;result%status=status;return;end if
      r=y-f;call solve_spd(v,jmat,vinvj,status,ldv);if(status/=NLME_SUCCESS)then;result%status=status;return;end if
      call solve_spd(v,r,vinvr,status);if(status/=NLME_SUCCESS)then;result%status=status;return;end if
      normal=matmul(transpose(jmat),vinvj);rhs=matmul(transpose(jmat),vinvr);obj=dot_product(r,vinvr)
      do st=1,p;normal(st,st)=normal(st,st)+lambda*max(1.0_dp,normal(st,st));end do
      call solve_spd(normal,rhs,delta,status)
      if(status/=NLME_SUCCESS)then;lambda=lambda*10.0_dp;cycle;end if
      candidate=theta+delta;call model(candidate,xdata,fc,st)
      if(st/=NLME_SUCCESS .or. any(.not.ieee_is_finite(fc)))then;lambda=lambda*10.0_dp;cycle;end if
      rc=y-fc;call solve_spd(v,rc,vinvr,status);if(status/=NLME_SUCCESS)then;result%status=status;return;end if
      newobj=dot_product(rc,vinvr)
      if(newobj<obj)then
        theta=candidate;lambda=max(1.0e-12_dp,lambda/3.0_dp)
        if(vector_norm2(delta)<=ctl%tolerance*(1.0_dp+vector_norm2(theta)))then;result%converged=.true.;exit;end if
      else
    ...
    r=y-f;call solve_spd(v,jmat,vinvj,status,ldv);if(status/=NLME_SUCCESS)then;result%status=status;return;end if
    call solve_spd(v,r,vinvr,status);if(status/=NLME_SUCCESS)then;result%status=status;return;end if
    normal=matmul(transpose(jmat),vinvj);call inverse_spd(normal,covp,status,logdetn)
    if(status/=NLME_SUCCESS)then;result%status=status;return;end if
    ...
    result%parameters=theta;result%parameter_cov=s2*covp;result%fitted=f;result%residuals=r;result%sigma=sqrt(s2)
    result%log_likelihood=-0.5_dp*(real(n,dp)*(log(2.0_dp*pi_dp)+log(s2))+ldv+dot_product(r,vinvr)/s2)
    result%iterations=min(it,ctl%max_iter);result%status=merge(NLME_SUCCESS,NLME_MAX_ITER,result%converged)
  ...
  subroutine fit_nlme(model,y,xdata,group,theta0,random_index,result,random,correlation,variance,time,&
    ...
    n=size(y);p=size(theta0);q=size(random_index)
    if(size(xdata,1)/=n .or. size(group)/=n .or. p<1 .or. q<1 .or. any(random_index<1) .or. &
       any(random_index>p))then;result%status=NLME_DIMENSION_ERROR;return;end if
    ctl=nlme_control();if(present(control))ctl=control
    call group_level_indices(group,levels,gindex,status);if(status/=NLME_SUCCESS)then;result%status=status;return;end if
    ng=size(levels);allocate(theta(p),theta_old(p),bre(ng,q),f(n),jall(n,p),xlin(n,p),zlin(n,q),ystar(n));
    theta=theta0;bre=0.0_dp;result%converged=.false.
    do it=1,ctl%max_outer
      theta_old=theta
      do g=1,ng
        call find_group_indices(group,levels(g),idx);allocate(local_theta(p));local_theta=theta
        do j=1,q;local_theta(random_index(j))=local_theta(random_index(j))+bre(g,j);end do
        call numerical_model_jacobian(model,local_theta,xdata(idx,:),flocal,jlocal,status,ctl%fd_step)
        deallocate(local_theta)
        if(status/=NLME_SUCCESS)then;result%status=status;return;end if
        f(idx)=flocal;jall(idx,:)=jlocal
      end do
      xlin=jall
      do j=1,q;zlin(:,j)=jall(:,random_index(j));end do
      ...
      lctl=ctl;lctl%max_iter=max(80,ctl%max_iter/2)
      ...
      if(linear_fit%status/=NLME_SUCCESS .and. linear_fit%status/=NLME_MAX_ITER)then
        result%status=linear_fit%status;return
      end if
      theta=linear_fit%beta;bre=linear_fit%random_effects
      change=maxval(abs(theta-theta_old))/max(1.0_dp,maxval(abs(theta_old)))
      if(change<=ctl%tolerance)then;result%converged=.true.;exit;end if
    end do
    allocate(final_fit(n));final_fit=0.0_dp
    if(allocated(flocal))deallocate(flocal)
    do g=1,ng
      call find_group_indices(group,levels(g),idx);allocate(local_theta(p));local_theta=theta
      do j=1,q;local_theta(random_index(j))=local_theta(random_index(j))+bre(g,j);end do
      allocate(flocal(size(idx)));call model(local_theta,xdata(idx,:),flocal,st);deallocate(local_theta)
      if(st/=NLME_SUCCESS)then;result%status=NLME_CALLBACK_ERROR;return;end if
    ...
    result%parameters=theta;result%parameter_cov=linear_fit%beta_cov;result%random_effects=bre;result%group_levels=levels
    result%fitted=final_fit;result%residuals=y-final_fit;result%sigma=linear_fit%sigma
    result%log_likelihood=linear_fit%log_likelihood;result%iterations=min(it,ctl%max_outer)
