...
module sgt_linalg
   ...
   use sgt_status, only : sgt_ok, sgt_invalid_input, sgt_size_mismatch, &
      sgt_singular_matrix, sgt_no_convergence, sgt_not_positive_definite
   implicit none
   private
...
contains
   ...
   subroutine solve_matrix_rhs(a,b,x,status)
      ...
      if (n < 1 .or. size(a,2) /= n .or. size(b,1) /= n .or. nrhs < 1) then
         allocate(x(0,0))
         if (present(status)) status = sgt_size_mismatch
         return
      end if
      ...
      do k = 1, n
         ...
         if (pivot_value <= 1000.0_dp*epsilon(1.0_dp)*scale) then
            x = 0.0_dp
            local_status = sgt_singular_matrix
   ...
   subroutine symmetric_eigen_jacobi(a,values,vectors,status,tolerance,max_iterations)
      ...
      local_status=sgt_no_convergence
      if (n==1) then
         values(1)=work(1,1)
         local_status=sgt_ok
      else
   ...
   subroutine inverse_symmetric_positive_definite(a,ainv,status)
      ...
      if (minval(values)<=tol) then
         ...
         if (present(status)) status=sgt_not_positive_definite
         return
      end if
      allocate(scaled(n,n)); scaled=vectors
      do i=1,n
   ...
   subroutine project_simplex_sum(v,total,w,status)
      ...
      if (n<1 .or. size(w)/=n .or. total<0.0_dp) then
         ...
         if (present(status)) status=sgt_invalid_input
         return
      end if
      if (total<=tiny(1.0_dp)) then
         w=0.0_dp; if (present(status)) status=sgt_ok; return
      end if
      allocate(u(n)); u=v
      do i=2,n
         key=u(i); j=i-1
   ...
   subroutine nnqp_projected_gradient(h,b,x,status,max_iterations,tolerance)
      ...
      if (size(h,1)/=n .or. size(h,2)/=n .or. size(x)/=n .or. n<1) then
         if (present(status)) status=sgt_size_mismatch; return
      end if
      ...
      if (lips<=tiny(1.0_dp)) then
         x=max(x,0.0_dp); if (present(status)) status=sgt_ok; return
      end if
      step=1.0_dp/lips
      ...
      do k=1,maxiter
         ...
         if (vector_norm2(xnew-x)/den<tol) then
            x=xnew; local_status=sgt_ok; exit
         end if
         x=xnew
      end do
      if (present(status)) status=local_status
   ...
   subroutine rows_correlation(x,c,status)
      ...
      if (p<1 .or. n<2) then
         allocate(c(0,0)); if (present(status)) status=sgt_invalid_input; return
      end if
