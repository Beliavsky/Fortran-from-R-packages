...
FLAGS=(-std=f2018 -Wall -Wextra -Wpedantic -Werror -fcheck=all -ffpe-trap=invalid,zero,overflow -fbacktrace -O0 -g -J"$BUILD/mod" -I"$BUILD/mod")
...
for file in "$ROOT"/example/*.f90 "$ROOT"/app/*.f90; do name=$(basename "${file%.f90}"); "$FC" "${FLAGS[@]}" "$file" "${OBJECTS[@]}" -o "$BUILD/bin/$name"; "$BUILD/bin/$name" >/dev/null; done
