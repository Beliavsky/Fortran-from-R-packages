...
mkdir -p "$build"
cd "$build"
flags="-std=f2018 -Wall -Wextra -Wpedantic -Werror -Wno-compare-reals -Wno-maybe-uninitialized -Wno-intrinsic-shadow -O3"
...
for f in $mods; do gfortran $flags -c -J . -I . "$root/src/$f.f90"; done
objs=""; for f in $mods; do objs="$objs $f.o"; done
