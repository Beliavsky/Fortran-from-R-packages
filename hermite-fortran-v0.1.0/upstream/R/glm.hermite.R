  ...
  if (link == "log") {
    ...
    if (!intercept)
    {
      if (m.fixed)
      {
        ...
        if (m == 1)
        {
          if(!inherits(class(try(glm(formula, family = poisson(link = link), data=data, start=start[-length(start)]),silent = TRUE)), "try-error"))
          {
      ...
      if (!inherits(fit2, "list")) output$vcov <- vcov(fit2)       
      output$hess <- hess
