...
& CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND, &
& IXDUM1, IXDUM2, IXDUM3, IXDUM4, IXDUM5, IXDUM6, IOWNS(6), &
& ICF, IERPJ, IERSL, JCUR, JSTART, KFLAG, L, &
& LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER, &
& MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
...
& WM, IWM, F, JAC, PJAC, SLVS,RPAR,IPAR)
...
& RWORK(LACOR),IA,JA,RWORK(LWM),IWK(2*LWM-1),IPFLAG, F, JAC, &
& rpar,ipar)
...
! karline initialised free to avoid warning
free = 1
!----initialize tag
tag = mark(ek)
!
...
& PDNORM, IOWND21, IOWND22, IOWND23, &
& ICOUNT, IRFLAG, JTYP, MUSED, MXORDN, MXORDS
INTEGER I, I1, IREDO, IRET, J, JB, M, NCF, NEWQ
...
! N      = the number of first-order differential equations.
!-----------------------------------------------------------------------
! KARLINE: INITIALISED IREDO, RH TO AVOID COMPILER WARNING - SHOULD HAVE NO EFFECT
IREDO = 0
RH = 1.D0
KFLAG = 0
TOLD = TN
...
& ROWNS21, ROWNS22, ROWNS23, PDNORM, &
& IOWND21, IOWND22, IOWND23, IOWNS21, IOWNS22, JTYP, &
& MUSED, MXORDN, MXORDS
INTEGER I, I1, I2, IER, II, J, J1, JJ, LENP, &
& MBA, MBAND, MEB1, MEBAND, ML, ML3, MU, NP1
...
& WM(LOCWP), IWM(LOCIWP), IER,rpar,ipar)
NJE = NJE + 1
IF (IER .EQ. 0) RETURN
IERPJ = 1
RETURN
...
& LIOM, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG, rpar,ipar)
NNI = NNI + 1
NLI = NLI + LIOM
NPS = NPS + NPSL
...
& LPCG, WM(LOCWP), IWM(LOCIWP),WM(LWK),IFLAG,rpar, ipar)
NNI = NNI + 1
NLI = NLI + LPCG
NPS = NPS + NPSL
IF (IFLAG .NE. 0) NCFL = NCFL + 1
...
& LIOM, WP, IWP, WK, IFLAG,rpar,ipar)
EXTERNAL F, PSOL
...
! karline: initialised RHO to avoid warning
RHO = 0.D0
IFLAG = 0
LIOM = 0
NPSL = 0
...
& WK, WP, IWP, HL0, JPRE, IER, NPSL, rpar,ipar)
  IF (IER .NE. 0) GO TO 300
...
& WP, IWP, HL0, JPRE, IER, NPSL,rpar,ipar)
EXTERNAL F, PSOL
...
& LGMR, WP, IWP, WK, DL, IFLAG,rpar,ipar)
EXTERNAL F, PSOL
integer ipar(*)
double precision rpar(*)
...
& EWT, ITOL, ATOL, Y, TEMP, H0, NITER, IER, rpar,ipar)
EXTERNAL F
