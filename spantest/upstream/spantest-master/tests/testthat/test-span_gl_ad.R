test_that("span_gl_ad returns correct structure and decision output", {
  ...
  expect_true(is.numeric(res$pval_LMC))
  expect_true(is.numeric(res$pval_BMC))
  expect_true(res$pval_LMC >= 0 && res$pval_LMC <= 1)
  expect_true(res$pval_BMC >= 0 && res$pval_BMC <= 1)
  expect_true(res$Decisions_string %in% c("Accept", "Reject", "Inconclusive"))
  expect_equal(res$H0, "alpha = 0 and delta = 0")
})
