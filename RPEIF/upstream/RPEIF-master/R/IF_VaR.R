...
#' # This example works without any additional packages
...
#' }
#'
IF.VaR <- function(returns = NULL, evalShape = FALSE, retVals = NULL, nuisPars = NULL, k = 4,
  ...
  if(evalShape){
    IFvals <- EvaluateShape(estimator = "VaR",
                            retVals = retVals, returns = returns, k = k, nuisPars = nuisPars,
                            IFplot = IFplot, IFprint = IFprint, alpha = alpha)
    if(IFprint)
      return(IFvals) else{
  ...
  IF.VaR.vector <- IF.VaR.fn(x = returns, returns = returns, alpha = alpha)
