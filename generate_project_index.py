#!/usr/bin/env python3
"""Generate a Markdown index by category, keyword, and NIST GAMS class."""
...
def render_gams_projects(groups: dict[str, set[str]]) -> list[str]:
    lines = ["## Projects by NIST GAMS classification", ""]
    lines.extend(
        [
...
def render_index(
    ...
    gams: dict[str, set[str]],
    missing_categories: list[str],
    missing_keywords: list[str],
    missing_gams: list[str],
) -> str:
    all_projects = set(missing_categories) | set(missing_keywords) | set(missing_gams)
    for projects in categories.values():
        all_projects.update(projects)
    for projects in keywords.values():
        all_projects.update(projects)
    for projects in gams.values():
        all_projects.update(projects)
    ...
    if missing_categories or missing_keywords or missing_gams:
        ...
        if missing_gams:
            ...
            lines.extend([f"Missing `gams`: {projects}", ""])
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a Markdown index by category, keyword, and NIST GAMS class."
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="exit unsuccessfully if the output file is absent or out of date",
    )
...
def main() -> int:
    ...
    if args.check:
        try:
            ...
            print(f"ERROR: {output.name} does not exist; run this script to generate it.", file=sys.stderr)
            return 1
        if current != generated:
            print(f"ERROR: {output.name} is out of date; run this script to regenerate it.", file=sys.stderr)
            return 1
