module lbfgsb3_core_mod
  !! Free-form module translation of L-BFGS-B 3.0 as distributed with
  !! the R package lbfgsb3c. The numerical L-BFGS-B code is BSD-3-Clause.
  !! See LICENSES/LBFGSB-BSD-3-Clause.txt and NOTICE.md.
  use, intrinsic :: iso_fortran_env, only : real64, output_unit
  implicit none
  private
  public :: setulb, set_core_output
  logical :: core_output_enabled = .false.

contains

  subroutine set_core_output(enabled)
    logical, intent(in) :: enabled
    core_output_enabled = enabled
  end subroutine set_core_output
  !
  ...
  !
  !=============================================================================
  ! JN 20150118 change name setulb to lbfgsb3
  !      subroutine setulb(n, m, x, l, u, nbd, f, g, factr, pgtol, wa, iwa,
  subroutine setulb(n, m, x, l, u, nbd, f, g, factr, pgtol, wa, iwa, itask, iprint, icsave, lsave, isave, dsave)
  ...
  lxp  = isave(15)
  lwa  = isave(16)
  !j      call intpr('Incoming value of iprint =',-1,iprint,1)
  
  call mainlb(n,m,x,l,u,nbd,f,g,factr,pgtol, &
       wa(lws),wa(lwy),wa(lsy),wa(lss),wa(lwt),wa(lwn),wa(lsnd), &
       wa(lz),wa(lr),wa(ld),wa(lt),wa(lxp),wa(lwa), &
       iwa(1),iwa(n+1),iwa(2*n+1),itask,iprint,icsave,lsave, &
       isave(22),dsave)
  
  !j      call intpr(" itask on return is ", -1, itask, 1)
  !w      write(6,*) " itask on return is ",itask
  ...
  implicit none
  integer          lsave(4)
  integer          n, m, iprint, itask, icsave, nbd(n), index(n), iwhere(n), indx2(n), isave(23)
  ...
  !$$$      character        word(4)
  integer          i,k,nintol,iback,nskip, head,col,iter,itail,iupdat, nseg,nfgv,info,ifun, iword,nfree,nact,ileave,nenter
  !j itmp for use in R output
  integer          itmp
  double precision theta,fold,dr,rr,tol, xstep,sbgnrm,ddum,dnorm,dtd,epsmch, cpu1,cpu2, sbtime,time1,time2, gd,gdold,stp,stpmx,time
  ...
  !w         write (6,1002) iter,f,sbgnrm
  !w 1002 format
  !w     +  (/,'At iterate',i5,4x,'f= ',1p,d12.5,4x,'|proj g|= ',1p,d12.5)
  !w         write (itfile,1003) iter,nfgv,sbgnrm,f
  endif
  ...
  call cauchy(n,x,l,u,nbd,g,indx2,iwhere,t,d,z,m,wy,ws,sy,wt, &
       theta,col,head,wa(1),wa(2*m+1),wa(4*m+1),wa(6*m+1),nseg, &
  ...
  !w     +'   refresh the lbfgs memory and restart the iteration.')
     ...
     call intpr( '   refresh the lbfgs memory and restart the iteration.', -1,0,0)
  ...
  if (wrk.eq.1) call formk(n,nfree,index,nenter,ileave,indx2,iupdat, updatd,wn,snd,m,ws,wy,sy,theta,col,head,info)
  if (info .ne. 0) then
  !          nonpositive definiteness in Cholesky factorization;
  !          refresh the lbfgs memory and restart the iteration.
  !w         if(iprint .ge. 1) write (6, 1006)
     if(iprint .ge. 1) then
       call intpr( ' Nonpositive definiteness in Cholesky factorization in formk;', -1, 0, 0)
       call intpr( '   refresh the lbfgs memory and restart the iteration.', -1, 0, 0)
  ...
  !w            if(iprint .ge. 1) write (6, 1008)
        if(iprint .ge. 1) then
          call intpr( ' Bad direction in the line search;', -1, 0, 0)
          call intpr( '   refresh the lbfgs memory and restart the iteration.', -1, 0, 0)
        endif
  ...
  !w         if(iprint .ge. 1) write (6, 1007)
     if (iprint .ge. 1) then
        call intpr( ' Nonpositive definiteness in Cholesky factorization in formt;', -1, 0, 0)
        call intpr( '   refresh the lbfgs memory and restart the iteration.', -1, 0, 0)
  ...
  !w     +'   refresh the lbfgs memory and restart the iteration.')
  !w 1006 format (/,
  !w     +' Nonpositive definiteness in Cholesky factorization in formk;',/,
  !w     +'   refresh the lbfgs memory and restart the iteration.')
  !w 1007 format (/,
  !w     +' Nonpositive definiteness in Cholesky factorization in formt;',/,
  !w     +'   refresh the lbfgs memory and restart the iteration.')
  !w 1008 format (/,
  ...
  if (iprint .ge. 0) then
     if (prjctd .eq. 1) then
     call intpr('initial X infeasible. Restart with projection.', -1, 0, 0)
     endif
     if (cnstnd .eq. 0) then
  ...
  if (iprint .gt. 0) then
     call intpr(' Variables exactly at bounds for X0 ',-1, nbdd, 1)
  endif
  ...
  subroutine cauchy(n,x,l,u,nbd,g,iorder,iwhere,t,d,xcp,m,wy,ws, &
       sy,wt,theta,col,head,p,c,wbp,v,nseg,iprint,sbgnrm,info,epsmch)
  implicit none
  integer          n, m, head, col, nseg, iprint, info, nbd(n), iorder(n), iwhere(n)
  double precision theta,epsmch,x(n),l(n),u(n),g(n),t(n),d(n), &
  ...
  integer nprt
  double precision f1,f2,dt,dtm,tsum,dibp,zibp,dibp2,bkmin, tu,tl,wmc,wmp,wmw,tj,tj0,neggi,sbgnrm, f2_org
  double precision one,zero
  parameter        (one=1.0d0,zero=0.0d0)
  ...
  !w         write (6,5010) dt
     call dblepr1('Distance to the next break point =  ', -1, dt)
     call dblepr1('Distance to the stationary point =  ', -1, dtm)
  !w         write (6,6010) dtm
  endif
  ...
  if (iprint .ge. 100) call intpr('Variable fixed, index ',-1,ibp, 1)
  ...
  !jn            write(6,*) ' index = ', i
        ...
        if (iwhere(k) .gt. 0) then
           ...
           if (iprint .ge. 100) then
              call intpr( 'Variable k leaves the set of free variables for k =', -1, k, 1)
           endif
  ...
  integer          n, iter, ifun, iback, nfgv, info, itask, icsave, nbd(n), isave(2)
  ...
  !w            write(6,*)' ascent direction in projection gd = ', gd
          call dblepr1(' ascent direction in projection gd = ', -1, gd)
        info = -4
        return
     endif
  endif
  ...
  !2345678901234567890123456789012345678901234567890123456789012345678901234567890
   if ((icsave .lt. 6) .or. ((icsave .gt. 8) .and. (icsave .lt. 23))) then
  !         task = 'FG_LNSRCH'
     itask = 20
     ifun = ifun + 1
     nfgv = nfgv + 1
  ...
  subroutine dcsrch(f,g,stp,ftol,gtol,xtol,stpmin,stpmax, itask,isave,dsave)
  !      character*(*) task
  ...
  if (brackt .and. (stp .le. stmin .or. stp .ge. stmax)) itask = 23
  !     +   task = 'WARNING: ROUNDING ERRORS PREVENT PROGRESS'
  if (brackt .and. stmax - stmin .le. xtol*stmax) itask = 26
  !     +   task = 'WARNING: XTOL TEST SATISFIED'
  if (stp .eq. stpmax .and. f .le. ftest .and. g .le. gtest) itask = 24
  !     +   task = 'WARNING: STP = STPMAX'
  if (stp .eq. stpmin .and. (f .gt. ftest .or. g .ge. gtest)) itask = 25
  !     +   task = 'WARNING: STP = STPMIN'
  !     Test for convergence.
  
  if (f .le. ftest .and. abs(g) .le. gtol*(-ginit)) itask = 6
  !     +   task = 'CONVERGENCE'
  
  !     Test for termination.
  
  !      if (task(1:4) .eq. 'WARN' .or. task(1:4) .eq. 'CONV') goto 1000
  if ((itask .ge. 23) .or. ((itask .le. 8) .and. (itask .ge. 6)))    goto 1000
  ...
  subroutine dpofa(a, lda, n, info)
    !! Cholesky factorization compatible with the LINPACK DPOFA interface.
  ...
  subroutine dtrsl(t, ldt, n, b, job, info)
    !! Triangular solve compatible with the LINPACK DTRSL interface.
    integer, intent(in) :: ldt, n, job
    double precision, intent(in) :: t(ldt, *)
    double precision, intent(inout) :: b(*)
    integer, intent(out) :: info
    integer :: j
    logical :: lower, transpose
    ...
    lower = mod(job, 10) == 0
    transpose = mod(job / 10, 10) /= 0

    if (.not. transpose .and. lower) then
      ! Solve lower-triangular T*x=b (JOB=00).
      do j = 1, n
        b(j) = b(j) / t(j,j)
        if (j < n) b(j+1:n) = b(j+1:n) - b(j)*t(j+1:n,j)
      end do
    else if (.not. transpose .and. .not. lower) then
      ! Solve upper-triangular T*x=b (JOB=01).
      do j = n, 1, -1
        b(j) = b(j) / t(j,j)
        if (j > 1) b(1:j-1) = b(1:j-1) - b(j)*t(1:j-1,j)
      end do
    else if (transpose .and. lower) then
      ! Solve transpose(lower T)*x=b (JOB=10).
      do j = n, 1, -1
        if (j < n) b(j) = b(j) - dot_product(t(j+1:n,j), b(j+1:n))
        b(j) = b(j) / t(j,j)
      end do
    else
      ! Solve transpose(upper T)*x=b (JOB=11).
      do j = 1, n
        if (j > 1) b(j) = b(j) - dot_product(t(1:j-1,j), b(1:j-1))
        b(j) = b(j) / t(j,j)
      end do
    end if
  end subroutine dtrsl

  subroutine intpr(label, nchar, value, n)
    character(len=*), intent(in) :: label
    integer, intent(in) :: nchar, value, n
    if (.not. core_output_enabled) return
    if (n > 0) then
      write(output_unit,'(a,1x,i0)') trim(label), value
    else
      write(output_unit,'(a)') trim(label)
    end if
  end subroutine intpr

  subroutine dblepr1(label, nchar, value)
    character(len=*), intent(in) :: label
    integer, intent(in) :: nchar
    double precision, intent(in) :: value
    if (.not. core_output_enabled) return
    write(output_unit,'(a,1x,es24.16)') trim(label), value
  end subroutine dblepr1

  subroutine dblepr(label, nchar, values, n)
    character(len=*), intent(in) :: label
    ...
    if (.not. core_output_enabled) return
    write(output_unit,'(a)') trim(label)
    if (n > 0) write(output_unit,'(*(1x,es16.8))') (values(i), i=1,n)
  end subroutine dblepr

end module lbfgsb3_core_mod
