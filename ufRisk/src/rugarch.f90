...
contains
   ...
   pure function rugarch_filter(series, specification, parameters, &
      realized_variance, mean_regressors, variance_regressors, backcast_value) result(out)
      ...
      real(dp), intent(in), optional :: backcast_value !! Optional fixed variance backcast.
      ...
      if (present(backcast_value)) backcast = max(backcast_value,100.0_dp*tiny(1.0_dp))
      ...
      variance_value = backcast
      component_value = previous_component
      scale_value = 0.0_dp
      ...
      do time = 1, observations
         ...
         case (rugarch_model_realgarch)
            if (time > 1) then
               lag = time - 1
               log_variance = parameters%omega + &
                  parameters%alpha(1)*log(realized_variance(lag)) + &
   ...
   pure elemental real(dp) function rugarch_log_density(value, distribution, &
      ...
      real(dp) :: scale, skew_value, first_moment, raw_mean
   ...
   pure function rugarch_forecast(fit, horizon, mean_regressors, &
      ...
      variance_value = variances(observations)
      scale_value = 0.0_dp
      component_value = component_path(observations)
      do step = 1, horizon
   ...
   function rugarch_simulate(specification, parameters, observations, burnin, &
      ...
      if (specification%variance_model == rugarch_model_egarch) &
         ...
         specification%distribution, parameters%shape, parameters%skew, &
         parameters%lambda, 1.0_dp)
      variance_value = initial_variance
      scale_value = 0.0_dp
   ...
   pure function rugarch_distribution_name(distribution) result(name)
      ...
      integer, intent(in) :: distribution !! Distribution code.
      character(len=12) :: name
      character(len=24) :: full_name

      full_name = shared_distribution_name(distribution)
      name = full_name(1:12)
      return
      select case (distribution)
      case (rugarch_distribution_normal)
